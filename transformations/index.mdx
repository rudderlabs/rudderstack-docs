...
### External Api FetchV2

FetchV2 is a wrapper around fetch call to provide more access to fetch response properties

The following properties are present in the fetchV2 response:
| Property Name   | Description                                                                                                                                                                    |
| :-------------- | :----------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `status`      | Status code of fetch response say 200|
| `url` | URL of fetch api                                                                                                                |
| `headers`     | Response headers                                                                                                                             |
| `body`     | Response body in JSON or TEXT, by default JSON

<div class="infoBlock">

  Use <code class="inline-code">try-catch</code> block syntax to capture cause for failures during fetch call like when timeout is applied 
</div>

An example of how to use the `fetchV2` function in transformations is shown below:
```javascript
export async function transformEvent(event) {
  try {
