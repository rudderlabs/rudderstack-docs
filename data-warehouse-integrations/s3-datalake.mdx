...
To send data to your S3 data lake, you will first need to add it as a destination in RudderStack and connect it to a source. Once the destination is enabled, the events will automatically start flowing to your data lake.
...
* **Staging S3 Storage Bucket Name**: Enter the name of the S3 bucket used to store the data before loading it into the S3 data lake. 
...
* **AWS Glue TimeWindowFormat**: Specify the key-value pair with which you want to partition tables. AWS Glue filters data by these partition values without having to read all the underlying S3 data. For example, `dt=2006-01-02` sets partition name to `dt` and partitions the table to the current day.

<div class="infoBlock">

For more information on managing partitions in AWS Glue, refer to the <a href="https://docs.aws.amazon.com/glue/latest/dg/aws-glue-programming-etl-partitions.html">AWS Glue documentation</a>.
</div>

* **S3 Prefix**: RudderStack creates a folder in the S3 bucket with this prefix and pushes all the data to that folder. 
...
If you don't specify a namespace in the settings, RudderStack sets to the source name by default.
</div>
...
<div class="warningBlock">
 If AWS Glue is enabled, ensure that you grant the following permissions:
<ul>
...
RudderStack converts your events into Parquet files and stores them in the configured S3 bucket. Before storing the events, RudderStack groups them by the event name based on the time \(UTC\) they were received. 
...
If AWS Glue is enabled and `timeWindowFormat` is set to `dt=2006-01-02`, the Parquet files will be dumped into the following folders:

| **Event Name**      | **Folder location**                                                                      |
| :------------------ | :--------------------------------------------------------------------------------------- |
| `Product Purchased` | `s3://<bucketName>/<prefix>/rudder-datalake/<namespace>/product_purchased/dt=2019-10-12` |
| `Cart Viewed`       | `s3://<bucketName>/<prefix>/rudder-datalake/<namespace>/cart_viewed/dt=2019-11-12`       |

<div class="infoBlock">
 
For more information on date-time layout options and layout formatting, refer to the <a href="https://pkg.go.dev/time#Time.Format">Time package documentation</a>
</div>
...
If you have not configured any prefix while setting up the S3 data lake destination, omit the prefix. Your path would then be:
<code class="inline-code">s3://testBucket/rudder-datalake/testNameSpace/</code>.
</div>

8. Then, under the **Add another data store** setting, select **No**.
...
10. Next, In the **Schedule** section, select the frequency of your crawler from the dropdown options, as shown:

<img src="../assets/scheduler.png" alt="Scheduler" />

11. In the **Output** section, configure the database that will store all the tables. Under the Grouping behavior for S3 data section, make sure you enable \(tick\) the **Create a single schema for each S3 path** option, as shown:
...
15. Then, click on your crawler and run it. Wait for the process to finish - you should see some tables getting created in your configured database.
...
1. Open your AWS Athena console. Then, go to the same AWS region which you used while configuring AWS Glue. 
...
By default, the namespace is set to your source name if it is not specified in the destination settings.
</div>
