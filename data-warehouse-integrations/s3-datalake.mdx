...
For more information on managing partitions in AWS Glue, refer to the <a href="https://docs.aws.amazon.com/glue/latest/dg/aws-glue-programming-etl-partitions.html">AWS Glue Managing Partitions documentation</a>.
</div>

* **AWS Glue Region**: Specify your AWS Glue region. For example, for N.Virginia, it would be **`us-east-1`**. 
* **AWS Glue TimeWindowFormat**: Specify your key-value pair with which you want to partition tables. AWS Glue filters data by these partition values without having to read all the underlying S3 data. For example, `dt=2006-02-01` sets partition name to `dt` and partitions the table to the current day.
...
If AWS Glue is enabled and `timeWindowFormat` is set to `dt=2006-01-02-15`, the Parquet files will be dumped into the following folders:
...
<div class="infoBlock">
For more information on date time layout options and layout formatting to current date, refer to the <a href="https://pkg.go.dev/time#Time.Format">Time package documentation</a>
</div>
