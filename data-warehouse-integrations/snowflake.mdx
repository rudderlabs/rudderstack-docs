...
2. **Create a role and attach the above policy in AWS**: Follow the steps listed below:
  ...
  a. Create a role of type **Another AWS account**.
  b. Enter your AWS account ID and enable the **Require External ID** option.
  c. For external ID, you can add a dummy value like 0000. This can be modified later.
  d. Attach the policy created in Step 1. Assign a name to this role and keep the role ARN handy for the next step.
...
3. **Grant service account permissions to access the bucket objects**: Create a custom IAM role with the required permissions to access the bucket and fetch the objects by following these steps:
   ...
   a. Log into the GCP console as a Project Editor.
   b. From the dashboard, go to **IAM & Admin** > **Roles**.
   c. Click on **CREATE ROLE**.
   d. Enter the title and description for the custom role.
   e. Click on **ADD PERMISSIONS**.
   f. Filter the following permissions in the **Enter property name or value** and add them to the list:
...
4. **Assign the custom role to the Cloud Storage service account**:
  ...
  a. In your GCP console dashboard, go to **Cloud Storage** > **Browser**.
  b. Select the bucket to configure the access.
  c. Select **SHOW INFO PANEL** in the upper right corner. The information panel for the bucket will pop out.
  d. In the **Add Members** section, get the service account name from the `DESC` command run in Step 2. 
  e. From the **Select a role** dropdown, select **Storage** > **Custom** > `<role>`, where `<role>` is the custom Cloud Storage role.
  f. Click on the **ADD** button. The service account name will be added to the **Storage Object Viewer** role dropdown in the information panel.
