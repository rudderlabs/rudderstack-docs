...
Note that there are some limitations when it comes to using the reserved words in a schema, table, or column names. If these words are used in the event names, traits or properties, RudderStack automatically prefixes an underscore(`_`) when creating the tables or columns for them in your schema.

<div class="warningBlock">
  
Integers are not allowed at the start of the schema or table name. Such schema, column, or table names will be prefixed with an underscore. For instance, <code class="inline-code">25dollarpurchase</code> will be changed to <code class="inline-code">_25dollarpurchase</code>.
</div>

Find more details about the list of reserved keywords in the following warehouse-specific documentation:
