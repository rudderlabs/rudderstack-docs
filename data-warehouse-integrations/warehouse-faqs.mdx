...
You can use either of the following ways to avoid lags in warehouse syncs while running a heavy load:

- Set an **exclusion window** which lets you set a start and end time when Rudderstack will not sync the data to the warehouse. You can run heavy loads during this time interval.

<img src="../assets/warehouse-destinations/exclusion-window.png" />

- Increase the **Sync frequency** interval so that there are lesser number of warehouse syncs throughout the day. You can run them during the non peak load interval by setting the **Sync Starting At** time interval.

<img src="../assets/warehouse-destinations/sync-frequency.png" />

- If you are performing only writes/updates to the warehouse, you can set up read replica to your load generation tool. Connect the read/write one to the Rudderstack.
