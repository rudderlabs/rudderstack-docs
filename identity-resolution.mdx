...
3. Later, the same user uses a laptop and browses through the e-commerce website **anonymously**. Again, RudderStack generates an `anonymous_id_2` corresponding to the user and a device ID `device_id_2`.
...
As discussed in the use case above, RudderStack provides the `merge` API to connect and merge these different user identities in the warehouse.
...
Alternatively, you can use [custom transformations](https://www.rudderstack.com/docs/transformations/) to merge user identifiers in your event data before sending it to the warehouse destination.
