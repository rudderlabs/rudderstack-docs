...
- **Send AnonymousId as Secondary UserId**: This is required for sending the `anonymousId` as user ID to Intercom when the `userId` is absent from the input payload.
...
For both identity and track calls userId or email is a mandatory field. In case they are missing, RudderStack will drop the event.
