...
- **Use device mode to send events**: Enable this setting to send events via the [RudderStack device mode](https://www.rudderstack.com/docs/connections/rudderstack-connection-modes/#device-mode).
...
<Tabs>
  <TabList>
    ...
    <TabPanels>
      <TabPanel>
        <div class="warningBlock">
        RudderStack does not support sending the <code class="inline-code">page</code> to HubSpot via the cloud mode.
        </div>
      </TabPanel>
      <TabPanel>
...
When you provide any custom property, RudderStack automatically converts the name into the lower case and replaces any space with an underscore. This is because HubSpot does not accept properties in the upper case and spaces.
</div>
