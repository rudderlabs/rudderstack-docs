...
You can set the  `event_id` as `traits.event_id: unique identifier` in the `context.traits` or `properties` object. It works for cloud as well as device mode. If you don't set the `event_id`, RudderStack will use the `messageId` to map to Facebook's `eventId`.
