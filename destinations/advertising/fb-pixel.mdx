...
You can set the `event_id` in `traits` , `context.traits` or `properties` object as an unique identifier. It works for cloud as well as device mode. If you don't set the `event_id`, RudderStack will use the `messageId` to map to Facebook's `eventId`.
