...
To set the `event_id`, you can set a unique identifier or a key-value pair in the `context.traits`, `properties`, or `traits` object. It works for cloud as well as device mode. If you don't set the `event_id`, RudderStack will use the `messageId` to map to Facebook's `eventId`.
