...
- **Skip Verify**: The following behavior can be observed depending on the setting:
    - Disabled: No impact
    - Enabled and **Enable Hashing** setting is enabled: Null values are replaced by an empty string `“”` , hashed and sent.
    - Enabled and **Enable Hashing** setting is disabled: Facebook throws an error as it does not take null values.
