...
RudderStack checks the `eventConversionType`event property to determine the type of event to send to Snap Conversion (web, mobile, or offline). If it is not found, RudderStack checks if `channel` is present. If `channel` is absent too, RudderStack automatically sets `eventConversionType` to `OFFLINE`.
...
query: "HDMI cable",
eventConversionType: "MOBILE_APP"
});
```
