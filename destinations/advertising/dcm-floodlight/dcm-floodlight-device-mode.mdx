...
RudderStack lets you send your event data to DCM Floodlight destination via the device mode using the native web SDK. It is recommended to use `gtag` instead of `iframe` or `img` tag. RudderStack uses global site tagging in device mode.
...
| :-------------| :-----------------|:-----------------|
| `google_nid`  | Network ID (NID) for the bidder account | from `externalId` `googleNetworkId` |
