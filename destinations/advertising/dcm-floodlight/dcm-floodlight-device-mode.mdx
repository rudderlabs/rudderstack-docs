...
RudderStack lets you send your event data to DCM Floodlight destination via the device mode using the native web SDK. 
...
| :-------------| :-----------------|
| `google_nid`  |  from `externalId` `googleNetworkId` |
| `google_hm`   |  `anonymousId` (Converts it to Base64-encoded ASCII string)  |
