...
- **Conversion Linker**: Enable this setting to use Google Ads' Conversion Linker feature.
...
- **Dynamic Remarketing**: Enable this setting to leverage the dynamic remarketing feature of Google Ads. 

<div class="infoBlock">
 
The <strong>Dynamic Remarketing</strong> feature lets you tailor your ads based on your users' previous interactions with your website. For more information on this feature, refer to the official Google Ads <a href="https://support.google.com/google-ads/answer/7305793">documentation</a>.
</div>

  <div class="infoBlock">
 
As this is a device mode-only integration, the <strong>Use device-mode to send events</strong> option will always be enabled.
</div>
...
A sample `page` call after enabling the **Dynamic Remarketing** option in the RudderStack dashboard is shown below:

```javascript
rudderanalytics.page('page_view', {
...
RudderStack maps `properties.value`, `properties.currency`, and `properties.order_id` to the supported Google Ads `value`, `currency`, and `transaction_id` fields, respectively. RudderStack also maps `properties.revenue` to `value`.
