...
RudderStack maps `properties.value`, `properties.currency`, and `properties.order_id` to the supported Google Ads `value`, `currency`, and `transaction_id` fields, respectively. RudderStack also maps `properties.revenue` to `value`.  Note that this is applicable only when <strong>Dynamic Remarketing</strong> is disabled in the dashboard settings.
</div>
