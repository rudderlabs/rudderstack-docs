...
The events are automatically grouped in a batch of 50 events per request using the `batch` call. However, if the payload contains **test_event_code** property, batching won't be applied. For more information on the `batch` call, please refer to the <a href="https://www.rudderstack.com/docs/rudderstack-api/http-api/#11-batch">RudderStack API Specification</a> documentation.
...
<pre> {"code":40002,"message":"No permission to operate pixel code: D1VF1C8AIKHA8PLC3GAM","request_id":"3021042921460500023604014862AFFF38"}</pre>
