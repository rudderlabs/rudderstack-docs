...
You must send the device-specific information for DCM Floodlight, such as <code class="inline-code">IDFA</code> or <code class="inline-code">advertisingId</code>, which are mapped to the <code class="inline-code">context.device.advertisingId</code>. You can do this by using the RudderStack's mobile SDK as it collects this information automatically. However, in case you are using a RudderStack server-side SDK, the <code class="inline-code">advertisingId</code> needs to be included manually.
</div>
