...
You must send the device-specific information for DCM Floodlight, such as the <code class="inline-code">IDFA</code> or the <code class="inline-code">advertisingId</code> . This can be done using the RudderStack's mobile SDK as it collects this information automatically. However, in case you use any server-side SDK, the <code class="inline-code">advertisingId</code> needs to be included manually.
</div>
