...
![Connection Settings for Facebook App Events](<../../.gitbook/assets/image (118).png>)
...
| ----------------------------------- | ----------------- | -------------------------------------------------------------------------- |
| **Data Processing Options State**   | **`0`**           | <p>Use Facebook’s geolocation to <br>determine the end-user's state.</p>   |
| **Data Processing Options Country** | **`0`**           | <p>Use Facebook’s geolocation to <br>determine the end-user's country.</p> |
...
* In some cases, you can delay the collection of `advertiser_id` to obtain user consent or fulfill any legal obligations instead of disabling it entirely. \
  \
  To do so, call the `setAdvertiserIDCollectionEnabled()` method of the `FacebookSDK` class and set it to `true` . This re-enables the collection of `advertiser_id` after the end-user provides the required consent, as shown:
