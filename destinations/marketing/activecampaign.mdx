...
In the above snippet, RudderStack captures relevant information about the user such as the `userId` as well as the associated traits such as `email`, `phone`, and the name (`firstName` and `lastName`) of that user.

<div class="infoBlock">

The <code class="inline-code">email</code> trait is a mandatory trait for mapping a user to ActiveCampaign. If a user already exists, the new values will be updated for that user.
</div>
...
The <code class="inline-code">tags</code> property should contain an array of tags which you want to associate with the user. If any tag is already created in the destination previously, RudderStack will automatically skip the creation of that tag.
</div>

### Custom fields

RudderStack also lets you update a contact’s custom fields in ActiveCampaign, with this integration.

To send custom fields to ActiveCampaign, you will need to first create the custom fields in ActiveCampaign for each custom field that you want to send. Then, when you call `identify` with the keys matching those traits, the custom fields for that contact will be automatically updated. You can use the `fieldInfo` trait to set values to the custom fields, as shown in the following snippet:
...
The <code class="inline-code">fieldInfo</code> property contains the value of the field information that you want to store for that contact. For using this feature, you have to create the fields from your ActiveCampaign dashboard \(For example - Interest, Country\) before passing the values for the given user.
</div>
...
In the above snippet, RudderStack captures the information related to the `Product Purchased` event, along with any additional information about that event - in this case the name of the product.
...
If you want to set a value for the <code class="inline-code">screen</code> and <code class="inline-code">track</code> type event, you need to pass <code class="inline-code">name</code> in the event properties, as shown in the above example.
</div>
...
No, your `screen` and `track` event names must contain only alphanumeric characters. ActiveCampaign prescribes that the `screen` and `track` event names must contain only alphabets and numbers, and does not allow any special characters to be used.
