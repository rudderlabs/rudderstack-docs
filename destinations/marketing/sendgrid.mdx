...
title: "Sendgrid"
description: >-
 Step-by-step guide to set up SendGrid as a destination in RudderStack.
---
...
<div class="infoBlock">
 ...
 Follow our guide on <a href="https://rudderstack.com/docs/how-to-guides/adding-source-and-destination-rudderstack">Adding a Source and Destination in RudderStack</a> for more information.
</div>

* Assign a name to the destination and click on **Next**. You should then see the following screen:

<img src="../../assets/sendgrid-connection-settings-1.png" alt="Sendgrid connection settings" />
<img src="../../assets/sendgrid-connection-settings-2.png" alt="Sendgrid connection settings" />
<img src="../../assets/sendgrid-connection-settings-3.png" alt="Sendgrid connection settings" />
<img src="../../assets/sendgrid-connection-settings-4.png" alt="Sendgrid connection settings" />
...
<div class="infoBlock">
 ...
 You can also send the subject via the <code class="inline-code">properties</code> in the <code class="inline-code">track</code> call. For more information on the <code class="inline-code">properties</code> object, refer to the <strong>Track</strong> section below.
</div>
...
You can also set the Template ID via the <code class="inline-code">properties</code> in the <code class="inline-code">track</code> call. Note that the template ID passed in the <code class="inline-code">properties</code> will override the ID specified in the RudderStack dashboard.
</div>

* Next, you can enable or disable the **Get mail by traits** option. If enabled, this option allows RudderStack to search for the user's `email` inside the user's `traits`. If it is found, then the event can be sent to SendGrid without the any properties in track call.

<div class="warningBlock">
 
This option is only useful when the <code class="inline-code">properties</code> in track call is empty. SendGrid also requires either the <code class="inline-code">templateId</code> or <code class="inline-code">content</code> to be present for a successful event delivery. 
</div>
...
You can also send the email and name inside <code class="inline-code">replyTo</code> via the <code class="inline-code">properties</code>. Note that these fields will override the <strong>Reply To Email</strong> and <strong>Reply To Name</strong> fields set in the RudderStack dashboard.
</div>

<div class="infoBlock">
 
For more information on how to send these details via the <code class="inline-code">replyTo</code> object, refer to the <strong>replyTo</strong> section below.
</div>
...
You can additionally include <code class="inline-code">email</code> and <code class="inline-code">name</code> inside the <code class="inline-code">from</code> object via the <code class="inline-code">properties</code> (shown below). Note that this will override the fields specified in the RudderStack dashboard.
</div>
...
If a <code class="inline-code">track</code> call is made with event not in this list, the call will be <strong>dropped</strong>.
</div>
...
The <code class="inline-code">content</code> array can also be sent via <code class="inline-code">properties</code> (shown below), which will override the dashboard settings.
</div>
...
SendGrid requires that each attachment element must contain <strong>content</strong> and <strong>filename</strong> The attachments can also be sent via the <code class="inline-code">properties</code> (shown below) which will override the dashboard settings.
</div>
...
The footer can also be sent via <code class="inline-code">properties</code> (shown below), which will override the dashboard settings.
</div>
...
The sandbox mode can also be enabled or disabled via the <code class="inline-code">properties</code> (shown below), which will override the dashboard setting.
</div>
...
The `track` call lets you send an event to SendGrid along with its properties. Note that the properties specified in track call will override the settings specified in the RudderStack dashboard.
...
* If the **Get mail from traits** option is enabled in the RudderStack dashboard and the `properties` is not sent in track call, RudderStack will look for the `email` in the event traits. If found, RudderStack will create a `personalizations` object and assign email to the `to` field. 
