...
<div class="infoBlock">
 ...
 The token is the Public API key used to configure Klaviyo as a destination in RudderStack. This is a mandatory field. Also, the customer properties must contain atleast one of <code class="inline-code">email</code> or  <code class="inline-code">phone_number</code>.
</div>
...
Note that specifying <code class="inline-code">event_id</code> is not mandatory, as Klaviyo automatically assigns the timestamp to the event. However, if specified explicitly, it <strong>must be unique</strong>. Events with duplicate <code class="inline-code">event_id</code> will be discarded automatically.
</div>
