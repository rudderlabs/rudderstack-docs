...
RudderStack uses the `identify` call to set a `userId` which is passed to AdRoll every time a subsequent `track` call is made. If the `email` trait is present in the `identify` call, RudderStack maps it to AdRoll's `window._adroll_email` property.
