...
- **Data Center**: Select the data center where you want to send the data:
  -  Standard: Default option. The base URL is https://api.getblueshift.com 
...
The `group` call lets you associate a user with a group. For each `group` call, the `event` API is triggered and an `identify` event is sent with the `groupId`. It then creates a group and adds that user to the group.
