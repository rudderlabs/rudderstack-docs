...
[Blueshift's](https://blueshift.com/) SmartHub customer data platform helps consumer brands intelligently scale customer engagement on every channel. It helps to increase the customer engagement frequency with your brand and brings it closer to the customer. 
...
- **Users API Key**: Your Users API Key generated for your Blueshift account in the **Account Settings** section under **API Keys**.
- **Blueshift EU data center**: Enable this option to send data to the EU servers. When enabled, the base URL for EU region turns out to be: `https://api.eu.getblueshift.com`

## Identify

The `identify` call lets you create new or update the existing customers. It also lets you record the traits about them like their name, email address, etc. It is recommended to limit the `identify` calls to 50 per second.
...
The following table details the mapping of the fields specified in the RudderStack and Blueshift, along with the relevant guidelines:

| RudderStack field | Blueshift field | Type | Presence |
| :-----| :--------| :------|:------|
| `email` | `email` | String |Required |
| `userId`  | `customer_id` | String | Required |
...
| `traits.gender`/`context.traits.gender`|`gender`| String | Optional |
...
BLueshift supports custom events to track any custom user action on your site or app. For more information, refer to the <a href ="https://developer.blueshift.com/docs/your-sites-event-data#custom-event">Blueshift Custom Event</a>.
</div>
...
Ensure that you specify values for at least one of these fields, namely, `customer_id`, `event`, `device_id`, `cookie` or `email`. Also, specify the value of the `cookie` property for anonymous users.
...
The <code class="inline-code">event</code> name should <strong>not</strong> contain period (.), numeric value, or more than 64 characters. The spaces are automatically converted  to underscore (_).
</div>

## Group

You can use the `group` call to assign users to a list. To create an empty user list, use the [userList](https://developer.blueshift.com/reference/post_api-v1-custom-user-lists-create) API of Blueshift.
...
| `customer_id` | `identifier_key` | Required |
...
If a `list_id` already exists, the user is added to the list. Otherwise, a new empty user list is created and the user is added to that list.
</div>
