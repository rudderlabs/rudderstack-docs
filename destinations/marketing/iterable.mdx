...
The `alias` call lets you merge different identities of a known user. To send an `alias` call to Iterable, you must set `previousId` to the current email and `userId` as new email, as shown below:
