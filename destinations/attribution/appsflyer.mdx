...
- **App ID**: RudderStack supports all the sources for the AppsFlyer destination. If your source is other than **Android** or the **Apple** sources (iOS, watchOS, ipados, tvOS) then transformation in dashboard is needed to manipulate the os name.
...
RudderStack supports all the other sources for the AppsFlyer destination, in addition to the Android and iOS sources. However, you will need to <a href="">add a transformation</a> to the destination to manipulate the OS name. Refer to the <a href="#connecting-appsflyer-to-non-androidios-sources">Connecting AppsFlyer to non-Android/iOS sources</a> section below for more information.
</div>
