...
- **App ID**: RudderStack supports all the sources for the AppsFlyer destination. If your source is other than **Android** or the **Apple** sources (iOS, watchOS, iPadOS, tvOS) then a user transformation in the dashboard is needed to manipulate the `os` name and version.
