...
- **App ID**: Rudderstack supports all sources for Appsflyer destination. If source is other than `android` or `apple family` (ios, watchos, ipados, tvos) then transformation in dashboard is needed to manipulate the os name.
