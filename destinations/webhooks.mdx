...
- **URL Method:** This is the HTTP method of the request that would be sent to the configured endpoint. Supported methods are `POST`, `PUT` and `GET`. By default, the `POST` method is used.
- **Headers:** Add custom headers for your events via this option. These headers will be added to the request made from RudderStack to your webhook. By default, RudderStack adds the following headers for `POST` and `PUT` requests:
