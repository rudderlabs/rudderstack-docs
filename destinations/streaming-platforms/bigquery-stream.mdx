...
The user or the account you have chosen should have the required access to the BigQuery APIs. Refer to the <strong>User Permissions</strong> section for more information.
</div>
...
If you disable the destination and want to re-enable it, you will need to authorize the credentials (give RudderStack the required permissions to access BigQuery Stream) again.
</div>
...
To stream events into BigQuery successfully, make sure the event payload has the correct format that matches the schema of the table specified in the RudderStack dashboard.
</div>
...
To successfully stream the events, the event payload tracked from your JavaScript SDK should look like the following sample:
