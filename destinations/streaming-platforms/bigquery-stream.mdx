...
description: Step-by-step guide to stream your event data from RudderStack to Google BigQuery via Google's streaming API.
---
...
- **Insert ID**: This is an **optional** field. Enter your insert ID which will be used by Google to de-duplicate the data sent to BigQuery.
...
Google leverages the insert ID to de-duplicate the data sent to BigQuery. The insert ID is essentially an event property that helps you uniquely identify an event.

<div class="warningBlock">

RudderStack currently supports only <strong>numeric</strong> or <strong>string</strong> values as the insert ID.
</div>
...
If you send an event called `Insert Product` to BigQuery, the `productId` can be used to uniquely identify the product. Hence it can be passed as the insert ID. BigQuery will perform the de-duplication of the data based on this `productId`. 
