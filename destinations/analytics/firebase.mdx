...
RudderStack supports Firebase as a [device mode destination](https://www.rudderstack.com/docs/rudderstack-cloud/rudderstack-connection-modes/#device-mode) to which you can seamlessly send your customer data for analytics.
...
1. Connect this destination to your Android/iOS/Unity/React Native source. You should then see the following screen:

<img src="../../assets/event-stream-destinations/firebase-connection-settings.png" alt="Firebase connection settings" />

2. As this is a device mode-only integration, click on **Next** to complete the destination setup.
...
```
      ...
      <TabPanel>
        <div class="infoBlock">
          If you’re using the Firebase iOS device mode v1.0.2, update to the <a href="https://cocoapods.org/pods/Rudder-Firebase">latest version</a> of the iOS device mode to get the latest updates, bug fixes, and performance enhancements.
        </div>
...
The [`identify`](https://www.rudderstack.com/docs/rudderstack-api/api-specification/rudderstack-spec/identify/) call sets the `userId` through the `setUserId` method from `FirebaseAnalytics`. RudderStack sets the other user properties from `RudderTraits` to Firebase using the `setUserProperty` method. RudderStack ignores `age`, `gender` and `interest` as these are reserved by Google.
...
RudderStack's [`track`](https://www.rudderstack.com/docs/rudderstack-api/api-specification/rudderstack-spec/track/) is mapped to the standard Firebase events wherever possible. 

The following table lists the event mapping:
...
Apart from the above-mentioned events, the following standard Firebase events are not mapped to any RudderStack event. You can mention the exact event names from the following list to be sent as a standard event in Firebase.
...
Firebase enforces a key value of length less than 100. RudderStack takes the substring of length 100 from the beginning if the length exceeds this permitted value.
</div>
...
- If the length of the key is more than 40, RudderStack takes the substring of length 40 from the beginning.
...
You can check the events and their properties along in the [Firebase DebugView](https://firebase.google.com/docs/analytics/debugview). To enable it for Android, run the following command from your terminal:
