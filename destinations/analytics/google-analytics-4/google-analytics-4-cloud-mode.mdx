...
A sample `track` call where `gtag` is used, is as shown:
...
A sample `track` call when `firebase` is used, is as shown:
...
You can pass the custom user properties to any of the events by passing it as `properties.user_properties`. Refer to the <a href="https://developers.google.com/analytics/devguides/collection/protocol/ga4/user-properties?client_type=gtag">User properties</a> for more information.
</div>
