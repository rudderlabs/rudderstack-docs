...
RudderStack supports both the `gtag` and `firebase` ways for tagging in websites in the cloud mode. However:
- If you use `gtag`, passing the `client_id` parameter becomes mandatory.
- If you use `firebase`, passing the `app_instance_id` parameter becomes mandatory.

<div class="infoBlock">
  
Refer to the <a href="https://developers.google.com/analytics/devguides/collection/protocol/ga4/reference?client_type=gtag#payload">Measurement Protocol (Google Analytics 4)</a> guide for more information.
  </div>

The mappings for the above parameters are listed below:
...
A sample `track` call when `gtag` is used is shown below:
...
A sample `track` call when `firebase` is used is shown below:
...
RudderStack maps the `page` call to a `page_view` event (by default) and passes it to Google Analytics 4 as a custom event. As mentioned above, the cloud mode supports both the `gtag` and `firebase` ways for tagging in websites. 

A sample `page` call when `gtag` is used is shown below:

```javascript
rudderanalytics.page();
```

A sample `page` call when `firebase` is used is shown below:
...
The `app_instance_id` can be retrieved through the Firebase SDK depending on the platform on which the SDK is installed: 
...
Refer to the [GA4 document](https://developers.google.com/analytics/devguides/collection/protocol/ga4/reference?client_type=firebase#payload_post_body) for more information. 
