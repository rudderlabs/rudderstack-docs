...
RudderStack lets you send your event data to Google Analytics 4 destination via the cloud mode.
...
- If you pass the `client_id` as `gtag`, passing the `client_id` parameter becomes mandatory.
- If you pass the `client_id` as `firebase`, passing the `app_instance_id` parameter becomes mandatory.
...
A sample `track` call where `client_id` is `gtag` is as shown:
...
A sample `track` call where `client_id` is `firebase` is as shown:
...
The below table lists the RudderStack and Google Analytics 4 events mappings:
...
The below table lists the RudderStack and Google Analytics 4 properties mappings based on the specific RudderStack events:
...
The below table details out the mappings for above events for the `Items` array:
...
You can pass the custom user properties to any of the events by passing it as `properties.user_property`. Refer to the <a href="https://developers.google.com/analytics/devguides/collection/protocol/ga4/user-properties?client_type=gtag">User properties</a> for more information.
</div>
