...
<ul>
    ...
    <li>When configured correctly, the <code class="inline-code">Info.plist</code> entry should look as follows:</li>
...
<li>To apply FullStory to all variants, including those used at debug time, add the following line below <code class="inline-code">org</code>:
<span>
...
### How to solve Build was configured to prefer settings repositories over project repositories but repository 'Google' was added by build file 'build.gradle'

Go to your `settings.gradle` file located in the root and then comment or remove the third line: `repositoriesMode.set(RepositoriesMode.FAIL_ON_PROJECT_REPOS)`.
