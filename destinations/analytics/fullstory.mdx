...
<div class="infoBlock">
    ...
    In the above snippet, <code class="inline-code">some_value</code> represents the value for the <b>FS ORG</b> field.
</div>
...
<ul>
<li>In your `Podfile`, add the following extension:
<span>
...
<li>After adding the dependencies followed by a `pod install` command, add the following imports to your `AppDelegate.m` file:
<span>
...
<li>Then, initialize your `RSClient` as shown:
<span>
...
```
    ...
    To complete the FullStory integration, you need to tell the framework which FullStory organization to record to. To do so, follow these steps:
    <ul>
    <li>Open your app's `Info.plist`. </li>
    <li>From the menu, choose <b>Editor</b> - <b>Add Item</b>. Set the key name to `FullStory`, and the type to `Dictionary`. </li>
    <li>Right-click on the FullStory row, and choose <b>Add Row</b>. Set the key name to `OrgId`, and the type to `String`. For the value, paste your assigned organization ID.</li>
    </ul>
    
    When configured correctly, the `Info.plist` entry should look as follows:
    
    <img src="" alt="Final entry"/>
...
<ul>
<li>Open your `app/build.gradle` \(Module: app\) file and add the following under the `dependencies` section:
<span>
...
<li>Then, add the FullStory Maven plugin to your build script. To do so, add the following snippet into the `Gradle Scripts` section of your root `build.gradle`:
<span>
...
<li> Make sure that you replace `PLUGIN_VERSION` with the correct version of the FullStory Android plugin. You can find the latest release notes <a href="https://docs.google.com/document/d/e/2PACX-1vRBDZpgGoLz2IdHntpXtS_1HWGMwlf6n1J1SurkcyguQB9Liv9OCQ1LHnNmfhxwcZ-ZQSl7QUgcMIR5/pub">here</a> (this is currently 1.18.0).
    </li>
    
<li>To apply the FullStory plugin, add the following snippet into your app-specific `build.gradle` (if your Gradle file adds plugins via plugin ID):
<span>
...
<li>Add the following permissions, if not added already, to your `AndroidManifest.xml`:
    <span>
...
<li>Finally, initialize the RudderStack SDK in your `Application` class' `onCreate()` method as shown:
    <span>
