...
Singular supports tracking revenue events. It receives revenue tracking whenever an event containing the `revenue` property is sent(including a zero value). Optionally, you can also pass the `currency` field as an [ISO code](https://www.iso.org/iso-4217-currency-codes.html). 
...
The `reset` method resets the current user's identity and creates a new anonymous session. It should be called when the users log out.
