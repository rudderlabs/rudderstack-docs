...
- **Send Both Timestamp and VisitorID for Timestamp Optional Reporting Suites**: This setting lets you choose whether to send both timestamp and the visitorID when sending events to server-side, in case you have Timestamp Optional Reporting Suite.

<div class="warningBlock">

Enabling this setting might lead to an out of order data. Hence, it is not recommended by Adobe. In order to make it work, you need to have optional timestamp setting enabled in the reporting suites.
</div>

- **No Fallbacks for Visitor ID**: Enabling this setting removes the fallbacks. However, this setting is applicable only for the server-Side when the **Drop Visitor ID** setting is disabled and a marketingCloudId is sent in the Adobe Analytics integration object. The default behavior is to set the Adobe Analytics visitorID based on the destination specific setting for visitorId, falling back to userId then anonymousId.
