...
When `send_to_destination` is set to `true`, the API returns the destination's response upon receiving the event payload. If set to `false`, this stage is skipped completely.
