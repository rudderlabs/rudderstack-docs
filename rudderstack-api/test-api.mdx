...
You will get the following error message in the absence of a message `type` in the payload, or if you specify any other `type` apart from the ones in the above list:

```json
{
    "message": "type is missing from message"
}
```
...
If `type` is invalid or missing in the event payload, the API will return the following error:
