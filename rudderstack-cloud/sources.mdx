...
5. Configure the source with relevant connection settings.
...
<div class="infoBlock">
  Note the source **Write Key**. It is required while configuring the RudderStack SDK to track and collect events from the source platform.
</div>
...
- **Rename**: Lets you rename the source by clicking on the edit icon next to the source name.
