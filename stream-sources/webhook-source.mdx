...
When you create a webhook source in the RudderStack dashboard, you get a write key associated with it. Use this write key in a webhook endpoint in the following format:
...
<ul>
<li>For more information on the data plane URL, refer to <a href="https://rudderstack.com/docs/get-started/installing-and-setting-up-rudderstack/#what-is-a-data-plane-url-where-do-i-get-it">this</a> section.</li>
<li>The write key is automatically appended to the webhook endpoint once you add a webhook source in the <a href="https://app.rudderstack.com/">RudderStack dashboard</a>. Refer to <a href="https://rudderstack.com/docs/connections/adding-source-and-destination-rudderstack/">this</a> guide for more information on how to add a source in RudderStack. </li>
</ul>
</div>
...
This section details the steps involved in setting up a webhook source in the RudderStack dashboard. As an example, we will configure [**Mailchimp**](https://mailchimp.com) as a source in RudderStack.
...
- Then, add a destination in RudderStack and connect it to this webhook source. As an example, we will configure [**Google Analytics**](https://rudderstack.com/docs/destinations/analytics/google-analytics-ga/) as a destination, as shown:
...
RudderStack then sends this payload to your destination (Google Analytics, in this example).

## Contact us

If you come across any issues while setting up and using a webhook source in RudderStack, you can [**contact us**](mailto:%20docs@rudderstack.com) or start a conversation on our [**Slack**](https://rudderstack.com/join-rudderstack-slack-community) channel.
