...
Make sure you add the write key as a query parameter to the URL. This is required to prevent the webhook from failing for the lack of a valid write key. 
</div>
...
Refer to the following table to understand how various events from App Center are converted into Rudderstack `track` calls:
