...
RudderStack supports periodic flushing of events from the RudderStack SDK’s internal database to the Rudder server, irrespective of whether the app is opened or not. This ensures that the events triggered just before closing the application are sent from the device even if the application is not opened by the user for a long time.
