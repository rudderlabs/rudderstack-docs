...
You can also enable the automatic recording of screen views by passing `recordScreenViews` as `true` while initializing the `rudderClient`. It automatically sends a `screen` call for every screen that a user views/visits. The default value for `recordScreenViews` is `false`. 

The `recordScreenViews` parameter records the screen views by the native Android Activities or iOS View Controllers only and not by the React Native. To track the screen views by React Native, you can use the following code snippet:
