...
We use the `deviceId` as anonymousId by default. You can use the `putAnonymousId` method to override the default `anonymousId`, as shown:
...
On iOS You need to call <code class="inline-code">putAdvertisingId</code> method before calling <code class="inline-code">initialize</code>
