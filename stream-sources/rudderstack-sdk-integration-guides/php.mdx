...
We accept the <code class="inline-code">DATA_PLANE_URL</code> with or without the protocol. Also, if the protocol is missing, it is automatically prepended to the URL based on the <code class="inline-code">ssl</code> option(default value <code class="inline-code">true</code>).

</div>

Below are the data_plane_url formats accepted by the system:

| **SSL value** | **Accepted Format**             |
| :------------ | :------------------------------ |
| `ssl=true`    | `https://example.dataplane.com` |
| `ssl=true`    | `example.dataplane.com`         |
| `ssl=false`   | `http://example.dataplane.com`  |
| `ssl=false`   | `example.dataplane.com`         |
