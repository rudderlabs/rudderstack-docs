...
{% hint style="success" %}
You can find the JavaScript SDK's GitHub repository [**here**](https://github.com/rudderlabs/rudder-sdk-js).
{% endhint %}
...
{% hint style="info" %}
You need to replace the `<WRITE_KEY>` with the write key in the RudderStack dashboard and `<DATA_PLANE_URL>` with the data plane URL.
{% endhint %}
...
{% hint style="info" %}
If the `anonymousId` is provided by the user using the `setAnonymousId` method, the user-specified `anonymousId` overrides the `anonymousId` generated by the SDK. 
