...
RudderStack lets you set a `secureCookie` flag while loading the SDK. This flag allows you to control how you send the cookie to the storage backend, i.e. via HTTP or HTTPS.
...
rudderanalytics.load(WRITE_KEY, DATA_PLANE_URL, {
  configUrl: string, // defaults to https://api.rudderlabs.com
  logLevel: "DEBUG",
  secureCookie: true // defaults to false
});
```

<div class="infoBlock">

  Setting <code class="inline-code">secureCookie</code> to <code class="inline-code">true</code> results in RudderStack sending the cookie over a secure connection, i.e. HTTPS. If set to <code class="inline-code">false</code>, the cookie is sent via HTTP.
</div>
