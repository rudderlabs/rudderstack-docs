...
By default, the device mode destinations are loaded from https://cdn.rudderlabs.com/v1.1/js-integrations/ in 2.x.x, depending on the dashboard settings. However you can also choose any of the below ways to load the device mode destinations:
...
3. Then, go to **Behaviors**, and check that the sub-path `/v1.1/js-integrations/*` is **not configured to be blocked** in any way.
...
4. Finally, use `destSDKBaseURL` option in the load call like below
...
3. Use `destSDKBaseURL` option in the load call like below
