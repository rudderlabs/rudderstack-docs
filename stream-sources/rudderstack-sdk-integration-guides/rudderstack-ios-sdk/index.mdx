...
<div class="successBlock">
  ...
  Starting from version <strong>1.3.0</strong>, the iOS SDK supports the <b>watchOS</b> platform.  You can now integrate the SDK with your watchOS apps and seamlessly track the user events without the need for any additional configuration.
</div>
...
| `recordScreenViews`     | `boolean` | Specify whether the SDK will capture screen view events automatically.
| `enableBackgroundMode`  | `boolean` | Specify whether the SDK should enable background mode to send events for some time when the app moves to background.                                                                                                                                                                                         | `false`                                                                    |
...
### How to ensure the events made just before closing/backgrounding the app are sent immediately and not on the next launch of the app ?

You can do this by setting `withBackgroundMode` to `YES` while creating the `RSConfig` object as shown below. By doing this your app actually requests `iOS` for some additional background run time to run the app which in turn enables the SDK to send those events made just before the app closing/backgrounding immediately instead on the next launch.
