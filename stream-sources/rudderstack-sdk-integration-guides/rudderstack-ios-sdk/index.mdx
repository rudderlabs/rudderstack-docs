...
<div class="successBlock">
  ...
  Starting from version <strong>1.3.0</strong>, the iOS SDK supports the <strong>watchOS</strong> platform.  You can now integrate the SDK with your watchOS apps and seamlessly track the user events without the need for any additional configuration.
</div>
...
| `enableBackgroundMode`  | `boolean` | Specify whether the SDK should send the events for some time when the app moves to background. Currently it is available for <code class="inline-code">iOS</code> & <code class="inline-code">tvOS</code> only. | `false`                                                                    |
...
To ensure that the events tracked just before closing/backgrounding the app are sent to RudderStack immediately, you can set `withBackgroundMode` to `YES` while creating the `RSConfig` object as shown below:
...
```objectivec
RSConfigBuilder *builder = [[RSConfigBuilder alloc] init];
[builder withBackgroundMode:YES];
...
This feature of the SDK actually relies on the background mode feature offered by the iOS and there isn't any arbitary number on the amount of background run time the apps gets this way, its completely abstracted by iOS. You can find more information regarding the same [**here**](https://www.raywenderlich.com/5817-background-modes-tutorial-getting-started#toc-anchor-008).
