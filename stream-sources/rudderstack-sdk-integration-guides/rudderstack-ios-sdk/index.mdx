...
The **RudderStack iOS SDK** integrates RudderStack with your iOS application track the event data and send it to your preferred destinations.
...
RudderStack uses SQLite to save the events to local database before sending it to RudderStack data-plane. Making calls like <code class="inline-code">SQLite.shutdown()</code> which is not thread-safe might lead to unexpected crash.
...
If you come across any bugs or discrepancies while using the iOS SDK, you can also open a new issue on our [**GitHub Issues page**](https://github.com/rudderlabs/rudder-sdk-ios/issues/new).
