...
Since the AMP source sends the data directly to the RudderStack data plane, we support only <code class="inline-code">cloud mode</code> destinations for AMP projects. For more information on the cloud mode, refer to the <a href="https://rudderstack.com/docs/connections/rudderstack-connection-modes/">RudderStack Connection Modes</a> guide.
</div>
...
You can send extra properties for your `page` or `track` events to add more information along with the event request. Once you mention the properties as `extraUrlParams` in the `amp-analytics` tag of your implementation, it will be passed to RudderStack for further processing.
...
A sample call with custom properties is shown below:
...
In case of any queries, feel free to [**contact us**](mailto:%20docs@rudderstack.com) or start a conversation on our [**Slack**](https://rudderstack.com/join-rudderstack-slack-community) channel. In case of any bugs or discrepancies you come across while using this SDK, you can also open an issue on our [**GitHub Issues**](https://github.com/rudderlabs/rudder-sdk-amp/issues) page.
