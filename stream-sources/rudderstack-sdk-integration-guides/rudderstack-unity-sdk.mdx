...
RudderStack provides some pre-defined API's for building the RudderTraits object, like `PutEmail()`, `PutAge()` etc. These can be used to set the values of standard traits by directly passing them as parameters. For the custom traits which do not have any pre-defined API, you can use the `Put()` method and pass a key-value pair of the trait as shown in the sample `identify` event below:
