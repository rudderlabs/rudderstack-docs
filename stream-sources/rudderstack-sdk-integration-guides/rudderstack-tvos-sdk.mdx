---
# slug: "/docs/stream-sources/rudderstack-sdk-integration-guides/rudderstack-tvos-sdk"
title: "tvOS"
description: >-
  Detailed technical documentation on RudderStack’s tvOS SDK using XCode to send
  events from your tvOS device to various destinations.
---

# tvOS

## What is the RudderStack tvOS SDK?

The RudderStack tvOS SDK allows you to integrate RudderStack to your tvOS application in order to track event data from your app. After integrating this SDK, you will also be able to send this data to your preferred analytics destination/s such as Google Analytics, Amplitude, and more.

<div class="successBlock">

  You can check the <a href="https://github.com/rudderlabs/rudder-sdk-tvos">codebase on
...
To set up the RudderStack tvOS SDK, there are a few prerequisites as mentioned below:

- You will need to set up a [RudderStack Account](https://app.rudderstack.com).
- Once signed up, your `tvOS` source `writeKey` will appear in the dashboard, as shown:
...
## Installing the RudderStack tvOS SDK

We distribute our tvOS SDK through [Cocoapods](https://cocoapods.org) and [Carthage](https://github.com/Carthage/Carthage). The recommended and easiest way to add the SDK to your project is through `Podfile`. Follow these steps:
...
RudderStack supports all the major API calls across all tvOS devices via the SDK. These include the `track`, `identify`, and `screen` calls.
...
The `optOut` API is available in the RudderStack tvOS SDK starting from version `1.0.24`.
</div>
...
| `controlPlaneUrl`       | `string`  | This parameter should be changed **only if** you are self-hosting the Control Plane. Check the section **Self-Hosted Control Plane** below for more information. The tvOS SDK will add `/sourceConfig` along with this URL to fetch the required configuration. | `https://api.rudderlabs.com`       |

### Self-Hosted Control Plane

If you are using a device mode destination like Adjust, Firebase, etc., the tvOS SDK needs to fetch the required configuration from the Control Plane. If you are using the [**Control Plane Lite**](https://rudderstack.com/docs/get-started/control-plane-lite/) utility to host your own Control Plane, then follow [this guide](https://rudderstack.com/docs/user-guides/how-to-guides/rudderstack-control-plane-lite/#what-is-the-control-plane-url) and specify `controlPlaneUrl` in `RudderConfigBuilder` that points to your hosted source configuration file.
...
The RudderStack tvOS SDK allows you to enable or disable event flow to a specific destination or all the destinations to which the source is connected. You can specify these destinations by creating a `RSOption` object as shown:
...
If you run into any issues regarding the RudderStack tvOS SDK, you can turn on the `VERBOSE` or `DEBUG` logging to find out what the issue is. To turn on the logging, change your `RudderClient` initialization to the following:
...
- Create a `CustomFactory.h` file by extending [`RSIntegrationFactory`](https://github.com/rudderlabs/rudder-sdk-tvos/blob/master/Rudder/RSIntegrationFactory.h), as shown:
...
- Next, create a `CustomIntegration.h` file by extending [`RSIntegration`](https://github.com/rudderlabs/rudder-sdk-tvos/blob/master/Rudder/RSIntegration.h).
...
- Register the `CustomFactory` with the RudderStack tvOS SDK during its initialization, as shown:
...
- RudderStack's tvOS SDK dumps every event it receives to the `dump()` method of the `CustomFIntegration` class. From here, you can process the event and hand it over to the native SDK of the Device Mode destination.
...
- RudderStack's tvOS SDK also triggers the `flush()` method of the `CustomFactory` class on every `flush()` call made via the SDK, which you can use to handle the destination-specific reset logic. You can make a `flush` call using the SDK as shown:
...
As of version `1.1.0`, the tvOS SDK supports the [**tvOS**](https://developer.apple.com/tvos/) platform.
...
### **Why is there a larger difference between `timestamp` and `received_at` for tvOS events vs. Android events?**

This scenario is most likely caused by the default behavior of tvOS apps staying open in the background for a shorter period of time after a user closes them.

When a user closes an tvOS or Android app, events will still continue to be sent from the queue until the app closes in the background as well. Any events still in the queue will remain there until the user reopens the app. Due to this lag, there are some scenarios where there can be significant differences between `timestamp` \(when the event was created\) and `received_at` \(when RudderStack actually receives the events\).

For Android apps, events can be sent from the background after apps close for a longer period of time than tvOS apps, therefore, more of the events coming from the Android SDK have closer `timestamp` and `received_at` times.

### Does RudderStack integrate with SKAdNetwork?

RudderStack does not integrate with SKAdNetwork. However, SKAdNetwork can be directly integrated into an tvOS application alongside RudderStack.
...
For more information on the `optOut` API, refer to the [**Enabling/Disabling User Tracking via optOut API \(GDPR Support\)**](https://rudderstack.com/docs/stream-sources/rudderstack-sdk-integration-guides/rudderstack-tvos-sdk#enabling-disabling-user-tracking-via-the-optout-api-gdpr-support) section above.
</div>
...
If you come across any issues while using the RudderStack tvOS SDK, you can [**contact us**](mailto:%20docs@rudderstack.com) or start a conversation on our [**Slack**](https://rudderstack.com/join-rudderstack-slack-community) channel.
