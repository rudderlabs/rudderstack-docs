...
This guide explains RudderStack’s data retention policy and your options for opting in or out of data storage. Our default policy is not to store your event data, but there are times when it may be useful for you to access or replay recent event data. See <Link to="/user-guides/administrators-guide/event-replay/">Event Replay</Link> for details on this feature.

We provide 3 options for managing your event data:

- Option 1. Do not store it anywhere
- Option 2. Store in your own cloud storage (Recommended)
- Option 3. Store in RudderStack cloud on a rolling 30-day basis

The sections below define different types of RudderStack data and provide steps on opting in to the right setup for your needs.
...
2. Error Codes 
3. Customer User Records (e.g., usernames, billing-related details)
...
### Option 1. Do not store event data
...
### Option 2. Store event data in your own cloud storage (Recommended)
...
### Option 3. Store event data in RudderStack cloud storage
