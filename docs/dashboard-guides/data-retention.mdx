...
RudderStack provides 3 options for managing your event data:

- Do not store it anywhere
- Store in your own cloud storage (**Recommended**)
- Store in RudderStack Cloud on a rolling 30-day basis

The following sections below define the different types of RudderStack data and provide steps on opting in to the right setup for your needs.

## Data definitions

RudderStack **does not** permanently store any customer data except in the following instances:

1. Aggregate “Count” data on Event Name, Event Type, Source ID, Destination ID
2. Error codes 
3. Customer user records (for example, usernames, billing-related details)

All other customer data can be classified as either **transient** or **non-transient** and these data may either be stored in your location, for example, AWS, or by RudderStack for up to 30 days. 
...
When the **Sample event data** setting is enabled, RudderStack will store and delete sample events and responses on a rolling 30-day basis. This data may be helpful with debugging efforts. 
