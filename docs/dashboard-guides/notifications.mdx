...
| :--------------| :-----------------------------|
| You're reaching your column limit in your `<table_name>` table. | <ul><li>Go to your warehouse and delete (drop) the unwanted table columns. </li> <li>If you're passing dynamic properties in your events, or if you're syncing semi-structured event data that is not defined by a fixed schema, you can leverage RudderStack's <Link to="/destinations/warehouse-destinations/json-column-support/">JSON Column Support</Link> feature to send your data.</li></ul>. |
