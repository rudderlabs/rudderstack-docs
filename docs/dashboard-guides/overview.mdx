...
**Directory** acts as a catalogue for all the sources (<Link to="/sources/event-streams/cloud-apps/">Cloud Apps</Link>, <Link to="/sources/extract/">Cloud Extract</Link> and <Link to="/sources/reverse-etl/">Reverse ETL</Link>) and destinations (<Link to="/destinations/warehouse-destinations/">Warehouse</Link> and <Link to="/destinations/streaming-destinations/">Streaming destinations</Link>) supported by RudderStack.
...
<img src="../assets/rs-cloud/data-plane-url.png" alt="Data Plane URL" />
...
This tab lets you change your workspace name and company name. You can also find your <Link to="#workspace-id">workspace ID</Link> and <Link to="#workspace-token">workspace token</Link> here.

<img src="../assets/rs-cloud/rudderstack-workspace-id.png" alt="Workspace ID" />

### Workspace ID

RudderStack uses the workspace ID for the internal tracking of the data.

This ID is required when setting up a IAM role for RudderStack required for authenticating all the AWS-based destinations. For more information, refer to the <Link to="/destinations/aws-iam-role-for-rudderstack/">Creating a AWS IAM Role for RudderStack</Link> guide.

### Workspace Token

The workspace token is a unique identifier associated with your current RudderStack workspace. It is used by the RudderStack backend to get the source and destination configurations from the <a href="https://app.rudderstack.com/">dashboard</a>.
...
To view the workspace token, click the <strong>Show</strong> button and enter the password associated with your RudderStack account.

<img src="../assets/rs-cloud/workspace-token-reveal.png" alt="Workspace ID" />
