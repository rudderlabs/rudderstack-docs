...
The easiest way to manage your configurations is via the RudderStack hosted control plane, that is, <a href="https://app.rudderstack.com/signup">RudderStack Cloud</a>. Our hosted control plane requires no setup, and supports features like <Link to="/dashboard-guides/live-events/">Live Events</Link> and <Link to="/features/transformations/">Transformations</Link> that are not supported in the open source version.
</div>
