...
The easiest way to manage your configurations is through the RudderStack hosted control plane, that is, <a href="https://app.rudderstack.com/signup">RudderStack Cloud</a>. It does not require any setup, and has some additional features like <Link to="/dashboard-guides/live-events/">Live Events</Link> and <Link to="/features/transformations/">Transformations</Link>.
</div>
...
## Using RudderStack SDKs

To use RudderStack SDKs as sources in the self-hosted control plane, you need to set up the source, export its configuration, host it at `<CONTROL_PLANE_URL>/sourceConfig` path, and finally point the SDKs to them. 
...
3. Host the exported file on your server such that it can be accessed via the path `<CONTROL_PLANE_URL>/sourceConfig`. Provide the base URL of your server (`<CONTROL_PLANE_URL>`) serving this file in your SDK initialization code snippet. 

For more information on SDK initialization, refer to the following sections in the respective SDK guides:
