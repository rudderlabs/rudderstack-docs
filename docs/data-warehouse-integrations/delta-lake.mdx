...
* **Enable delta tables creation in an external location**: Disabled by default, Rudderstack creates delta tables at a default storage location for non-external Hive tables.
  ...
  When enabled, it lets you specify the external location to create delta tables: 
    ...
    <img  src="../assets/dw-integrations/delta-lake-connection-settings-13.png" alt="Delta Lake destination settings in RudderStack" align="left"/>

    The delta tables can therefore be accessed at `{externalLocation}/{schema}/{table}`.
...
The new tables will be created with partition support. Refer to the Databricks document for more information on [Partitions](https://docs.databricks.com/sql/language-manual/sql-ref-partition.html).

</div>

### How to convert an existing unmanaged table at a location to an unmanaged table at a new location? / How to convert an existing managed table to an unmanaged table at a new location?

1. Set an exclusion window (using the **Exclude window** connection setting) so that RudderStack does not process anything while performing the below changes.
2. Run the following queries in the Databricks Cluster/SQL Endpoints to:
   - Create a temporary table using new location.
...
Refer to the Databricks document for mor information on [Managed and unmanaged tables](https://docs.databricks.com/data/tables.html#managed-and-unmanaged-tables).

</div>

### How to convert an existing unmanaged table at a specific location to a managed table?
