...
* **Enable delta tables creation in an external location**:  Disabled by default, Rudderstack creates delta tables at default storage location for non-external Hive tables.

  However, when enabled, it lets you specify the external location to create delta tables, which is `{externalLocation}/{schema}/{table}` and needs to be entered here:

  <img src="../assets/dw-integrations/delta-lake-connection-settings-13.png" alt="Delta Lake destination settings in RudderStack" />

  <div class="infoBlock">

  Please note the stability of this feature currently:<img src="https://img.shields.io/badge/Stability-Beta-blueviolet" alt="Github Badge" class="githubBadge" />
  </div>
...
### How to modify existing tables to partitioned tables?

1. Set exclusion window so that RudderStack don't process anything during the below changes.
2. Make necessary changes in Rudderstack WareHouse configurations.
3. Rename the already existing tables with _temp suffix.
4. Add column event_data to the _temp table.
5. Back fill the data back into the original table.
...
Meanwhile, Rudderstack will create the new tables with the partition support. Later, they can backfill the data.

### How to convert existing unmanaged table at location 'A' to an unmanaged table at location 'B'?

1. Set exclusion window so that RudderStack don't process anything during the below changes.
2. Create a temporary table using location 'B'.
3. Drop temporary table.
4. Drop original table.
5. Update external location and remove exclusion window as well.
6. Make necessary changes in Rudderstack WareHouse configurations.
7. RudderStack will create the table again during the next syncs.
...
### How to convert existing managed table to an unmanaged table at location 'A'?

1. Set exclusion window so that RudderStack don't process anything during the below changes.
2. Create a temporary table using location 'A'.
3. Drop temporary table.
4. Drop original table.
5. Enable external location support in Rudderstack WareHouse configurations and update the location. Remove exclusion window as well.
6. RudderStack will create the table again during the next syncs.
...
### How to convert existing unmanaged table at location 'A' to managed table?

1. Set exclusion window so that RudderStack don't process anything during the below changes.
2. Create a temporary table.
3. Drop original table.
4. Rename temporary table to original table.
5. Disable external location support in Rudderstack WareHouse configurations. Remove exclusion window as well.
