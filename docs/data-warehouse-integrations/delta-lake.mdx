...
* **Enable delta tables creation in an external location**: If enabled Rudderstack will create the delta tables at `{externalLocation}/ {schema}/{table}` location.
- If not enabled Rudderstack will create delta tables at default storage location for non-external Hive tables.
