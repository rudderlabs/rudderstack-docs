...
You can also configure the number of warehouse slaves (responsible for load file generation) when the warehouse is run as a separate service in Kubernetes. To do so, configure the following in docker-compose.yaml:

```yaml
deploy:
  replicas: <count>
```
The `<count>` can be increased for speeding up the load file generation step of the upload. Its default value is 2.
...
### How does Rudderstack configure the column names? OR
