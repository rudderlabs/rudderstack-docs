...
The Namespace field is populated based on the presence of following:

- If the Namespace and Database fields are present:
  - Database field is picked up for Clickhouse destination.
  - Namespace field is picked up for all the other destinations.

- If `RSERVER_WAREHOUSE_(DEST_TYPE)_CUSTOM_DATA_SET_PREFIX` parameter is present, the namespace is picked up as (DEST_TYPE)_(CUSTOM_DATA_SET_PREFIX)_(SOURCE_NAME). For example:
...
- If none of the above are specified:
  ...
  - The source name is picked if no namespace is configured.


### How does Rudderstack configure table names and column names?

**Table names**

The table names are picked up from the event names and are modified to follow the snake case convention (for example, source name -> source_name). 

For the following sample snippet, the table name is set as **product_purchased**.
...
The column names are picked up from the event properties and are modified to follow the snake case convention. The following FAQ explains how the column names are configured based on normal, array and complex event properties.

### How does RudderStack normalize arrays and complex event properties in the warehouse schema?

RudderStack flattens out the event properties and converts them to snake case (for example, source name -> source_name). Further, each of these event properties act as a column for the warehouse.
