...
Refer to the [warehouse-specific documentation](https://www.rudderstack.com/docs/data-warehouse-integrations/) for more information on configuring the namespace.
</div>
...
| Scenario | Sync  | Namespace    | Warehouse.&lt;destType&gt;.customDataset Prefix | Source name | Schema name| Notes|
| :-- | :-------- | :------------ | :------- | :------- | :------- |:------- |
| 1 | First sync   | AB | XY | S | AB | The namespace is given priority over all the other values. |    
| 2 | First sync   | Not set | XY | S | XY_S | RudderStack combines the `customDataset` prefix and the source name to set the schema name, if the namespace is absent.|
| 3 | First sync   | Not set | Not set | S | S | RudderStack sets the source name as the schema name if the namespace and `customDataset` prefix are absent.|
| 4| First sync   | AB | Not set | S | AB |The namespace is given priority over the other values.|
| 5 | Second sync onwards   | ABC | XYZ | SS | ABC | The namespace, source name and the `customDataset` prefix have all been modified. The new namespace is given priority and set as the schema name. <br /><br />All the data from the second sync will now be stored in the new schema (ABC) and the original schema (AB) will be left as is. |
| 6 | Second sync onwards   | Not set | XYZ | S | XYZ_S | The `customDataset` prefix name has been modified. RudderStack combines the `customDataset` prefix and the source name to set the schema name. |
| 7 | Second sync onwards   | Not set | XYZ | SS | XYZ_SS | The source name and `customDataset` prefix have been modified. RudderStack combines them together to set the schema name. |
| 8 | Second sync onwards   | ABC | Not set | SS | ABC | The namespace and source names have been modified. The namespace is given priority and set as the schema name.|
| 9 | Second sync onwards   | Not set | Not set | SS | S | The source name has been modified. However, it does not impact the schema name and it remains the same as in the first sync. |
...
<ul>
  ...
  <li>If the namespace is absent, RudderStack sets the source name as the schema name.</li>
  <li>If the <code class="inline-code">RSERVER_WAREHOUSE_(DEST_TYPE)_CUSTOM_DATA_SET_PREFIX</code> parameter is set in the <code class="inline-code">config.yaml</code> file of your RudderStack deployment, RudderStack sets the schema name in the <code class="inline-code">customDataset_sourcename</code> format.</li>
</ul>
</div>

<div class="infoBlock">

If the schema name changes from AB to ABC
</div>
