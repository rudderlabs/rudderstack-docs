...
Yes, you can. RudderStack sets the schema name as:
- The Namespace field value in the dashboard. You can set the desired schema name here. <br />
**OR**
- By default, it sets the source name as the schema name with some modifications to it, if the Namespace is not provided.
...
### How is the schema name configured for my data warehouse?

The schema name is configured based on the values mentioned in the below table. The columns in the table corresponds to the following:

- **Sync**: Indicates whether its the first data sync or subsequent syncs after the source is set up.
- **Namespace**: Namespace field value in the RudderStack dashboard.
- **Warehouse.&lt;destType&gt;.customDataset Prefix**: `RSERVER_WAREHOUSE_(DEST_TYPE)_CUSTOM_DATA_SET_PREFIX` parameter in the config.yaml file.
- **Source name**: Name of the source connected to warehouse destination.

| Sync  | Namespace    | Warehouse.&lt;destType&gt;.customDataset Prefix | Source name | Schema name| Notes|
| :-------- | :------------ | :------- | :------- | :------- |:------- |
| First sync   | AB | XY | S | AB | The namespace is given priority over all the other values. |    
| First sync   |  | XY | S | XY_S | The customDataset prefix and source name are combined together if the namespace is absent.|
| First sync   |  |  | S | S | The source name is set as the schema name if the namespace and customDataset prefix both are absent.|
| First sync   | AB |  | S | AB |The namespace is given priority over the other values.| 
| Second sync onwards   | ABC | XYZ | SS | ABC | The namespace, source name and customDataset prefix have been modified. The namespace is given priority and set as the new schema name. Note that all the data from now onwards will be stored in the new schema (ABC) and the original schema will be left as it is. |
| Second sync onwards   |  | XYZ | S | XYZ_S | The customDataset prefix name has been modified. The customDataset prefix and source name are combined together and set as the schema name.|
| Second sync onwards   |  | XYZ | SS | XYZ_SS | The source name and customDataset prefix have been modified and are combined together to be set as the schema name.|
| Second sync onwards   | ABC |  | SS | ABC | The namespace and source names have been modified. The namespace is given priority and set as the schema name.|
| Second sync onwards   |  |  | SS | Same as the first sync. | The source name has been modified. However, it does not impact the schema name from the second sync onwards. It will be retained the same as it was in the first sync.|
