...
### How is the schema name configured in RudderStack?
...
However, the following scenarios with examples provide more details on how the schema name is configured bsed on the presence/absence of Namespace field:

**Non-empty Namespace field**
- If the Namespace field is set to "ABC" and the Source name is "X", the schema name would be "ABC". However, if you change the Namespace field to "DEF" later on, the schema name would become "DEF".
