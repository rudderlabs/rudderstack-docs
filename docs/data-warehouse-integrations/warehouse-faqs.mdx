...
By default, the schema name is set as:
- The Namespace field value in the RudderStack dashboard. **OR**
- The Source name value in the RudderStack dashboard, if the above is not provided.

However, the following scenarios with examples provide more details on how the schema name is configured based on the presence/absence of Namespace field:

**When Namespace field is present**
- If the Namespace field is set as "ABC" and the Source name is "X", the schema name would be "ABC". However, if you change the Namespace field to "DEF" later on, the schema name would become "DEF".

**When Namespace field is absent**
- If the Source name is "X", the schema name would be "X". However, if you change the Source name to "Y" later on, the schema would still be "X" as it is picked from the existing syncs.
- If the Source name is "X", the schema name would be "X". However, if you add the Namespace field as "Y" later on, the schema name would become "Y".




| Sync  | Destination config Namespace    | Warehouse.</destType/>.customDataset Prefix | Source Name | Final Winner|
| :-------- | :------------ | :------- | :------- | :------- |
| First upload sync   | ABC | XYZ | S | ABC |       
| First upload sync   |  | XYZ | S | XYZ_S | 
| First upload sync   |  |  | S | S | 
| First upload sync   | ABC |  | S | ABC | 
| Second upload onwards   | ABC | XYZ | S | ABC | 
| Second upload onwards   |  | XYZ | S | XYZ_S | 
| Second upload onwards   |  | XYZ | S | XYZ_S | 
| Second upload onwards   | ABC |  | S | ABC |
| Second upload onwards   |  |  | S | First upload namespace | 
