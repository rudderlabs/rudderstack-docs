...
You can also configure the number of warehouse slaves (responsible for load file generation) when the warehouse is run as separate service in Kubernetes. In the docker-compose.yaml, configure the following:
