...
- If `RSERVER_WAREHOUSE_CUSTOM_DATA_SET_PREFIX` parameter is present, the Namespace is populated as `(CUSTOM_DATA_SET_PREFIX)_(SOURCE_NAME)`. For example:
...
Namespace:
SNOWFLAKE_<some_value>_<source_name>
```
