...
Yes, Shopify is compatible as an event stream data source. For more information, refer to <Link to="/sources/cloud-apps/shopify/">Shopify</Link>. We also have users that integrate the JavaScript SDK into their Shopify sites. In some cases, they even do it through Google Tag Manager.
...
For more information on the SDK-specific retry and backoff logic, refer to the <Link to="/sources/sdks/sdk-faqs#what-is-the-rudderstack-retry-and-backoff-logic-after-the-connection-fails">SDK FAQ</Link> guide.
</div>
