...
The workspace token is not available for the [RudderStack Pro and Enterprise](https://www.rudderstack.com/pricing/) versions as the RudderStack team handles the deployment. 
</div>
...
On the other hand, the <Link to="/rudderstack-cloud/dashboard-overview/#workspace-token">workspace token</Link> is a unique identifier for your configuration settings which RudderStack uses to fetch and track your instrumentations. It be obtained by navigating to **Settings** > **Company** in your RudderStack dashboard, as shown:
...
3. Check if the database is accessible by allowlisting all the RudderStack IPs listed <Link to="/data-warehouse-integrations/warehouse-faqs/#which-rudderstack-ips-should-i-allowlist">here</Link>.
