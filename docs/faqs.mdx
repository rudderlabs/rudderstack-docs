...
The workspace token is not available for the [RudderStack Pro and Enterprise](https://www.rudderstack.com/pricing/) versions. 
</div>
...
By default, the workspace token is hidden, for security purposes. You must have <Link to="/rudderstack-cloud/teammates/#admin">administrative privileges</Link> to access the token.
</div>
