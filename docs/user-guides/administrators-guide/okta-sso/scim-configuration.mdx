...
Currently, RudderStack supports the followin provisioning features:
...
<div class="infoBlock">
Currently, you can update only the user's display name. Updating the email is not supported.
</div>
...
<div class="infoBlock">
When a user is deactivated through SCIM, RudderStack does not delete the user from its database; it only revokes their organization user role leading to them losing access to their workspace.
</div>

## Requirements

To configure the SCIM app, you need a <Link to="/dashboard-guides/personal-access-token/">personal access token</Link> with **administrative privileges**.
...
3. Enter the token and from the **Role** dropdown, select **Admin**.
