...
description: Get started with the RudderStack various RudderStack Grafana dashboard panels for better observability and performance monitoring.
---
...
This section refers to the number of events RudderStack has received in the selected time period. The RudderStack API allows multiple events to be [batched inside a single HTTP request](https://www.rudderstack.com/docs/api/http-api/#11-batch).
...
Corresponds to the metrics on any failures and retries before the event is delivered. If the event keeps failing even after a configured window, then it is marked as aborted. The aborted events are also shown in this panel.
