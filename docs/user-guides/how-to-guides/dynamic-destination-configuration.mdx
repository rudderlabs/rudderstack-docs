...
To use this feature, enter the path of your desired configuration value from the event payload using object notation in a [handlbars.js](https://handlebarsjs.com/guide/expressions.html) expression, as well as a default value for the setting in case the value is not present:
...
Any value present in the path will be passed as a configuration setting to your destination. If no value is present in the payload path, the default value will be used to configure the destination.



If `event.path.to.key` exists, its value will be sent as a destination config setting. If not present, we will send the default value that you supply in the expression.
...
In the next section we'll walk through an example of dynamic configuration using multiple Google Analytics tracking IDs.

## Use Case: Dynamically configure your destination by Google Analytics Tracking ID

Suppose you have two web properties with `rudderanalytics.js` implemented, each with a unique Universal Analtyics (UA) tracking ID. You don't want to set up different RudderStack destinations for each property; rather, you would like to pass the event data to the appropriate tracking ID in your Google Analytics instance.
...
To populate the configuration values from the event payload, add a <Link to="/features/transformations/#adding-a-transformation">transformation function</Link> to your Google Analytics destination.
...
If there is no match for the `sourceId`, the default value that you define in the next step will be passed to GA.
...
Once you have added a transformation, the next step is to <Link to="/dashboard-guides/destinations/#adding-a-destination">add a destination</Link> for Google Analytics in the RudderStack dashboard.
...
With the transformation in place, and your tracking ID connection settings configured, event data will be routed to Google Analytics based on the tracking ID in the payload.

## FAQ

### How is this feature useful to me?

This feature is useful if you want to configure multiple instances of the same destination with different configurations for specific fields. Refer to the use-case section above to see how you can use this feature.
