...
If `event.path.to.key` exists, its value will be sent as a destination config setting. If not present, RudderStack will send the default value that you supply in the expression.
...
This feature is useful if you want to configure multiple instances of the same destination with different configurations for specific fields. Refer to the <Link to="#use-case-dynamically-configure-your-destination-by-google-analytics-tracking-id">use case</Link> section above to see how you can use this feature.
