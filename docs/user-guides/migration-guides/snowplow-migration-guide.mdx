...
RudderStack supports setting values for the following Snowplow configuration objects in mobile SDKs (iOS/Android). If not set, the default values are assigned.
...
<div class="infoBlock">
In Android SDK, RudderStack ignores the first argument as it is a dummy argument.
</div>
      </TabPanel>
      <TabPanel>
<span>
...
<div class="infoBlock">
For mobile SDKs, Snowplow's default timeout session is 30 minutes whereas RuddeStack's default timeout session is 5 minutes. Refer to the <Link to="/sources/event-streams/sdks/session-tracking/#mobile-sdks">RudderStack Session Tracking</Link> documentation for more information.
</div>   
