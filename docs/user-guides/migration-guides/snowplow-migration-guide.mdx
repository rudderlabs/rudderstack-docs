...
This document covers the necessary steps to migrate from [Snowplow](https://snowplow.io/) to RudderStack and replace your instrumentation code. Once done, you can start using the RudderStack SDKs to track your events with minimal code changes.
...
The <a href="https://docs.snowplow.io/docs/collecting-data/collecting-from-own-applications/mobile-trackers/tracking-events/#session"><code class="inline-code">SessionConfiguration</code></a> class can be used to capture sessions which help to track the user activity in the app as shown:
...
<div class="warningBlock">
Snowplow does not provide any call to identify a user. Although, RudderStack sends an <Link to="/event-spec/standard-events/identify/"><code class="inline-code">identify</code></Link> call if you initialize the <code class="inline-code">SubjectConfiguration</code> class while initializing the SDK. If not initialized, a user is not identified. 
</div>
...
In the below sample call, <code class="inline-code">alex@example.com</code> is the <code class="inline-code">userId</code> and the <code class="inline-code">firstName</code>, <code class="inline-code">lastName</code>, and <code class="inline-code">city</code> are the user traits.
<span>
