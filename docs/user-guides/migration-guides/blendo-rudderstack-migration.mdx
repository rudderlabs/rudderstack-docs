...
In case you don't want to sync all the historical data and start from a checkpoint instead, <a href="mailto:support@rudderstack.com">contact us</a>.
</div>
...
You can use the `useBlendoCasing` integration option to retain the column names from Blendo infrastructure. The following sample snippet shows how to retain the Blendo column names for Delta Lake destination:
...
```
<div class="warningBlock">
Refer to the <Link to="/destinations/warehouse-destinations/warehouse-schema/#skip-creation-of-tracks-table">Warehouse Schema</Link> guide to know more about the supported names for different warehouse destinations in the above snippet.
</div>
...
- We are working on **improving the performance of the data pipelines,** compared to Blendo**.** In case you have come across any performance-related differences between Blendo and RudderStack, feel free to [contact us](mailto:%20support@rudderstack.com).
