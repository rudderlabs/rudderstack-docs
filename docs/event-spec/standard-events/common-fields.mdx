...
The IP address of a user is captured automatically in the `request_ip` property as a **common field**. You can see it in the event received at the destination but not in the <Link to="https://www.rudderstack.com/docs/dashboard-guides/live-events/">Live Events</Link> tab. The reason is that it is added from the server at the backend while being sent to the destination and not as the initial data generated at the source.
