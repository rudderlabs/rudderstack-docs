...
| `ip` | - | Yes | - |
| `locale` | Yes | Yes | Yes |
| `network.bluetooth` | - | Yes, if permission is present | No | 
...
<div class="infoBlock">
From Android SDK V1.6.0, the bluetooth status is included in the <code class="inline-code">network.bluetooth</code> field only if the bluetooth permission is present in the app.
From iOS SDK V1.6.3, the bluetooth status is not included in the <code class="inline-code">network.bluetooth</code> field as it’s a run time permission.
</div>
