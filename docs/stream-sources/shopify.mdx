...
The user-related information is mapped into the <code class="inline-code">traits</code> object in RudderStack payload. The products information present in the above resources in line_items is mapped to the <code class="inline-code">products</code> array in RudderStack payload. Refer to the <Link to="/rudderstack-api/api-specification/rudderstack-spec/">Event Specification</Link> and <Link to="/rudderstack-api/api-specification/rudderstack-spec/common-fields/">Common Fields</Link> to understand more about RudderStack payload nomenclature.
</div>
...
1. Shopify provides the store owners with a capability to make user login compulsory or allow guest checkout in their stores. However, if you prompt  users to login on your platform, RudderStack fetches the user details to provide you with more meaningful events corresponding to the user's journey on your platform.

### Supported client side calls

It is recommended **NOT** to make any drastic changes to your store structure to preserve data integrity. If done, RudderStack may lead to incorrect or missing calls owing to the disparity between URL and triggered event. 

Refer to the following additional details for e-commerce events:

- **Cart Viewed**: The URL is recommended to be of the `[store-url]/cart` format.
- **Checkout Started**: This event is triggered when the Checkout button is clicked on the `[store-url]/cart` page. It can also be triggered from the product description pages by clicking on **Buy it Now**. Another way to trigger it is using the pop-ups when adding a product.
- **Product Added**: This event is triggered when **Add to Cart** button is clicked on a product description page with `[store-url]/products/product_name` URL format.
...
- **Product Viewed**: This event is triggered when a product’s description page with the `[store-url]/products/product_name` URL format is opened. It is an indicative of the product listing page being viewed.
...
The `requestIP` is the IP address of Shopify backend server which pushes data to RudderStack while the `browser_ip` is the IP address of user's browser.
...
Note that the `analytics` object of JS SDK (for example, `rudderanalytics.track()` or `rudderanalytics.identify()`) is already loaded on the `window` object of the store where the app is installed. Hence, the `analytics` object should not be loaded separately to collect additional/custom fields from Shopify.

### How does RudderStack set Anonymous ID and can it persist?

RudderStack fetches Shopify's `customerId` and uses it as `anonymousId` to allow identity stitching in downstream destinations.

No, the `anonymousId` does not persist and keeps on changing as the Shopify servers push data to RudderStack over server to server API's.

### How can I differentiate between data collected from Shopify Cloud (Webhooks) and Shopify Device (Web Device Mode)?

You can differentiate between the data collected in cloud and device mode by looking at the signature in payload:

- In cloud mode, RudderStack subscribes to Shopify webhooks to consume data from Shopify. Data collected from Shopify web has the following signature in the payload:
...
```
- In device Mode, RudderStack loads its JavaScript SDK on every Shopify store page for web tracking. Data collected from Shopify device mode has the following signature in the payload:
