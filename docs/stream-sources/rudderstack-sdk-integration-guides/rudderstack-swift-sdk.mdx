...
The <Link to="/rudderstack-api/api-specification/rudderstack-spec/alias"><code class="inline-code">alias</code></Link> call associates the user with a new identification. A sample `alias` call for the API is below:
...
2. Then, create a `CustomDetination` class file by extending `RudderDestination` and initialize `RSCustomDestination` inside `init()`, as shown:
