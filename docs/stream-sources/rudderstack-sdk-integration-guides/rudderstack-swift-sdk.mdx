...
title: "Swift"
description: >-
  Detailed technical documentation on the RudderStack Swift SDK to send
  events from your iOS/macOS/tvOS/watchOS apps to various destinations.
---

# Swift

The **RudderStack Swift SDK** lets you track your customer event data from your iOS, macOS, tvOS, and watchOS applications and send it to your specified destinations via RudderStack.

Check out the [GitHub codebase](https://github.com/rudderlabs/rudder-sdk-swift) to get a more hands-on understanding of the SDK.

<div class="successBlock">

You can now integrate the Swift SDK with your tvOS and watchOS apps and seamlessly track user events without the need for any additional configuration. The SDK supports tvOS starting from version <strong>1.1.0</strong> and <strong>watchOS</strong> is supported starting from the SDK version <strong>1.3.1</strong>.

macOS will be supported from the SDK version <strong>2.0.0</strong>.
</div>
...
To set up the RudderStack Swift SDK, the following prerequisites must be met:
...
## Installing the RudderStack Swift SDK

The RudderStack Swift SDK is distributed through [Cocoapods](https://cocoapods.org/pods/Rudder) and [Carthage](https://github.com/Carthage/Carthage).
...
The Swift SDK supports all the API calls specified in the <Link to="/rudderstack-api/api-specification/rudderstack-spec/">RudderStack Events Spec</Link> guide. These include `identify`, `track`, `screen`,  `group`, `alias`, and `reset` calls.
...
You only need to call the <code class="inline-code">setoptOutStatus</code> API with the relevant parameter only once, as the information persists within the device even if you reboot it.
</div>
...
| [macOS](https://developer.apple.com/macos/) | 2.0.0 |

### How do I migrate from an older SDK version to the current version?

To migrate from older SDK versions, update the usage of the following classes:
