...
You can now integrate the iOS SDK with your tvOS and watchOS apps and seamlessly track user events without the need for any additional configuration. The iOS SDK supports tvOS starting from version <strong>1.1.0</strong> and <strong>watchOS</strong> is supported starting from the SDK version <strong>1.3.1</strong>.
</div>

// Add stability beta badge here
...
- Set up an iOS source in the dashboard. For more information, follow the [Adding a Source and Destination in RudderStack](https://rudderstack.com/docs/connections/adding-source-and-destination-rudderstack/) guide.  You should then see a **Write Key** for this source, as shown:

<img src="../../../assets/event-stream-sources/ios-sdk-1.png" alt="iOS source write key" />

- You also need a data plane URL. Follow [**this section**](https://rudderstack.com/docs/rudderstack-open-source/installing-and-setting-up-rudderstack/#what-is-a-data-plane-url-where-do-i-get-it) for more information on the data plane URL and where to get it.
...
The recommended and easiest way to add the iOS SDK to your project is through <code class="inline-code">Podfile</code>.
</div>

To install the iOS SDK depending on your preference, follow the steps below:
...
<Tabs>
  ...
  <TabPanels>
    <TabPanel>
      <ul>
      <li>Add the iOS SDK to your <code class="inline-code">Podfile</code>, as shown:
<span>
...
RudderStack uses <a href="https://sqlite.org/index.html">SQLite</a> to temporarily store the events before sending them to the backend (data plane). Making calls which are not thread-safe, like <code class="inline-code">SQLite.shutdown()</code>, might lead to unexpected crashes.
</div>
...
RudderStack automatically tracks the following <a href="https://www.rudderstack.com/docs/rudderstack-api/api-specification/application-lifecycle-events-spec/">application lifecycle events</a>:
<ul>
...
You can disable these events using the <code class="inline-code">trackLifecycleEvents</code> method of <code class="inline-code">RSConfig</code> and passing <code class="inline-code">false</code>. However, it is highly recommended to keep them enabled.
</div>

## Configuring the RudderStack client

You can configure your client based on the following parameters using `RudderConfig`:

| Parameter               | Type      | Description    | Default value      |
| :---------------------- | :-------- | :-------------- | :------- |
| `logLevel`     | `RSLogLevel`     | Controls how much of the log you want to see from the iOS SDK.                | `RSLogLevel.None`                                                           |
...
| `controlPlaneUrl`       | String  | This parameter should be changed **only if** you are self-hosting the control plane. Check the [Self-hosted control plane](https://rudderstack.com/docs/stream-sources/rudderstack-sdk-integration-guides/rudderstack-ios-sdk/#self-hosted-control-plane) section below for more information. The SDK will add `/sourceConfig` along with this URL to fetch the required configuration. | `https://api.rudderlabs.com`   |

### Self-hosted control plane

If you are using a device mode destination like Adjust, Firebase, etc., the iOS SDK needs to fetch the required configuration from the control plane. If you are using the [Control Plane Lite](https://www.rudderstack.com/docs/rudderstack-open-source/control-plane-lite/) utility to host your own control plane, then follow [this section](https://www.rudderstack.com/docs/rudderstack-open-source/control-plane-lite/#control-plane-url) to specify `controlPlaneUrl` in `RudderConfig` that points to your hosted source configuration file.

<div class="warningBlock">

You should not pass the <code class="inline-code">controlPlaneUrl</code> parameter during the SDK initialization if you are using <a href="https://app.rudderstack.com">RudderStack Cloud</a>. This parameter is supported only if you are self-hosting the control plane using the <a href="https://www.rudderstack.com/docs/rudderstack-open-source/control-plane-lite/">Control Plane Lite</a> utility.
</div>

## Supported API calls

The RudderStack iOS SDK supports all the API calls specified in the [RudderStack Events Specification](https://www.rudderstack.com/docs/rudderstack-api/api-specification/rudderstack-spec/). These include `identify`, `track`, `screen`,  `group`, `alias`, and `reset` calls.

## Identify

The [`identify`](https://www.rudderstack.com/docs/rudderstack-api/api-specification/rudderstack-spec/identify/) call lets you identify a visiting user and associate them to their actions. It also lets you record the traits about them like their name, email address, etc. Once you identify the user, the SDK persists all the user information and passes it on to the subsequent `track` or `screen` calls. To reset the user identification, you can use the `reset` method.
...
```objectivec
[client identify:@"user_id" traits:@{@"email": @"alex@example.com"}];
```
</span>
      </TabPanel>
      <TabPanel>
...
[client identify:@"user_id" traits:@{@"email": @"alex@example.com"} option:eventOption];
```
</span>
      </TabPanel>
      <TabPanel>
...
client.identify("user_id", traits: ["email": "alex@example.com"], option: messageOption)
```
</span>
      </TabPanel>
    </TabPanels>
...
The [`track`](https://www.rudderstack.com/docs/rudderstack-api/api-specification/rudderstack-spec/track/) call lets you record the user events along with any properties associated with them.
...
```objectivec
[client track:@"sample_track_event" properties:@{
    @"key_1": @"value_1",
    @"key_2": @"value_2"
}];
```
...
| `options`    | `RudderOption` | Optional       | Extra event options.                                         |

## Screen

The [`screen`](https://www.rudderstack.com/docs/rudderstack-api/api-specification/rudderstack-spec/screen/) call lets you record whenever a user views their mobile screen, with any additional relevant information about the screen.
...
```objectivec
[client screen:@"ViewController" category:NULL properties:@{
  @"key_1": @"value_1",
  @"key_2": @"value_2"
} 
options:NULL];
...
```swift
client.screen("ViewController", properties: [
    "key_1": "value_1",
    "key_2": "value_2"
])
```
...
| `options`    | `RudderOption` | Optional       | Extra options to be passed along with the `screen` call.     |

## Group

The [`group`](https://www.rudderstack.com/docs/rudderstack-api/api-specification/rudderstack-spec/group/) call lets you link an identified user with a group like a company, organization, or an account. It also lets you record any traits associated with that group, like the name of the company, number of employees, etc.
...
| `options` | `RudderOption` | Optional       | Extra event-level options to be passed along with the `group` call.              |
...
The [`alias`](https://www.rudderstack.com/docs/rudderstack-api/api-specification/rudderstack-spec/alias/) call associates the user with a new identification. A sample `alias` call for the API is below:
...
```objectivec
[client alias:@"new_user_id" options:NULL];
```
</span>
      </TabPanel>
      <TabPanel>
...
| `options` | `RudderOption` | Optional       | Event-level options.                              |
...
You can use the `reset` method to clear the persisted user traits for the `identify` call. This is required for the user logout operation.
...
You only need to call the `setoptOutStatus` API with the relevant parameter only once, as the information persists within the device even if you reboot it.
</div>
...
Once the user grants their consent, you can enable user tracking once again by using the `setOptOutStatus` API with `NO` or `false` as a parameter sent to it, as shown:
...
The <code class="inline-code">setOptOutStatus</code> API is available in the RudderStack iOS SDK starting from version <code class="inline-code">SDK_VERSION</code>.
</div>
...
RudderStack has separated the IDFA collection from the core library so that you have better control over it. 
...
Make sure the <code class="inline-code">destination display name</code> you pass while specifying the custom destinations should exactly match the destination name as shown in the <a href="https://app.rudderstack.com/directory">RudderStack dashboard</a>.
</div>
...
[client track:@"sample_track" properties:@{@"key_1": @"value_1", @"key_2": @"value_2"} option:eventOption];
```
</span>
      </TabPanel>
      <TabPanel>
...
client.track("sample_track", option: eventOption)
```
</span>
      </TabPanel>
    </TabPanels>
...
- Create a `RSCustomDestination.swift` file by extending `RSDestinationPlugin`, as shown:
...
- Then, create a `CustomDetination` class file by extending `RudderDestination` and initialize `RSCustomDestination` inside `init()`, as shown:
...
- Finally, add the `CustomDestination` with the RudderStack iOS SDK after its initialization, as shown:
...
### Does the iOS SDK support the tvOS and watchOS platforms?

Yes, the iOS SDK supports tvOS and watchOS platforms. Refer to the table below for the version details:

| Platform | Minimum SDK version supported |
| :--------| :----------------|
| [tvOS](https://developer.apple.com/tvos/) | 1.1.0 |
| [watchOS](https://developer.apple.com/watchos/) | 1.3.0 |

### How do I migrate from v1.0.2?

To migrate from an older version 1.0.2, update the usage of the following classes:
...
In case of client-side errors, e.g. if the source write key passed to the SDK is incorrect, RudderStack gives you a **400 Bad Request** response and aborts the operation immediately. For other types of network errors, for e.g. invalid data plane URL, the SDK tries to flush the events to RudderStack in an incremental manner (every 1 second, 2 seconds, 3 seconds, and so on).
