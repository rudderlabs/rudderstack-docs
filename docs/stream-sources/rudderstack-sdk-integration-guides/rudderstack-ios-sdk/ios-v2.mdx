...
## Push notification support for device mode destinations

With the iOS SDK v2, you need not call the individual destination's API in your app to implement the push notifications support; calling the SDK's push notification APIs is sufficient.

<div class="warningBlock">

This feature is specific to RudderStack iOS SDK v2 and is <strong>not available</strong> in the earlier SDK versions. Also, it is only applicable for the <Link to="/rudderstack-cloud/rudderstack-connection-modes/#device-mode">device mode</Link> integrations.
</div>
...
When sending events to a destination via the <Link to="/rudderstack-cloud/rudderstack-connection-modes/#device-mode">device mode</Link>, you can explicitly specify which events should be discarded or allowed to flow through - by allowlisting or denylisting them.
