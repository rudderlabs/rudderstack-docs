...
<div class="successBlock">
  ...
  Starting from version <strong>1.1.0</strong>, the iOS SDK supports the <strong>tvOS</strong> platform. RudderStack also supports the <strong>watchOS</strong> platform starting from version <strong>1.3.1</strong>. You can now integrate the iOS SDK with your tvOS and watchOS apps and seamlessly track user events without the need for any additional configuration.
</div>

 <a href="https://cocoapods.org/pods/Rudder">
    <img src="https://img.shields.io/cocoapods/v/Rudder.svg?style=flat?style=flat" alt="Github Badge" class="githubBadge" />
 </a>
...
For example, if the `flushQueueSize` is 30 and there are 180 events in the database advanced the `flush()` API is called, the iOS SDK will retrieve all those events and divide them into batches of 30 messages each, that is, total 6 batches. 
