...
Yes, you can use RudderStack's [Client-side Event Filtering](https://www.rudderstack.com/docs/stream-sources/rudderstack-sdk-integration-guides/event-filtering/) feature to specify which events should be discarded or allowed to flow through - by whitelisting or blacklisting them in the RudderStack dashboard while setting up your destination. This method is useful if you are sending the events via the [device mode](https://www.rudderstack.com/docs/rudderstack-cloud/rudderstack-connection-modes/#device-mode).
