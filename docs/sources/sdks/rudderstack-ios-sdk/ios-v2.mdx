...
| `controlPlaneUrl`       | String  | This parameter should be changed **only if** you are self-hosting the control plane. Refer to the <Link to="/sources/sdks/rudderstack-swift-sdk/#self-hosted-control-plane">Self-hosted control plane</Link> section below for more information. The SDK will add `/sourceConfig` along with this URL to fetch the required configuration. | `https://api.rudderlabs.com`   |
