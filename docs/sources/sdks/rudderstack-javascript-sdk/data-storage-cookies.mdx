...
title: "Data Storage"
description: >-
  Detailed technical documentation on persistent user data stored in cookies or local storage by the JavaScript SDK.
---

The JavaScript SDK stores persistent user data in the cookies by default. However, if the cookies are not supported, local storage is used instead. 
...
The local storage is used by the JavaScript SDK to keep track of the events sent to the RudderStack backend. They are listed in the below table:
