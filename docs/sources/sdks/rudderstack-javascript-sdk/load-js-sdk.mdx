...
|  `configUrl`          | String                              |  Server (<Link to="/get-started/rudderstack-open-source/control-plane-lite/">Control Plane</Link>) endpoint serving your destination configurations. Note that the source config is automatically appended to this endpoint. Default value: `https://api.rudderlabs.com`. |
|  `queueOpts`          | <Link to="#queueopts">QueueOpts</Link>         | Internal queue to hold the data and send it as XMLHttpRequest.          |
...
It is an internal queue to hold the data and send it as XMLHttpRequest . Its structure is defined as follows:
