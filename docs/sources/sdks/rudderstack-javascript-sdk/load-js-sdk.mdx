...
You can load the JavaScript SDK using the `load` API method. It loads the `analytics.js` file which tracks and sends events from your website to RudderStack and can be defined as:

```javascript
rudderanalytics.load(WRITE_KEY, DATA_PLANE_URL, [load_options]);
```
...
The **load_options** in the `load` call above can be used to define various options while loading the SDK. It includes the following parameters:
...
|  `configUrl`          | String                              |  Server (<Link to="/get-started/rudderstack-open-source/control-plane-lite/">Control Plane</Link>) endpoint serving your destination configurations. Note that `sourceConfig` is automatically appended to this endpoint. Default value: `https://api.rudderlabs.com`. |
|  `queueOpts`          | <Link to="#queueopts">QueueOpts</Link>         | definition          |
...
| `useBeacon`           | Boolean                             | Determines whether the SDK <Link to="/sources/sdks/rudderstack-javascript-sdk/send-events-beacon/">sends the event payloads via the  `navigator.sendBeacon()` utility</Link> (false by default).  |
...
(definition) . Its structure is defined as follows:
...
The `anonymousIdOptions` object in the `load` call options can automatically capture the anonymous ID from a source and set it as Rudderstack's `anonymousId`.
