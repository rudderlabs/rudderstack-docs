...
<div class="infoBlock">
You need to replace the <Link to="/get-started/glossary/#write-key"><code class="inline-code">WRITE_KEY</code></Link> and <Link to="/dashboard-guides/overview/#data-plane-url"><code class="inline-code">DATA_PLANE_URL</code></Link>.
</div>

### Load options 

You can use the `options` parameter in the `load` call to specify different options while loading the JavaScript SDK. It includes the following parameters:
...
The parameter type and description is as follows:
...
| `useBeacon`           | Boolean                             | Defaults to `false`. If set to `true`, the SDK sends the event payloads via the  `navigator.sendBeacon()` utility.  Refer to [Beacon transport mechanism](https://rudderstack.com/docs/stream-sources/rudderstack-sdk-integration-guides/rudderstack-javascript-sdk/javascript-sdk-enhancements/#sendbeacon-as-a-transport-mechanism) for more details. |
...
<div class="infoBlock">
Refer to the <a href="https://www.rudderstack.com/docs/rudderstack-open-source/control-plane-lite/#control-plane-url">Control plane URL section</a> to get the <code class="inline-code">CONTROL_PLANE_URL</code>.
</div>
