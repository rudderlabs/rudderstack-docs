...
The structure of **apiOptions** parameter is as shown:
...
| **Parameter**                   | **Type**                                | **Description**  |
| :-------------------------- | :---------------------------------- | :----------------------- |
| `integrations`         | <Link to="/sources/sdks/rudderstack-javascript-sdk/load-js-sdk/#integrationopts">IntegrationOpts</Link> | Use to send event data only to the <Link to="/sources/sdks/rudderstack-javascript-sdk/filtering/#filtering-destinations">selective destinations</Link>.   |
...
<div class="warningBlock">
Refer to the <Link to="/event-spec/ecommerce-events-spec/">E-Commerce events specification</Link> guide to know about the e-commerce events captured by RudderStack.
</div>
...
The `context` and `traits` dictionaries are provided in the <Link to="/sources/sdks/rudderstack-javascript-sdk/supported-api/#apiOptions">`apiOptions`</Link> parameter of the supported API methods.
...
The `traits` is an optional dictionary included within <Link to="#context">`context`</Link> which specifies the user's unique traits. This is a very useful field for linking the user's information from a previously made <Link to="http://localhost:8001/sources/sdks/rudderstack-javascript-sdk/supported-api/#identify">`identify`</Link> call to the subsequent <Link to="http://localhost:8001/sources/sdks/rudderstack-javascript-sdk/supported-api/#track">`track`</Link> or <Link to="http://localhost:8001/sources/sdks/rudderstack-javascript-sdk/supported-api/#page">`page`</Link> calls.
...
The traits in the above event are `name`, `email`, `subscriptionStatus`, and `plan`. If you wish to add new or override any traits in the subsequent `track` or `page` event triggered by the user, you can do so by passing it in `traits` as shown:
...
The above snippet will add a new trait `addOn` and modify the `plan` traits value to `gold` in the `track` event.