...
The **JavaScript SDK (v1.1)** is the latest lightweight, efficient, and optimized version of the SDK. Its size is reduced by approximately 70% which considerably increases its loading speed.

Another major improvement in the latest version (v1.1) is that the <Link to="/destinations/rudderstack-connection-modes/#device-mode">device mode</Link> destinations are published as individual plugins and loaded dynamically as per the specified dashboard configurations. In v1, the the <Link to="/destinations/rudderstack-connection-modes/#device-mode">device mode</Link> destinations were bundled with the core SDK.
...
The device mode destinations are not bundled in the core JavaScript SDK v1.1 and needs to be loaded using any of the below-mentioned ways: 

### From RudderStack CDN

You can load the device mode destinations from https://cdn.rudderlabs.com/v1.1/js-integrations/ path by appending the name of the required destination as:
https://cdn.rudderlabs.com/v1.1/js-integrations/{dest-name.min.js}


For example, if you want to load Hubspot, the download path will be: https://cdn.rudderlabs.com/v1.1/js-integrations/HubSpot.min.js, for Google Analytics, the download path will be: https://cdn.rudderlabs.com/v1.1/js-integrations/GA.min.js, etc.
