...
The following sections cover the finer technical details of the improvements in the JavaScript SDK that make it more efficient and easier to use.
...
- The Beacon requests sent from the SDK using the Beacon utility only pushes the events to browser's Beacon queue. Further, it depends on the browser's engine to send these events from the queue. Hence, RudderStack **does not guarantee** if any events get discarded due to the 5xx or other network-related errors (request timed out, end resource unavailable, etc.).
