...
<div class="infoBlock">
You can assign some other role to this user later on in the RudderStack dashboard settings as specified in the <Link to="#configuring-the-connection-credentials">Configuring the connection credentials</Link> section.
...
2. Enter the relevant settings in the **Connection Credentials** section as listed below:
    ...
    - **Role**: Enter a role you want to assign to the above user. You can use this role for syncing data apart from the default role (`RUDDER_ROLE`) assigned in the <Link to="#step-1-creating-a-new-role-and-user-in-snowflake">Creating a new role and user</Link> section.
