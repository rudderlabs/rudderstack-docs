---
title: "Snowflake"
description: Ingest your data from Snowflake into RudderStack.
---
...
<div class="infoBlock">
You can assign some other role to this user in the RudderStack dashboard settings later, as specified in the <Link to="#configuring-the-connection-credentials">Configuring the connection credentials</Link> section.
</div>
...
2. Enter the relevant settings in the **Connection Credentials** section as listed below:
    ...
    - **Role**: Enter a role you want to assign to the above user. For syncing the data, you can use this role apart from the default role (`RUDDER_ROLE`) assigned in the <Link to="#step-1-creating-a-new-role-and-user-in-snowflake">Creating a new role and user</Link> section.
...
3. Click **Continue**. RudderStack will then verify and validate your credentials. 
