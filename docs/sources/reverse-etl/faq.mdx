...
Reverse ETL sources support sending the `identify` and `track` events. Refer to the <Link to="/sources/reverse-etl/features/syncing-events/">Syncing Events</Link> guide for more information on how to send these events.
