...
Reverse ETL sources support the `identify` and `track` events. Refer to the <Link to="/sources/reverse-etl/features/syncing-events/">Syncing Events</Link> guide for more information on sending these event types.
