...
### What event types are supported by Reverse ETL sources?

Reverse ETL sources support `identify` and `track` events. Refer to the <Link to="/sources/reverse-etl/features/visual-data-mapper/">Sending Event Data</Link> guide for more information on sending these event types.
