...
<div class="infoBlock">
You can create a model in the RudderStack dashboard and use it while connecting a Reverse ETL source to a destination. For detailed steps on how to create a model, refer to the <Link to="/sources/reverse-etl/features/models/">Models</Link> guide.
</div>
...
<div class="infoBlock">
Alternatively, you can map the columns using the <Link to="/sources/reverse-etl/features/visual-data-mapper/">Visual Data Mapper</Link> feature. However, note that this feature is currently supported only for selective destinations.
</div>
