...
RudderStack supports S3 as a source from which you can ingest data and route it to the desired downstream destinations.

## Setting up the S3 source in RudderStack

To set up an S3 source in RudderStack, follow these steps:
...
<div class="infoBlock">
As an alternative to JSON mapping, you can map the columns using the <Link to="/sources/reverse-etl/features/visual-data-mapper/">Visual Data Mapper</Link> feature. However, this feature is currently supported only for selective destinations.
</div>
...
After creating the managed policy, follow these steps to create the IAM role for RudderStack.
...
8. Enter a unique name for your role. Note that this name **cannot** be distinguished by case. For example, you cannot create a role named `RUDDERSTACK` if `rudderstack` already exists.
...
2. From the upper right corner, click your account and go to <strong>Security Credentials</strong>.
...
<div class="warningBlock">
For setting up the S3 source, some S3 actions must be attached to your access keys. Refer to the <Link to="#s3-permissions">S3 permissions</Link> section above for more information on these actions.
</div>
