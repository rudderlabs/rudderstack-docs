...
RudderStack provides some useful features which you can use while configuring your Reverse ETL pipelines to add customizations and automate the seamless event flow from your data warehouse to various analytics, sales, and marketing tools.

## Models

RudderStack supports using **Models** in the Reverse ETL pipeline  by defining custom SQL queries. You can create and run these queries on your warehouse and send the resulting data to specific destinations. You can create as many models and reuse them with their corresponding Reverse ETL sources.
...
<div class="infoBlock">
Visual Data Mapper is currently supported for the following destinations:
<ul>
...
To use this feature, you can add a <a href="https://rudderstack.com/docs/sources/reverse-etl/">Reverse ETL source</a> and then connect it to a VDM-supported destination. For more information, refer to the <Link to="/sources/reverse-etl/common-settings/visual-data-mapper/">Visual Data Mapper</Link> guide.
</div>


## Airflow Provider

RudderStack's Airflow Provider lets you programmatically schedule and trigger your Reverse etl syncs from outside RudderStack and integrate them with your existing Airflow workflows. Refer to the <Link to="/sources/reverse-etl/features/airflow-provider/">Airflow Provider</Link> guide for more information on this feature. 