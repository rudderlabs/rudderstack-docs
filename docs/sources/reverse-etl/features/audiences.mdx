...
- Save time from updating the audience each time or debugging issues while building audiences in proprietary tools.
...
- **Schema**: Select the database schema from the dropdown.
- **Table**: Select the required table from the dropdown.
...
Yes, you can. To do so, select the **Custom SQL filter** option from the dropdown and enter a corresponding SQL query which includes the relevant filtering condition for the JSON column, as shown:

<img src="../../../assets/warehouse-actions-sources/custom-sql-filter.png" alt="Custom SQL filter" />