...
To use the Airflow Provider, you must have a working Apache Airflow installation. For more information, refer to the <Link to="/apache-airflow/stable/installation/index.html">Airflow documentation</Link>.
</div>
...
For more information on where to find the source ID, refer to <Link to="/rudderstack-cloud/sources/">Sources</Link> guide.
</div>
