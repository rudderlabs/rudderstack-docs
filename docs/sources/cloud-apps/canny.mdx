...
| `type`    | `event`  | -|
| `object.voter.userId`    | `userId`  | Ony if `type`is `vote.created`/`vote.deleted` |
| `object.author.userId`     | `userId`   |- |
| sha256 hash of(`object.author.email`)        | `anonymousId` | Only if `userId` is not present |
| sha256 hash of(`object.voter.email`)        | `anonymousId` |- |
| `object.voter.id`        | `context.externalId[0].value` | -|
| `cannyId`    | `context.externalId[0].type`  |- |
...
```json
{
    "anonymousId": "d6a9d06e8a46432850ecc6",
    "event": "post.created",
    "integrations": {
        "Canny": false
    },
