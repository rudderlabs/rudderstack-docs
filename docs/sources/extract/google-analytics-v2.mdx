...
- **Custom Reports (Optional)**: Use this field to sync your [custom reports](https://support.google.com/analytics/answer/1033013?hl=en) from Google Analytics. Click **Add Report** and enter the **Report Name**, **Metrics**, and **Dimensions** which RudderStack will add as a JSON array, for example: `{"name": string, "dimensions": [string], "metrics": [string]}`.
...
You can choose the Google Analytics data you want to ingest by selecting the required resources:
