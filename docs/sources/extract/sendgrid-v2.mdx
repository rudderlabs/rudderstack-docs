...
Choose the SendGrid data to ingest via RudderStack. Refer to the <Link to="#supported-resources">Supported resources</Link> section below for more information. 

You can either select all the data or choose specific SendGrid data attributes as per your requirement.
...
RudderStack supports syncing the following resources from SendGrid to your warehouse destination:

| Resource | Full Refresh sync support | Incremental sync support | SendGrid API endpoint |
| :---| :---- | :---- | :---- |
...
- `single_sends`: RudderStack uses the `stats` API instead of `marketing` to ingest the data from this resource.
