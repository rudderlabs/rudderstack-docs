...
While syncing data to the warehouse, RudderStack creates columns for only those headers (first rows in the sheet) which also have data present in the second row and beyond.
