...
- **Spreadsheet Link**: From the dropdown, select the spreadsheet from which RudderStack ingests and syncs the data.
...
<ul>
  ...
  <li>For the <strong>Service Account Key Authentication</strong> option, you need to first provide access to the required spreadsheet. <strong>Only the sheets for which you provide the access will be listed in the dropdown</strong>. For more information, refer to the <Link to="#faq">FAQ</Link> section below.</li>
</ul>
</div>

- **Row Batch Size**: Enter the number of rows to be fetched from the spreadsheet while syncing the data. If not specified, RudderStack sets this value to **200** by default.
...
<div class="warningBlock">
While syncing data to the warehouse destination, RudderStack creates columns for only those headers (first rows in the sheet) which also have data present in the second row. The mere presence of a header <strong>does not</strong> create a column in the destination.
</div>
