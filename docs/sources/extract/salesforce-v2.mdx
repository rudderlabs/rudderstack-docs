...
- **Start Date**: Select the start date when you want RudderStack to ingest the Salesforce data.
      - If you specify the start date, RudderStack selects all the Salesforce data associated with the `updated` field available from that date and **does not** replicate any data before this date.
      - If you do not specify the start date, RudderStack replicates all the data associated with the `created` field by default.
...
RudderStack syncs your Salesforce data in the following manner:

1. When RudderStack first connects to your Salesforce data source, it will sync the historical data from the <Link to="#connection-settings">**Start Date**</Link> specified in the dashboard. If no start date is specified, RudderStack will sync **all** your historical data.
2. It then syncs with your Salesforce source based on the specified schedule and run frequency and incrementally replicates your data, that is, it syncs only the new or updated rows to the connected warehouse destination.
