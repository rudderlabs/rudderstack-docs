...
- **Start Date**: Select the start date when you want RudderStack to ingest the Salesforce data. RudderStack will not replicate any data before this date.

<div class="warningBlock">
RudderStack will replicate all your data by default if you do not set this field.
</div>

<div class="infoBlock">
For more information on how RudderStack replicates your Salesforce data, refer to the <Link to="#how-does-rudderstack-sync-my-salesforce-data">FAQ</Link> section below.
</div>
...
RudderStack replicates all the Salesforce data associated with the `updated` field. If not present, it picks the data associated with the `created` field.
