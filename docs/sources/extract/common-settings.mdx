...
A sync mode determines how RudderStack reads the data from a source and writes to a warehouse destination.

Currently, RudderStack supports the following two sync modes:

- Full Refresh
- Incremental

<div class="infoBlock">
In case a resource supports both the sync modes, RudderStack currently uses the <strong>Incremental</strong> mode for syncing the data.
</div>
...
In this mode, RudderStack syncs or replicates only the **new** or **modified** data. RudderStack does not replicate the data that has been already synced before.

<div class="infoBlock">
During incremental syncs, RudderStack only updates the the existing rows that have been modified as opposed to adding a new version of the row with the updated data.
</div>
