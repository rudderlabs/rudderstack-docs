...
[Mailmodo](https://www.mailmodo.com/) is an interactive email marketing tool that provides several features such as sending transactional campaigns, email automation and drip emails with customer journeys, WYSIWYG email editor, and many other tool integrations.
...
4. Log into your <a href="https://manage.mailmodo.com/auth/logi">Mailmodo account</a> and enter the **Webhook URL** based on the required format. Mailmodo can send data to a webhook in any of the following formats:    

- Send data via webhook by setting it up as an action in the user journey. Follow the detailed steps from [Mailmodo documentation](https://support.mailmodo.com/support/solutions/articles/84000365396-send-webhook-as-an-action-in-the-journey):

<img src="../../../assets/event-stream-sources/webhook_url_1.png" alt="Mailmodo source webhook URL" />

- Add Mailmodo's webhook integration from the template builder and use it to export data. Follow the detailed steps from [Mailmodo documentation](https://support.mailmodo.com/support/solutions/articles/84000351525-exporting-mailmodo-template-submissions-to-your-software-stack-using-webhook):
...
| :------------------ | :------------- |:------------- |
| `mailmodoUserId`  | `context.externalId[0].type`    | Only when [webhook is set up as an action in the user journey](https://support.mailmodo.com/support/solutions/articles/84000365396-send-webhook-as-an-action-in-the-journey). |
| `triggerData.userId`        | `context.externalId[0].value`        | Only when [webhook is set up as an action in the user journey](https://support.mailmodo.com/support/solutions/articles/84000365396-send-webhook-as-an-action-in-the-journey). |
...
- If you choose to export Mailmodo template submissions to your software stack using Webhook, a sample payload sent by Mailmodo is shown below:
