...
- If the event `type` is `ss`, then RudderStack sends it as an `identify` event:
...
- If the event `description` is `Add members to an organization`, then RudderStack sends it as a `group` event:
...
- RudderStack sends the rest of the events as `track` events:
