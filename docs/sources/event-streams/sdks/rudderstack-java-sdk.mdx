...
- Sign up to [RudderStack Cloud](https://app.rudderstack.com/signup).
- <Link to="/dashboard-guides/sources/#adding-a-source">Set up a Java source</Link> in your dashboard. You should be able to see a <strong>write key</strong> for this source, as shown:
...
The Java SDK is distributed through [**Maven Central**](https://search.maven.org).It is highly recommended to use the Maven build system to add the SDK to your project.
...
The Java SDK adds the information present in `context.library` with every message. If you pass your `context` object, then the SDK automatically merges this information except the information contained in `library`. 
...
Every SDK API you call <strong>does not</strong> result in a HTTP request but it is queued in the memory instead. RudderStack flushes the events in batches in the background, allowing for faster operations.
...
The Java SDK supports the `BlockingFlush` class that lets you flush your events from memory.
...
A more detailed implementation of the `BlockingFlush` class is shown below. Note that this is just a sample code snippet and you can modify it as per your use case.
...
To remove the limitations on the maximum number of supported parties, the Java SDK also supports the `TierBlockingFlush` class. 
...
The following snippet highlights the use of `TierBlockingFlush` class with support for more than 65535 parties. Note that this is just a sample code snippet and you can modify it as per your use case.
...
<div class="warningBlock">
Both <code class="inline-code">BlockingFlush</code> and <code class="inline-code">TierBlockingFlush</code> classes are <strong>not</strong> a part of the core Java SDK. The snippets provided above are only for reference; you can modify them as per your requirement.
</div>
...
Yes, you can use the `ImmutableMap` class via the <a href="https://guava.dev/">Guava library</a> or using the Java maps.
...
Calling the `flush()` method notifies the RudderStack client to upload the events and make sure no events are left in the queue at any given point.
