...
| `startSession()` | `sessionId` (**Long** integer with minimum length of 10 characters) <br /><br /> It is recommended to use a decimal number. | You can pass a custom `sessionId` and trigger a new user session. |
