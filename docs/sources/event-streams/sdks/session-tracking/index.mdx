...
JavaScript SDK provides `getSessionId` method to fetch the session ID of the current session. In case the session ID is unavailable, the method returns `null` value.
