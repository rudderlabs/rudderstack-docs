...
title: "Node.js"
description: >-
  Detailed technical documentation on using RudderStack’s Node SDK to send
  events to various destinations.
---

The **RudderStack Node SDK** lets you track your customer event data from your Node.js applications and send it to your specified destinations via RudderStack.
...
To set up the RudderStack Node SDK, the following prerequisites must be met:

- You need to set up a [**RudderStack account**](https://app.rudderstack.com/signup).
- Once signed up, <Link to="/dashboard-guides/sources/#adding-a-source">set up a Node source</Link> in the dashboard. You should be able to see a **Write Key** for this source, as shown below:

<img src="../../../assets/event-stream-sources/node-write-key.png" alt="Node source write key" />
...
The <strong>Setup</strong> tab in the RudderStack dashboard (seen above) has an SDK installation snippet containing both the write key and the data plane URL. You can use it to integrate the Node SDK into your application.
</div>
