...
The `traits` object is an optional dictionary included within <Link to="#context">`context`</Link> specifying the user's unique traits. This is a very useful field for linking the user's information from a previously made <Link to="http://localhost:8001/sources/event-streams/sdks/rudderstack-javascript-sdk/supported-api/#identify">`identify`</Link> call to the subsequent calls, for example, <Link to="http://localhost:8001/sources/event-streams/sdks/rudderstack-javascript-sdk/supported-api/#track">`track`</Link> or <Link to="http://localhost:8001/sources/event-streams/sdks/rudderstack-javascript-sdk/supported-api/#page">`page`</Link>.
