...
## Allowlist domain to load device mode destination

While using JavaScript SDK with any [device mode destination](https://github.com/rudderlabs/rudder-sdk-js/tree/production/integrations), you need to whitelist the domain from where the destination is loaded.

This is required as the JavaScript SDK uses a content security policy to add an extra layer of protection from any type of cross-site scripting, clickjacking, and data injection attacks.

You can refer to the device mode <Link to="/destinations/streaming-destinations/">destination documentation</Link> to obtain the destination specific domain which needs to be whitelisted. 
