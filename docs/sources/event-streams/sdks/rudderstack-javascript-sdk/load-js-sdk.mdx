...
While using the JavaScript SDK with any [device mode destination](https://github.com/rudderlabs/rudder-sdk-js/tree/production/integrations), you might need to allowlist the domain from where the destination SDK will load in your content security policy(CSP).
