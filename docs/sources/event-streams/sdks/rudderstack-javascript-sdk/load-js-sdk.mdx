...
You **must** specify the actual destination in the `<Destination>` parameter (as listed in the [RudderStack dashboard](https://app.rudderstack.com/directory)) and not the name you have assigned to the destination. For example, the below sample snippet sends the event data only to **Google Analytics** and **Intercom** destinations:
