...
| `lockIntegrationsVersion` | Boolean       |  Determines whether the JavaScript SDK (when installed using the <Link to="/sources/event-streams/sdks/rudderstack-javascript-sdk/quick-start-guide/#using-npm">npm package</Link>) should retrieve the same version of the destination SDK as the npm package, or the latest one. The default value is `False` which retrieves the latest version of the destination SDK.  |


Determines whether the SDK when used as npm package will retrieve the destination integration bundles for the same version of the package or always the latest.
Default value is false, behavior is to always retrieve the latest destination integration bundles. |
