...
Yes, you can send your <Link to="/sources/event-streams/sdks/rudderstack-javascript-sdk/filtering/#filtering-destinations">event data only to the specific destinations</Link> by stopping the SDK from loading the other <Link to="/destinations/rudderstack-connection-modes/#device-mode">device mode</Link> integrations.
