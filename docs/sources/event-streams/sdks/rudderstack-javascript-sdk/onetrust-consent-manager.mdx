...
Yes, it needs to be loaded after OneTrust SDK is initialized. We need the consent the user has given before making decisions on routing to any destination.

If the user denies onetrust, that Rudderstack does the following:
A. Does not load the Connections SDK (e.g. google anlaytics JS)
B. Does not attempt to relay events to that connection (since there’s no SDK)
...
## Consent management with other manager 

Say the customer have integrated a different consent manager and configured the categories of cookie that they want to show on their website when a new user visits their website.
Now they need to create an internal mapping between these categories and the destinations configured in Rudderstack. 
Ex: Category destination mapping:
1. Analytical cookie - Amplitude, Google Analytics
2. Targeting cookie - Bing Ads, Google Ads
3. Marketing cookie - Braze, Mailchimp, Customer.io

Now when a user provides consent, customer should fetch the consent (via API/ SDK provided by the consent manager) and filter the destinations depending on the consent. Now the customer have the list of destinations that the user has consented. Next step is to load the Rudderstack SDK(using the script provided in the doc) and configure the load call like below
...
rudderanalytics.load(WRITE_KEY,DATA_PLANE_URL,{
   integrations: {
       ...
       // only provide the destinations the the user has consented
   }
   // pass other initialization option
});
```

In this case we have considered user has given consent for analytical cookie so other destinations are not mentioned here.
...
### What happens if RudderStack JavaScript SDK is loaded before OneTrust box is closed?

Rudderstack SDK will not be able to capture the consent, hence it will load all the integrations and events will flow through all the connected destinations.