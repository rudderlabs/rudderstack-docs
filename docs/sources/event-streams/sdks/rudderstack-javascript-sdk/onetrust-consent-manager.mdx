...
<div class="warningBlock">
As the above snippet shows, you must load the OneTrust script first and if the user provides consent, then only load the JavaScript SDK. This is because the JavaScript SDK determines whether to send events to a destination based on the user's consent. If a user denies OneTrust, then Rudderstack:
- Does not load the destination SDK.
- Does not attempt to relay events to the connected destination.
</div>
...
If you are using a consent manager other than OneTrust, you can configure the cookie categories you want to show on your website whenever a new user visits your website.
...
When a user provides consent, you should fetch the consent (via API/SDK provided by your consent manager) and filter the destinations depending on the consent. Once you have the list of destinations that the user has provided consent for, you can load the Rudderstack JavaScript SDK. 

For example, if a user has provided consent for analytical cookie, then you can load the JavaScript SDK as shown:
...
### What will happen if RudderStack JavaScript SDK is loaded before the OneTrust modal is closed?

In such a case, the JavaScript SDK will not be able to capture the user consent. Hence, it will load all the connected destinations and the events will flow through all of them.