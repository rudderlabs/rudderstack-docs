...
<div class="warningBlock">
As the above snippet shows, you <strong>must</strong> first load the OneTrust script and if the user provides consent, only then load the JavaScript SDK. This is because the JavaScript SDK determines whether to send events to a destination or not based on the user's consent. If a user denies OneTrust, then Rudderstack neither loads the destination SDKs nor sends any events to them.
</div>
...
When a user provides consent, you should fetch the consent (via API/SDK provided by your consent manager) and filter the destinations depending on the consent. Once you have the list of destinations that the user has provided consent for, you can load the Rudderstack JavaScript SDK for those destinations.
...
In such a case, the JavaScript SDK will not be able to capture the user consent. Hence, it will load all the connected destinations and all the events will flow through them.