...
- If you have self-hosted the JavaScript SDK in the below recommended structure:
  ...
  In this structure, the file name for the JavaScript SDK is `rudder-analytics.min.js` and device mode destination SDKs are located next to the core SDK file under the `js-integrations` directory. To migrate to v1.1, update the script tag in your website, as shown:
