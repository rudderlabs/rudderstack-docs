...
## Migrate to JavaScript SDK v1.1
...
## Migration based on JavaScript SDK v1 installation
...
- If you have self-hosted the JavaScript SDK in the below recommended structure:
  ...
  Meaning, the file name for the JavaScript SDK is `rudder-analytics.min.js`, and the device mode destination SDKs are located next to the core SDK file under the `js-integrations` directory, update the script tag as shown:

    ```html
    <script src="https://<subdomain>.<yourdomain>.com/rudder-analytics.min.js"></script>
    ```

  - If the file name of your self-hosted JavaScript SDK is `rudder-analytics.min.js`, and the destination SDKs are **not** located next to the core SDK file under the `js-integrations` directory, update the script tag as shown:
...
- If your JavaScript SDK file name is **not** `rudder-analytics.min.js`, update the script tag in your website, as shown:
  ...
  // provide the location of the destination SDKs in the load options
  rudderanalytics.load(WRITE_KEY, DATA_PLANE_URL, {
      destSDKBaseURL: "https://<subdomain>.<yourdomain>.com/js-integrations"
  });
  // ...
  </script>
  <script src="https://<subdomain>.<yourdomain>.com/<path-to-custom-analytics-file>"></script>
  ```
...
By default, the device mode destinations are loaded from https://cdn.rudderlabs.com/v1.1/js-integrations/, depending on the dashboard settings. The name of the destination is appended to the URL as:
