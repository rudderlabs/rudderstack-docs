...
2. <Link to="/dashboard-guides/sources/#adding-a-source">Set up a Java source</Link> in your dashboard. You should be able to see a <strong>write key</strong> for this source.
...
To use the SDK, create a client instance as shown below:
...
To manually batch events, use the `Analytics.batch` as shown:
