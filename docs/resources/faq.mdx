...
You should use the `track` method instead. For the JavaScript SDK's `track` method parameters specific to ecommerce, you can refer to the <Link to="/event-spec/ecommerce-events-spec/">Ecommerce Events Spec</Link>.
...
<div class="successBlock">
For a comprehensive list of warehouse destinations-specific FAQ, refer to the <Link to="/destinations/warehouse-destinations/faq/">Warehouse FAQ</Link> guide.
</div>
