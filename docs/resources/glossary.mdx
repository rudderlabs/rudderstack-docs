...
For more information on the audit logs feature, refer to the <Link to="/dashboard-guides/audit-logs/">Audit Logs</Link> guide.
...
For more information on the various Cloud Extract sources and how to set them up in RudderStack, refer to the <Link to="/sources/extract/">Cloud Extract</Link> documentation.

## Connection (Pipeline)

A connection is a one-to-one directional flow of events between a RudderStack source and a destination.

For more information on sources and destinations in RudderStack, refer to the <Link to="/dashboard-guides/overview/">Dashboard Guide</Link> section.
...
For more information, refer to the <Link to="/destinations/rudderstack-connection-modes/">Connection Modes</Link>.
...
For more information on the control plane, refer to the <Link to="/resources/rudderstack-architecture/">RudderStack Architecture</Link> guide.
...
For information on Control Plane Lite and how to use it, refer to the <Link to="/get-started/rudderstack-open-source/control-plane-lite/">Control Plane Lite</Link> guide.
...
RudderStack's <Link to="/features/data-governance/">Data Governance</Link> feature gives you the ability to access all your events and their metadata programmatically and identify any inconsistencies in them. This includes vital information related to the event schema, event payload versions, data types, and more.
...
For more information on the data plane, refer to the <Link to="/resources/rudderstack-architecture/">Architecture</Link> guide.
...
* <Link to="/get-started/rudderstack-cloud/">RudderStack Cloud</Link>: RudderStack hosts both control plane and data plane
* <Link to="/get-started/rudderstack-open-source/">RudderStack Open Source</Link>: RudderStack hosts the control plane and the user hosts the data plane. Alternatively, the user hosts both the control plane and the data plane.
...
<div class="infoBlock">
Refer to the <Link to="/resources/rudderstack-architecture/">Architecture</Link> guide for more information on the RudderStack data plane.
</div>

- If you're using [RudderStack Cloud Free](https://app.rudderstack.com/), the data plane URL is present in your dashboard itself.
- If you're using [RudderStack Pro](https://rudderstack.com/pricing/) or [Enterprise](https://rudderstack.com/pricing/), [contact us](mailto:support@rudderstack.com) for the data plane URL with the email ID you used to sign up for RudderStack.
- If you're using <Link to="/get-started/rudderstack-open-source/">RudderStack Open Source</Link>, you are required to <Link to="/get-started/rudderstack-open-source/data-plane-setup/">set up your own data plane</Link> in your preferred environment.
...
Refer to the <Link to="/api/data-regulation-api/">Data Regulation API</Link> guide for more information.
...
Refer to the <Link to="/destinations/overview/">Destinations</Link> guide for the complete list of the supported destinations.
...
Refer to the [RudderStack blog](https://rudderstack.com/blog/rudderstack-cloud-extract-makes-cloud-to-warehouse-pipelines-easy) to read more about ELT and how RudderStack facilitates it via the <Link to="/sources/extract/">Cloud Extract</Link> feature.
...
For more information on the RudderStack event spec, refer to the <Link to="/event-spec/standard-events/">Event Spec</Link> guide.
...
For more information on the various Event Streams sources supported by RudderStack, refer to the <Link to="/sources/event-streams/">Cloud Apps</Link> guide.

## Live Events

RudderStack's <Link to="/dashboard-guides/live-events/">Live Events</Link> feature is a debugger that shows the live events collected from your sources and sent to the connected destinations in real-time. With this feature, you can easily debug any errors in the failing events at a destination level and reduce your troubleshooting time and efforts.

Broadly speaking, this feature can be further classified into two major categories:

* <Link to="/dashboard-guides/live-events/#source-live-events">Source Live Events</Link>: This feature gives you real-time visibility into the source events collected by RudderStack. This way, you can confirm if your source is configured correctly and is collecting & sending data as expected. 
* <Link to="/dashboard-guides/live-events/#destination-live-events">Destination Live Events</Link>: When routing events to a destination, sometimes events don't show up in your destination. This feature gives you real-time visibility into the destination's responses and helps you troubleshoot the problem.
...
Refer to the [RudderStack blog](https://rudderstack.com/blog/reverse-etl-is-just-another-data-pipeline) to read more about Reverse ETL. For more information on the supported Reverse ETL sources and setting them up in RudderStack, refer to the <Link to="/sources/reverse-etl/">Reverse ETL</Link> documentation.

## SDK

RudderStack offers SDK support for your web, mobile, and server-side sources and lets you track your event data seamlessly.

 For the SDK-specific documentation, refer to the <Link to="/sources/event-streams/sdks/">SDK Integration Guides</Link>.
...
For more information on RudderStack sources, refer to the <Link to="/sources/">Sources</Link> guide.
...
For more information on this feature, refer to the <Link to="/sources/reverse-etl/features/visual-data-mapper/">Visual Data Mapper</Link> guide.
...
For more information on this feature, refer to the <Link to="/dashboard-guides/teammates/">Teammates</Link> guide.

## Transformations

RudderStack's <Link to="/features/transformations/">Transformations</Link> feature lets you leverage your custom JavaScript functions that you can use to implement a variety of use-cases like: 
...
For more details on this feature and how to use it, refer to the <Link to="/features/transformations/">Transformations</Link> guide.
...
For more information on how to set up these warehouse destinations, refer to the <Link to="/destinations/warehouse-destinations/">Warehouse Destinations</Link> guide.
...
Refer to the <Link to="/destinations/warehouse-destinations/warehouse-schema/">Warehouse Schema</Link> guide for more details.
...
<div class="infoBlock">
The workspace ID is required when setting up an IAM role for RudderStack for authenticating all the AWS-based integrations.
</div>

To get the workspace ID, log in to your [RudderStack dashboard](https://app.rudderstack.com/) and go to **Settings** > **Company**, as shown:

<img src="../assets/rs-cloud/workspace-id.png" alt="Workspace ID" />
...
To get the workspace token, log in to your [RudderStack dashboard](https://app.rudderstack.com/) and go to **Settings** > **Company**, as shown:
...
To view the workspace token, click <strong>Reveal Token</strong> and enter the password associated with your RudderStack account.
...
For more information on finding the source write key, refer to the <Link to="/dashboard-guides/sources/#faq">Sources</Link> guide.
