...
You can delete a user by specifying <code class="inline-code">userId</code> or any custom identifier in the event for all the above destinations except <strong>Redis</strong> and <strong>S3</strong>. For <strong>Redis</strong> and <strong>S3</strong>, RudderStack supports deleting a user only by specifying the <code class="inline-code">userId</code>.
</div>
