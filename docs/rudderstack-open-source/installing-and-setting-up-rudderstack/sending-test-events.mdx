...
The source write key is different from your <Link to="/rudderstack-cloud/dashboard-overview/#workspace-token">workspace token</Link>. The write key is associated with the source, while the workspace token is associated with your RudderStack workspace.
</div>
