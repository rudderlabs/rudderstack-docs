...
<div class="infoBlock">
Refer to the <Link to ="/tmp-destinations/warehouse-destinations/">warehouse-specific documentation</Link> for more information on configuring a namespace while setting up the destination in RudderStack.
</div>
...
While configuring Snowflake as a destination in RudderStack, you need to enter your <Link to="/tmp-destinations/warehouse-destinations/snowflake/#connection-settings">Snowflake connection credentials</Link> which includes the **Account** field, as shown below:
