...
In case of Google BigQuery, you can use the views created over the tables to query for unique users in the dataset. Refer to the <Link to="/tmp-destinations/warehouse-destinations/bigquery#schema-partitioned-tables-views-and-deduplication">BigQuery documentation</Link> for more details.
</div>
...
Yes, you can. Although the default namespace will be the source name, RudderStack gives you the option to explicitly set the namespace while setting up your warehouse destination. For more information, refer to the <Link to="/tmp-destinations/warehouse-destinations/">warehouse-specific destination settings</Link> for configuring the namespace in the RudderStack dashboard.
