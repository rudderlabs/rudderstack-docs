...
- **Use device mode to send events**: If enabled, RudderStack will send the events to PostHog via the <Link to="/tmp-destinations/rudderstack-connection-modes/#device-mode">device mode</Link>.
