...
<div class="infoBlock">
As this is a web device mode-only integration, the <strong>Use native SDK to send events</strong> option cannot be disabled. For more information on the web device mode, refer to the <Link to="/tmp-destinations/rudderstack-connection-modes/">RudderStack Connection Modes</Link> guide.
</div>
