...
<div class="infoBlock">
This option is applicable only if you're sending events to HubSpot via the web device mode. Refer to the <Link to="/sources/sdks/event-filtering/">Client-side Event Filtering</Link> guide for more information on this feature.
</div>

### Web SDK settings

- **Use device mode to send events**: Enable this setting to send events via the <Link to="/tmp-destinations/rudderstack-connection-modes/#device-mode">device mode</Link>.
