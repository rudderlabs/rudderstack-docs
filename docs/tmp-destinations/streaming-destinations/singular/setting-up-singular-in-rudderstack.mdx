...
<div class="infoBlock">
For more information on sending session events to Singular, refer to the <Link to="/tmp-destinations/streaming-destinations/singular/singular-cloud-mode/">Singular Cloud Mode</Link> guide.
</div>

- **Use device mode to send events**: When using the Android or iOS platforms, you can enable this setting to send events via the <Link to="/tmp-destinations/rudderstack-connection-modes/#device-mode">device mode</Link>. Then, follow the <Link to="/tmp-destinations/streaming-destinations/singular/singular-device-mode/">Singular Device Mode</Link> guide for steps on adding Singular to your project.
