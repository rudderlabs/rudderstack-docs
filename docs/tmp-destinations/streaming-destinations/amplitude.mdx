...
Session tracking is only supported for the RudderStack <Link to="/tmp-destinations/rudderstack-connection-modes/#device-mode">device mode</Link> because the integration loads Amplitude's SDK. 
Events sent via the <Link to="/tmp-destinations/rudderstack-connection-modes/#cloud-mode">cloud mode</Link> connections will have session IDs as `-1`.
