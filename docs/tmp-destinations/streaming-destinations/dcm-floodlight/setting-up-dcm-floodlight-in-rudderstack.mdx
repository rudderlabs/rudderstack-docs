...
<div class="infoBlock">
This option is applicable only if you're sending events to DCM Floodloght via the web device mode. Refer to the <Link to="/sources/sdks/event-filtering/">Client-side Event Filtering</Link> guide for more information on this feature.
</div>

### Web SDK settings

<div class="infoBlock">
All the settings mentioned in this section are applicable only for the <Link to="/tmp-destinations/rudderstack-connection-modes/#device-mode">device mode</Link>.
</div>

<img src="../../../assets/event-stream-destinations/dcm-floodlight-connection-settings4.png" alt="DCM Floodlight connection settings"/>

- **Use device mode to send events**: Enable this setting to send events via the <Link to="/tmp-destinations/rudderstack-connection-modes/#device-mode">RudderStack device mode</Link>.
