...
The above callback triggers a call to RudderStack SDK's <Link to="/event-spec/standard-events/track">`track`</Link> API which dumps the `track` call payload to a configured S3 destination in your RudderStack dashboard, each time the syncing of the DSP Pixels happens.
{% endtab %}
