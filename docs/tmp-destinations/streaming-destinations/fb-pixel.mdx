...
You can set the `event_id` in `traits` , `context.traits` or `properties` object as a unique identifier. This works for both the <Link to="/tmp-destinations/rudderstack-connection-modes/#cloud-mode">cloud mode</Link> as well as the <Link to="/tmp-destinations/rudderstack-connection-modes/#device-mode">device mode</Link>.
