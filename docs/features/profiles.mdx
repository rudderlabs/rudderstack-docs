...
This guide walks you through RudderStack **Profiles**. It lets you build a unified customer profile from all the cross-platform data residing in your warehouse.
...
All of this data is typically scattered across multiple tables within a warehouse. **Profiles** is RudderStack's solution to tie all the user data from these tables into a unified view, giving you a 360-degree view of the user - often called as **Customer 360**.
...
To understand how Profiles works, you will implement a use case by leveraging a sample dataset based on [RudderStack's ecommerce tracking plan](https://www.rudderstack.com/docs/event-spec/ecommerce-events-spec/). This data is available in Snowflake, which you can access by following the steps in this section. 
...
<div class="infoBlock">
These are very basic features to demonstrate how you can use Profiles. You can add a lot more features with complex logic to explore its full capabilities.
</div>
...
2. Set up a connection profile to connect to RudderStack's data warehouse having the test dataset for this use case. To do so, go to your terminal and enter `pb init profile` and follow the prompts. A sample profile for a Snowflake account is shown below:
...
In the `pb_project` YAML  file, `entities` and `id_types` play an important role. **All the features are computed at an entity level**. A user is one of the most common entity types; some other examples are organization/company, products, plans, etc. 

Also, an entity can have multiple identifiers like email, phone number, user ID, etc. All these identifiers are listed under `id_types`.

<img src="../assets/features/profiles-entities-idtypes.png" alt="Entities and ID types" />

In the above example, both the `id_types` fields are the same. In the first instance, we tie `id_types` to `entities` and in the second instance, we define them.
...
6. Next, go to the `profiles.yaml` file where the models are defined. You will find two models here - `id_stitcher` and `feature_table_model`. For ID stitching, retain only the first model (`model_type: id_stitcher`) and **comment out the second model** (`model_type: feature_table_model`). Also, change the `name` field from `domain_profile_id_stitcher` to `user_id_stitcher`, as in **Step 4**. 

<div class="infoBlock">
In the <code class="inline-code">name</code> field, it is important to use the same entity name set in <strong>Step 4</strong>. In doing so, you link the ID stitcher in the <code class="inline-code">pb_project.yaml</code> file to the model spec in <code class="inline-code">profiles.yaml</code>.
</div>
...
In this section, you will define the features mentioned in the [use case](#use-case) to create a feature table. For this, you need to modify the `feature_table_model` model in the `profiles.yaml` file of your project folder.

9. Uncomment the lines you commented out in <Link to="#part-2-setting-up-id-stitching">Step 6</Link>. Then, modify the `name` from `domain_profile` to `user_profile`. Similar to `user_id_stitcher`, this is the name of the output table that will be created in the warehouse. Also, delete all the information present in the `vars` key as you will <Link to="#feature-definitions">write your own features</Link> for the model instead.
...
You can write each feature in the `entityvar` field as shown:
