---
title: "Profiles"
description: Create comprehensive and unified customer profiles with RudderStack's Profiles feature.
---

This guide walks you through RudderStack's **Profiles** feature with a simple use case. It lets you build a unified customer profile from all the cross-platform data residing in your data warehouse.

## Introduction

Companies gather user data across various digital touchpoints like websites, mobile apps, enterprise systems like CRMs, marketing platforms, and various other systems. A user is also identified with multiple identifiers across their product journey like email ID, phone number, device ID, anonymous ID, account usernames, and so on, with multiple such identifiers as they change their device, use work and personal emails together, and so on.
...
9. Uncomment the lines you commented out in <Link to="#part-2-setting-up-id-stitching">Step 6</Link> and modify the `name` from `domain_profile` to `user_profile`. Similar to `user_id_stitcher`, this is the name of the output table that will be created in the warehouse. Also, delete all the information present in the `vars` key - as you will write your own features instead.
...
Once you add all the features, the `profiles.yaml` file should look something like below:
