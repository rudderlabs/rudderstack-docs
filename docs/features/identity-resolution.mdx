...
If you are using the merge call **for warehouse operations**, not to worry: it will continue to work for the foreseeable future. We would encourage you to consider joining our [Profiles beta]() when it is available in 2023. Contact your RudderStack Account Manager for details. Profiles will perform identity resolution automatically, without the need for a separate call.

**[INTERNAL/REVIEW NOTES]**:

How much more/less do we want to say about RS360?

I didn't get much into explaining how the merge call works, especially as it relates to the streams/identify/alias. If we think we should go into more detail clarification on that, it might be better to weave that into the doc itself, rather than in a top-of-page callout.
</div>
