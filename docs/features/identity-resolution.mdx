...
In the first half of 2003 we will begin the deprecation process for our `merge` API, which allows you to merge different user identities into a single customer profile in the warehouse. The `merge` call is only used in 
**warehouse operations**. 

If you are using the merge call **for warehouse operations**, not to worry: it will continue to work for the forseeable future. We would encourage you to consider joining our [RS360 beta]() when it is available in 2023. Contact your RudderStack Account Manager for details. RS360 will perform identity resolution automatically, without the need for a separate call.
