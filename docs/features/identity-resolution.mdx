...
<div class="infoBlock">
Currently, RudderStack supports identity resolution only for the <Link to="/tmp-destinations/warehouse-destinations/bigquery/">Google BigQuery</Link> and <Link to="/tmp-destinations/warehouse-destinations/snowflake/">Snowflake</Link> warehouse destinations.
</div>
...
- `userId` and `anonymousId` for the <Link to="/event-spec/standard-events/track/">`track`</Link>, <Link to="/event-spec/standard-events/group/">`group`</Link>, and <Link to="/event-spec/standard-events/screen/">`screen`</Link> calls.
