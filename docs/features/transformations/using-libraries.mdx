...
Suppose you want to filter the events that don't contain an email address with the RudderStack domain. To do so, you can import a `rudderEmail` function from a transformation library called `isRudderEmail`, as shown:
