...
In case you are using both the above-mentioned options, your events are filtered (based on the integration options passed along with the event and/or event filtering set in the RudderStack dashboard) and transformed (based on the applied transformation) before being sent to the destination.

### Is there any added cost to use the transformations in device mode?
 
Yes, while applying transformations in device mode, RudderStack sends a network call for each event which is an extra overhead.
    
### Can I switch back to the previous version of an SDK while using a device mode transformation?
...
For JavaScript, React-Native, and Cordova SDKs, there is a chance of losing the  events in case of a network error.
