...
<div class="infoBlock">
For more information, refer to the <Link to="/features/transformations/using-transformations#applying-transformation-on-a-batch-of-events">Applying transformation on a batch of events</Link> section.
</div>
...
- **<Link to="/sources/event-streams/sdks/event-filtering/">Event filtering</Link>**: When you need to specify which events should be discarded or allowed to flow through to the destination.
- **Transformations**: When you need to mask the data, enrich events, implement specific actions on events or remove sensitive PII information from the events.

In case you plan to use both the above-mentioned features, your events will be filtered (based on the <Link to="/sources/event-streams/sdks/rudderstack-javascript-sdk/load-js-sdk/#integrationopts">`integrationOpts`</Link> passed along with the event and/or <Link to="/sources/event-streams/sdks/event-filtering/">event filtering</Link> in the RudderStack dashboard) and transformed (based on the applied transformation) before being sent to the destination.

### Is there any added cost to use the transformations in device mode?
 
Yes, RudderStack sends a network call for each event when the transformations are used in the device mode. It leads to extra overhead and latency.
...
- **For JavaScript SDK**: It depends on the SDK installation method:
    ...
    - If installed through NPM, it's possible to downgrade the SDK version and choose whether to use the transformation or not. 
...
However, there is a chance of events getting lost while sending them via JavaScript SDK in case of a network error.
...
1. Create a new transformation by following the steps in the <Link to="/features/transformations/using-transformations#adding-a-transformation">Adding a transformation</Link> section.
2. Add the updated transformation code as specified in the <Link to="/features/transformations/using-transformations#transformevent-function">`transformEvent` function</Link> section.
3. Connect the transformation to your destination. For more information, refer to the <Link to="/features/transformations/using-transformations#connecting-transformation-to-a-destination">Connecting a transformation to the destination</Link> section.
