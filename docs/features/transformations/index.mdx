...
RudderStack's **Transformations** feature lets you write custom functions to implement specific use-cases on your event data, like event filtering, sampling, aggregation, enrichment, and more. 

## Key features

- Easier to build, manage, debug, and reuse.
- You can add custom logic to your source events in real-time, before sending them to your destination.
- Support for JavaScript and <Link to="#python-transformations">Python</Link>.
- You can use the transformations across your <Link to="/sources/event-streams/">Event Streams</Link>, <Link to="/sources/extract/">Cloud Extract</Link>, and <Link to="/sources/reverse-etl/">Reverse ETL</Link> pipelines in both <Link to="/features/transformations/cloud-mode-transformations">cloud mode</Link> and <Link to="/features/transformations/device-mode-transformations">device mode</Link>.
- Ability to version control your transformations.
- You can create an organization-wide sandbox where your team can store all the transformations before publishing them in production.
...
RudderStack currently supports only two Python packages for writing your transformations - <a href="https://pypi.org/project/requests/">requests</a> and <a href="https://pypi.org/project/python-dateutil/">dateutil</a>.
...
4. In the **Transformation** window, select the language to write your transformations, as shown:

<img src="../../assets/features/adding-a-transformation-1.png" alt="Adding a Transformation" />

5. Add your transformation function in the , as shown:
