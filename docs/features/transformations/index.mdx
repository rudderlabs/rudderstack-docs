...
You can use transformations when sending events to destinations via Android, iOS, JavaScript, React-Native, and Cordova SDKs in <Link to="/destinations/rudderstack-connection-modes/#device-mode">device mode</Link>.
</div>
...
- Events routed to device mode destinations that don’t have any transformation service attached will be sent immediately to that destination.
- We store the events in the local DB before sending them for transformation. So if the transformation fails for any reason e.g., due to network unavailability we again try after some time. In this way, events will not be lost due to network issues.
    
  Saving of events in the local DB is supported only on Mobile SDKs.
...
- Now, along with the cloud mode destinations, all device mode destinations will support transformation service.
- Users can make use of their existing transformation service for device mode transformations as well.
- Events are filtered during device mode transformation based on the integration options passed along with the event and/or [event filtering](https://www.rudderstack.com/docs/sources/event-streams/sdks/event-filtering/) set at the dashboard.

Cons: 

- For supporting device mode transformations, we need to send an HTTP request for each event/batch of events, which will cause latency.
- Increased network traffic. Each event will generate an API call, that’s going to increase network consumption substantially.


### Limits for DMT

- If device mode transformation is enabled then Realtime event push to destination SDKs won't be possible.
- Latency will depend upon network availability and server response.
- For iOS, there is no support for background processing when the app is killed. This means a backlog event will be sent, the next time the app is opened.
