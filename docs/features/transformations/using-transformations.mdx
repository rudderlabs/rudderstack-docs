...
### Additional device mode limitations 
...
- Latency in sending the events depend upon the network availability and server response.
- For iOS SDK, the background processing of an event is not supported when the app is closed. The backlog event is sent the next time the app is opened.