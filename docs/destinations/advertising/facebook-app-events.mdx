...
description: >-
  Step-by-step guide on sending your event data from RudderStack to Facebook App Events.
---
...
RudderStack supports Facebook App Events as a destination to which you can seamlessly send your event data.
...
Before configuring App Events as a destination in RudderStack, verify if the source platform is supported by referring to the table below:
...
To know more about the difference between cloud mode and device mode in RudderStack, refer to the <Link to="/rudderstack-cloud/rudderstack-connection-modes">Connection Modes</Link> guide.
</div>

Once you have confirmed that the source platform supports sending events to App Events, follow these steps:

1. From your [RudderStack dashboard](https://app.rudderstack.com/), add the source. Then, from the list of destinations, select **Facebook App Events**.
2. Assign a name to your destination and click on **Continue**.

### Connection settings

To successfully configure App Events as a destination, configure the following settings:

<img src="../../assets/event-stream-destinations/app-events-connection-settings.png" alt="App Events connection settings" />

- **App ID**: Enter your Facebook App ID.

<div class="infoBlock">

For more information on obtaining the Facebook App ID, refer to the <Link to="#faq">FAQ</Link> section below.
</div>

The following option is applicable **only if** you are using the device mode to send your events to App Events:

- **Limited Data Use**: If this option is enabled, you can send the country and state of the user so that Facebook processes the user data according to the data regulations set for that region.

<div class="infoBlock">

For more information on this option, refer to the <Link to="/destinations/advertising/facebook-app-events/#limited-data-use">Limited data use</Link> section below.
</div>
...
<Tabs>
  <TabList>
    ...
    <TabPanels>
      <TabPanel>
        To integrate App Events with your iOS app, follow these steps:
<ol>
...
<li>Send the user's consent to App Events, as shown:
  <ul>
...
<li>Then, initialize the RudderStack iOS SDK, as shown:
...
```
      ...
      <TabPanel>
        To integrate App Events with your Android app, follow these steps:
<ol>
...
<li>Open your <code class="inline-code">/app/res/values/strings.xml</code> file and add the following lines. <strong>Remember to replace <code class="inline-code">[APP_ID]</code> with your Facebook app ID</strong>.
<br /><br />
<span>
...
For App Events, RudderStack supports <code class="inline-code">identify</code> events only in the mobile device mode.
</div>

RudderStack's [`identify`](https://rudderstack.com/docs/rudderstack-api/api-specification/rudderstack-spec/identify/) call sets the `userId` through the `setUserID` method from `AppEventsLogger`.

RudderStack sets the following properties \(if applicable\) using the `setUserData` method.
...
RudderStack's [`track`](https://rudderstack.com/docs/rudderstack-api/api-specification/rudderstack-spec/track/) events are logged to Facebook using the `logEvent` method of the `AppEventsLogger` class. 

RudderStack uses the same event name present in the `track` event along with the properties before passing the data to App Events.
...
You can find the **Facebook App ID** by logging into your Facebook developer account, and navigating to the **Home** page of your app's dashboard.
