...
| `ord` | Makes the Floodlight tag unique and prevent browser caching. |
| `quantity` | RudderStack adds the quantity of all the products in the `products` array or refers to the top-level `quantity` property. Applicable for **Sales** tags only. |
| `value` | RudderStack sends the `revenue` parameter to DCM Floodlight. Applicable for **Sales** tags only. |
...
| RudderStack custom field | DCM Floodlight property | Counting method| 
| :--- | :--- |:--- |
...
| `GDPR` | `tfua` | All | 
| `npa` | `npa` | All | 
| `properties.matchId` | `matchId` | All | 
| `properties.num` | `num` | Counter - unique | 
| `properties.sessionId` | `session_id` | Counter - per_session |
