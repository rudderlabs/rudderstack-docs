...
| `quantity` | Applicable for **Sales** tags only - RudderStack adds the quantity of all the products in the `products` array or refers to the top-level `quantity` property. |
...
| RudderStack custom field | DCM property | Counting method| 
| :--- | :--- |
| `properties.ord` | `ord` | All| 
...
Based on the presence of `name` and `category` field in the `page` call, RudderStack transforms the event in the following format:
