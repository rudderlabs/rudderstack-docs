...
| `properties.ord` | `ord` |Counter - `per_session` |
...
| RudderStack custom field | Counting method| 
| :--- | :--- |
| `properties.ord` | All| 
| `context.device.adTrackingEnabled` | All |
| `COPPA` | All | 
| `GDPR` |  All | 
| `npa` | All | 
| `properties.matchId` | All | 
| `properties.num` | Counter - unique | 
...
* When you do not pass the `name` field in the `page` call, RudderStack transforms it to the **Viewed Page** format.
* When you pass the `name` field in the `page` call, RudderStack transforms it to the **Viewed `PAGE NAME` Page** format.
* When you pass the `category` and `name` fields in the `page` call, RudderStack transforms it to the **Viewed `CATEGORY NAME` `PAGE NAME` Page** format.
