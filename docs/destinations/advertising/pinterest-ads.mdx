...
- **Enable Enhanced Match on Page Load**: This setting is enabled by default and attaches the hashed email address on the initial page load. Any further calls to Pinterest are an Enhanced Match.
...
Rudderstack supports [Pinterest Enhanced Match](https://help.pinterest.com/en/business/article/enhanced-match) when the following conditions are met:
  ...
  - A user visits your site anonymously but is identified at some later stage by making an `identify()` call.
...
For the above example, the following properties must be added in the **Custom Properties** section:
