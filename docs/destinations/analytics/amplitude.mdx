...
Suppose you defined <strong>Group Name Trait</strong> as `RS` and <strong>Group Value Trait</strong> as `RudderStack` and made the `group` call. Then, the user would be associated with a Group name : `RS` and a Group Value : `RudderStack`.
