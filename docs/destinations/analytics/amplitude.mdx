...
<div class="infoBlock">
When sending events via the device mode, <code class="inline-code">group</code> is currently supported only by the RudderStack JavaScript SDK.
</div>
...
Suppose you defined <strong>Group Name Trait</strong> as `company_id` and <strong>Group Value Trait</strong> as `company_name` and made the `group` call. Then, the user would be associated with a Group name : `RS` and a Group Value : `RudderStack` and you can also see the user property with type `RS` and value `RudderStack` in the user profile.
