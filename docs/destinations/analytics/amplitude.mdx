...
RudderStack supports sending `event_id` to Amplitude in the device mode. It can be added under the `integrations` object and is supported for all the above mentioned API calls, namely, `identify`, `track`, `page`, `screen`, and `group`.
  
A sample `identify` call with `event_id` looks like the following:
