...
<div class="infoBlock">
This feature is applicable <strong>only</strong> for the web device mode integration.
</div>
...
```
      ...
      <TabPanel>
        To add Amplitude to your Android project, follow these steps :
<ol>
...
<div class="infoBlock">
<strong>Applicable for mobile only</strong>: If a property with the name <code class="inline-code">optOutOfSession</code> and value <code class="inline-code">true</code>then this <code class="inline-code">track</code> call will be opted out of the current session if it exists or does not start a new session if there isn't any active session.
</div>
...
<div class="infoBlock">
RudderStack supports sending the <code class="inline-code">group</code> calls in the cloud mode. However, when sending events via the device mode, <code class="inline-code">group</code> is currently supported only by the JavaScript SDK.
</div>

To use the Amplitude Groups feature with RudderStack, you need to define the <strong>Group Name Trait</strong> and <strong>Group Value Trait</strong> in the dashboard settings and pass them as traits while making a `group` call.
...
Suppose you have defined the <strong>Group Name Trait</strong> as `RS` and <strong>Group Value Trait</strong> as `RudderStack` and made the `group` call, then the user would be associated with the group name as `RS` and the group value as `RudderStack`.
