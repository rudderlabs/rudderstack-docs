...
- **Enable Server Side Identify**: Enable this setting to send an <Link to="/rudderstack-api/api-specification/rudderstack-spec/identify/">`identify`</Link> call.
- **Trait to differentiate between new or existing user**: Enter a trait key traits to determine whether its a new or an existing user.
- **Convert identify event to Login or Signup event**: Enable this setting to convert the `identify` call to a `sign_up` event in case of a new user or a `login` event in case of an existing user.
- **Trait key for the 'method' parameter in Login or Signup event**: Enter the trait key for passing the `method` parameter for `login` and `sign_up` event.
- **Send 'generate_lead' event for 'identify' event**: Enable this setting to send a `generate_lead` event based on a new or an existing user. Note that `generate_lead` will only be fired for new users and not for the existing ones. When enabled and a its new user, you also need to enter the following details:
  - **Value - traits for lead generation**: Enter the key that will be retrieved from traits to determine **value** for Lead Generation.
  - **Currency - traits for lead generation**: Enter the key that will be retrieved from traits to determine **currency** for Lead Generation.
