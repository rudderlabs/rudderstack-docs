...
User-ID is an advanced feature that lets GA4 present a cross-device, cross-platform view of your customers' behavior. Google Analytics 4 uses `userId` and `deviceId` \(GA's `cid` value from Universal Analytics terminology\) to identify users. The `userId` is a unique identifier that tells Google Analytics 4 if two devices/browsers belong to the same user.
...
The Google Signals feature is available in the web <Link to="/rudderstack-cloud/rudderstack-connection-modes/#device-mode">device mode</Link> only.
</div>
