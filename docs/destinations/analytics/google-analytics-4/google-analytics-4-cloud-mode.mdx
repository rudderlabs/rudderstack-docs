...
| `non_personalized_ads`   |  `context.device.adTrackingEnabled` | Indicates whether the events should be used for personalized ads. Set to `true` if they should not be used for personalized ads.|

<div class="infoBlock">
  
Google Analytics 4 Measurement Protocol supports events only with timestamps 72 hours into the past and 15 minutes into the future. Any event with timestamps out of this range will throw an error.
</div>
