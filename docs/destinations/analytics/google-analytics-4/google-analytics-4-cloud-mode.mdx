...
- If you use `gtag`, passing the `client_id` parameter is mandatory. The `client_id` uniquely identifies a user instance of a web client. Google Analytics uses it as an identifier to distinguish the site visitors.
...
The mappings for common parameters across `gtag` and `firebase` are listed in the following table:

| Optional Parameters    | Mapping value  | Description |
| :-------------| :-----------------|:-----------------|
| `user_id`         |  `userId`, `traits.userId`, `traits.id`, `context.traits.userId`, `context.traits.id` | Unique identifier for a user which helps Google Analytics 4 know that two devices/browsers belong to the same user.|
| `timestamp_micros`   |  `originalTimestamp` or `timestamp`| Unix timestamp (in microseconds) to associate the time with an event.|
| `non_personalized_ads`   |  `context.device.adTrackingEnabled` | Set to `true` to indicate that these events should not be used for personalized ads.|

<div class="infoBlock">
  
Google Analytics 4 Measurement Protocol supports events only with timestamps of 72 hours into the past and 15 minutes into the future. Any event with timestamps out of this time range will throw an error.
</div>
