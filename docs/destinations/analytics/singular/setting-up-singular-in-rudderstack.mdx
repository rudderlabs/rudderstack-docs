...
<div class="infoBlock">
For more information on sending session events to Singular, refer to the <Link to="/destinations/analytics/singular/singular-cloud-mode/">Singular Cloud Mode</Link> guide.
</div>

- **Use device mode to send events**: When using the Android or iOS platforms, you can enable this setting to send events via the <Link to="/rudderstack-cloud/rudderstack-connection-modes/#device-mode">device mode</Link>. Then, follow the <Link to="/destinations/analytics/singular/singular-device-mode/">Singular Device Mode</Link> guide for steps on adding Singular to your project.

<img src="../../../assets/event-stream-destinations/singular-connection-settings-2.png" alt="Singular connection settings" />

<div class="successBlock">
When sending events via the device mode, RudderStack also lets you specify which events should be discarded or allowed to flow through. For more information, refer to the <Link to="/stream-sources/rudderstack-sdk-integration-guides/event-filtering/">Client-side Event Filtering</Link> guide.
</div>
