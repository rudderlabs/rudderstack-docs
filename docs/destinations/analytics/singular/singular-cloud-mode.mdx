...
| `context.device.adTrackingEnabled` | `dnt` | Required | Pass `true` if do not track is enabled, else pass `false`. This is automatically captured if you pass the advertising ID to the SDK. |
...
To anonymize your IP, you can send a dummy IP in the the <code class="inline-code">context.ip</code> field. RudderStack uses it as the IP address instead of capturing it automatically from the backend.
</div>
...
<ul>
  <li><Link to="/stream-sources/rudderstack-sdk-integration-guides/rudderstack-android-sdk/#setting-the-android-device-token">Android</Link></li>
  <li><Link to="stream-sources/rudderstack-sdk-integration-guides/rudderstack-ios-sdk/#setting-the-device-token">iOS</Link></li>
</ul>
</div>

<div class="infoBlock">
RudderStack supports only <code class="inline-code">fcm</code> for mapping two values to one key.
</div>
...
| `context.os.name` | `p` | Required | The source platform (Android or iOS). |
| `context.app.namespace` | `i` | Required | The package name (Android) or bundle ID (iOS) of your app. |
| `context.ip` / `request_ip` (in that order) | `ip` | Required | The user's IP address. |
