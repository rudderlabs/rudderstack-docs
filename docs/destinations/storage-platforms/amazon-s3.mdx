...
There are two ways to give RudderStack the relevant permissions for writing in your bucket. You can choose any one of them based on your internal security policies.
  ...
  "Statement": [
    {
      "Effect": "Allow",
      "Principal": {
        "AWS": "arn:aws:iam::422074288268:user/s3"
      },
...
By adding the above policy, the RudderStack user `arn:aws:iam::422074288268:user/s3` has the permission to write into your bucket.

<div class="infoBlock">
The above approach works <strong>only if</strong> you are using managed RudderStack
</div>
...
If you're using your S3 bucket as an **intermediary object storage** for the <Link to="/data-warehouse-integrations/">warehouse destination</Link>, then you will need the following bucket permissions:
...
- **AWS Secret Access Key**: Enter the AWS secret key key.
...
<div class="infoBlock">
For more information on the Amazon S3 encryption, refer to this <a href="https://docs.aws.amazon.com/AmazonS3/latest/dev/SSEUsingRESTAPI.html">Amazon S3 documentation</a>.
</div>
