---
title: "RudderStack IAM Role for AWS-based Destinations"
description: >-
...
5. Under **Options** check **Require external ID** and enter your <Link to="/resources/glossary/#workspace-token">workspace token</Link> as the **External ID**.
6. Optionally, you can choose to enable **Require MFA** based on your organization's security policies and requirements. Enabling this setting restricts the role only to the users who sign in with <a href="https://docs.aws.amazon.com/IAM/latest/UserGuide/id_credentials_mfa.html">multi-factor authentication (MFA)</a>.
...
8. Select your destination-specific permission policies applicable for the RudderStack IAM role. To create a new policy from scratch, click **Create policy**. For more information, refer to the <a href="https://docs.aws.amazon.com/IAM/latest/UserGuide/access_policies_create-console.html#access_policies_create-start">Creating IAM policies</a> guide.
9. **Optional**: You can also set a <a href="https://docs.aws.amazon.com/IAM/latest/UserGuide/access_policies_boundaries.html">permissions boundary</a>. Expand the **Set permissions boundary** section, choose **Use a permissions boundary to control the maximum role permissions**, and select the policy to use for the permissions boundary. An example is shown below:

<img src="../assets/destinations/aws-role-3.png" alt="Setting up AWS IAM Role for RudderStack" />

10. Review all the settings carefully and click **Next** to proceed.
11. Enter a unique name for your role. Note that this name cannot be distinguished by case. For example, you cannot create a role named `RUDDERSTACK` if `rudderstack` already exists.

<div class="warningBlock">
You cannot edit the name of the role after it has been created.
</div>

12. **Optional**: Enter the description for this role.
13. To edit the use cases and permissions for the role, click the **Edit** button next to the **Step 1: Select trusted entities** or **Step 2: Add permissions** sections.

<img src="../assets/destinations/aws-role-4.png" alt="Setting up AWS IAM Role for RudderStack" />

14. **Optional**: You can also add metadata to the role by attaching tags as key-value pairs. For more information, refer to the <a href="https://docs.aws.amazon.com/IAM/latest/UserGuide/id_tags.html">Tagging IAM resources</a> guide.
15. Finally, click **Create role** to complete the setup.
...
- <Link to="">Kinesis</Link>
- <Link to="">Kinesis Firehose</Link>
- <Link to="">Lambda</Link>
- <Link to="/destinations/streaming-destinations/aws-personalize/#connection-settings">Personalize</Link>