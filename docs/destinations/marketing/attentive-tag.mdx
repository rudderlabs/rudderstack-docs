...
- **API Key**: Your API Key is the unique key generated against your Attentive Tag account. It can be found in your Attentive Tag dashboard in the **Account** section under **Settings**.
...
The <code class="inline-code">signUpSourceId</code> provided in the <code class="inline-code">integrations</code> object is given the highest priority in the destination definition.
</div>
...
The `identifyOperation` should be passed as `unsubscribe` in the `integrations` to unsubscribe a user, as shown:
...
Currently, only <code class="inline-code">en-US</code> and <code class="inline-code">fr-CA</code> are supported as notification languages and these are case sensitive.
</div>
...
RudderStack maps the following properties to the Attentive Tag properties:
