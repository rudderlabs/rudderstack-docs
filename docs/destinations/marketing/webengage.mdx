...
| :-------| :--------| :------| :------|
| `userId` | `userIdOnly` | Required if `anonymousId` is not present. | `userId` cannot exceed 100 characters. |
...
| `dateOfBirth`/`dateofbirth`/`birthday`/`dob`/`DOB` | `birthDate` | Optional | User's birth date in the ISO format: `yyyy-MM ddTHH:mm:ss±hhmm`. |
...
| `company` | `company` | Optional | The user's company. |
...
| `postalcode` / `address.postalCode` | `gender` | Optional | The user's postal code. |
| `attributes` | `email` | Optional | The user's custom attributes as key-value pairs. |

### Identify traits considerations

Note the following when sending `identify` traits to WebEngage:

- Either `userId` or `anonymousId` must be present in the `identify` call. If both these fields are present, RudderStack ignores `anonymousId` and sends `userId` to WebEngage.
- WebEngage prioritizes `hashedEmail` over `email`. Hence, you once you set `hashedEmail`, it can then be updated only through a new `hashedEmail`.
- WebEngage prioritizes `hashedPhone` over `phone`. Hence, you once you set `hashedPhone`, it can then be updated only through a new `hashedPhone`.
...
Either <code class="inline-code">userId</code> or <code class="inline-code">anonymousId</code> must be present in the <code class="inline-code">track</code> call. In case both are present, RudderStack will ignore <code class="inline-code">anonymousId</code> and send <code class="inline-code">userId</code> to WebEngage.
</div>
...
| `properties` | `attributes` | Optional | The custom event attributes as key-value pairs. |

### Setting custom attributes

WebEngage lets you send custom attributes as key-value pairs in the `attributes` parameter as described in the <Link to="#property-mapping">above section</Link>. However, there are a few things you should keep in mind:
...
RudderStack internally transforms the `page` call into a `track` call before sending it as a custom event to WebEngage. It sends the event/category name by transforming it in the format `Viewed ${event} ${name} ${category} ${page}`.

For example, consider the following `page` call:

```javascript
rudderanalytics.page("Cart Viewed", {
  path: "/best-seller/1",
...
RudderStack internally transforms the `screen` call into a `track` call before sending it as a custom event to WebEngage. It sends the event/category name by transforming it in the format `Viewed ${event} ${name} ${category} ${screen}`.
...
To get your WebEngage public API key, follow these steps:
