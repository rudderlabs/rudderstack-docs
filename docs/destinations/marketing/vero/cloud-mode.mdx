...
RudderStack sends the `identify` events to Vero via their [`users`](https://developers.getvero.com/?bash#users) API to create, update, and manage your user records and their subscription status.
...
RudderStack sends the <Link to="/rudderstack-api/api-specification/rudderstack-spec/alias/">`alias`</Link> events to Vero via their [`users/reidentify`](https://developers.getvero.com/?bash#users-alias) endpoint to change a user's identifier (`id`).
