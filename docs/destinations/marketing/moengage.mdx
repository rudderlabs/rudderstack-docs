...
```
      ...
      <TabPanel>
        ...
        Follow these steps to add MoEngage to your iOS project:
<ol>
<li>Add MoEngage as a destination in the <a href="https://app.rudderstack.com">RudderStack dashboard</a>.</li>
...
<Tabs>
  <TabList>
    ...
    <TabPanels>
      ...
      <TabPanel>
        <ol>
          <li>In your <code class="inline-code">plist</code> file, add the <code class="inline-code">MoEngageAppDelegateProxyEnabled</code> key and set it <code class="inline-code">false</code> to disable swizzling. </li>
          <li>Then, generate the <code class="inline-code">.pem</code> file and upload it to MoEngage.</li>
          <li>Finally, implement the relevant methods in your <code class="inline-code">AppDelegate</code> file by referring to <Link to="/stream-sources/rudderstack-sdk-integration-guides/rudderstack-ios-sdk/ios-v2/#supporting-push-notifications-for-device-mode-destinations">this section</Link>.</li>
    </ol>
      </TabPanel>
      <TabPanel>
...
Before configuring MoEngage SDK for receiving push notifications please make you have configured MoEngage in your application, if not done already refer to the <a href="https://firebase.google.com/docs/android/setup">documentation</a> and complete setup.
Also, make sure you have added the MoEngage Messaging dependency in your application <code class="inline-code">build.gradle</code> file.
...
<li> Receiving the Push payload from MoEngage Cloud Messaging(FCM) service and showing the notification on the device. The above can either be handled by the application or MoEngage SDK. There is some configuration required based on whether the above-mentioned things are handled by the application or SDK.
</li>
</ol>
...
To pass the push payload to the MoEngage SDK call the MoEngage API from the `onMessageReceived()` from the MoEngage receiver.
...
An application can get this callback by implementing <code class="inline-code">MoEngageEventListener</code> and registering for a callback in the Application class' <code class="inline-code">onCreate()</code>using <code class="inline-code">MoEFireBaseHelper.getInstance().setEventListener()</code>
<h3>In-App messaging:</h3>
