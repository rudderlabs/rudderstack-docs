...
Yes, you can. The `rudder_discards` table contains columns like `message_id`, `table_name`, `column`, and `value`. You can use the `table_name` and map `message_id`  to the `id` column in your event’s table.

<div class="infoBlock">
The <code class="inline-code">rudder_discards</code> table is applicable for all the warehouse destinations except <Link to="/destinations/warehouse-destinations/s3-datalake/">Amazon S3 Data Lake</Link>.
</div>
