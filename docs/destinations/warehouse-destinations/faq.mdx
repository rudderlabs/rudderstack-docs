...
### How can I add the table prefixes if there are multiple sources connected to the same destination?
...
```javascript
if (SOURCE_ID == '<your_source_ID>') {
  event.event = '<some_prefix>' + event.event;
}
```
