...
- If you're syncing semi-structured event data that is not defined by a fixed schema, you can leverage RudderStack's <Link to="/destinations/warehouse-destinations/json-column-support/">JSON Column Support</Link> feature to send the data to your warehouse without worrying about the column count limit.
