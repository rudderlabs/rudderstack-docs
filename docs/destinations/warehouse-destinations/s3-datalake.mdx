...
<div class="warningBlock">
If this setting is disabled, your S3 policy needs to include certain permissions. Refer to the <Link to="#s3-permissions-for-data-lake-destination">S3 permissions for the data lake destination</Link> section below for more information.
</div>
...
If you have disabled the **Register schema on AWS Glue** setting in the <Link to="#connection-settings">destination settings</Link>, then you need to set the following permissions in your S3 policy to successfully send data to your S3 data lake:
