...
RudderStack lets you connect to your PostgreSQL database securely by using an SSH tunnel connection. Enter the following details to do the same:

- **SSH Connection**: Enable this setting to use the SSH tunnel connection while connecting to your PostgreSQL database.
- **SSH Host**: Enter the IP address of your SSH bastion host.
- **SSH Port**: Enter the SSH port for the above host.
- **SSH User**: Enter the user name of your PostgreSQL database.
...
You can also enable the SSH tunnel connection for an existing PostgreSQL destination by navigating to the destination's **Configuration** tab, selecting **Edit configuration** and enabling the **SSH connection** setting, as shown:

<img src="../../assets/warehouse-destinations/postgresql-edit-configuration.png" alt="Redshift edit configuration" />
