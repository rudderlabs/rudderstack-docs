...
#### Skip creation of `tracks` table

You can choose to skip the creation of `tracks` table by setting the `skipTracksTable` option to `true`. The following sample snippet shows how to skip creating it for the Delta Lake destination:
...
Similarly, you can use the following names for the desired warehouse destination in the above snippet:

| Warehouse destination |  Format |
| :------------------- | :---------- |
| PostgreSQL | POSTGRES |
| Snowflake  | SNOWFLAKE |
| Google BigQuery   | BQ |
...
The following integration options are supported for Warehouse destinations:

| Integration option | Description |
| :------------ | :---------- |
| `skipTracksTable`     | Skips the creation of `tracks` table. <Link to="#skip-creation-of-tracks-table">Reference</Link>.         |
| `useBlendoCasing`     | Retains the Blendo column names when <Link to="/user-guides/migration-guides/blendo-rudderstack-migration/#retain-column-names">migrating from Blendo to RudderStack</Link>.         |
| `jsonPaths`     | Declares given path columns as json fields in warehouse. <Link to="/destinations/warehouse-destinations/json-column-support/">Reference</Link>.       |
