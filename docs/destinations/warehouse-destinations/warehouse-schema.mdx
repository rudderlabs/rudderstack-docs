...
You can choose to skip the creation of `tracks` table by setting the `skipTracksTable` option to `true`. The following sample snippet shows how to skip the `tracks` table for Delta Lake destination:
...
Similarly, you can use the following names for the required warehouse destination in the above snippet:
...
The following integration options supported for Warehouse destinations:
