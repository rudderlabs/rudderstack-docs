...
<div class="warningBlock">
RudderStack drops the non-standard properties declared at the top level of an event.
</div>
...
RudderStack drops the non-standard properties (any property apart from the <Link to="#standard-rudderstack-properties">standard properties</Link>) declared at the top level of an event. However, it allows you to add such properties in the `context` or `properties` section of the event payload.
