...
Find the open source code for this destination in the <a href="https://github.com/rudderlabs/rudder-transformer/tree/master/src/v0/destinations/rs">GitHub repository</a>.
</div>
...
2. Click the **Create Cluster** option, as shown:
...
1. Click the **Editor** option in the left pane. You can run the queries to create a new user to access the Redshift cluster in the **Query editor**, as shown:
...
RudderStack lets you connect to your Redshift database securely by using an SSH tunnel connection. Enter the following details to do the same:

- **SSH Connection**: Enable this setting to use the SSH tunnel connection while connecting to your Redshift database.
- **SSH Host**: Enter the IP address of your SSH bastion host.
- **SSH Port**: Enter the SSH port for the above host.
- **SSH User**: Enter the user name of your Redshift database.
- **SSH Public Key**: Copy the public key provided in this field and add it to the `authorizated_keys` file on your SSH bastion host. Rudderstack will use the private key corresponding to this public key to establish the SSH connection successully.

You can also enable the SSH tunnel connection for an existing Amazon Redshift destination by navigating to the destination's **Configuration** tab, selecting **Edit configuration** and enabling the **SSH connection** setting, as shown:

<img src="../../assets/warehouse-destinations/redshift-edit-configuration.png" alt="Redshift edit configuration" />
...
- **Staging S3 Storage Bucket Name**: If **Use RudderStack-managed object storage** is disabled in the dashboard, enter the relevant <Link to="/destinations/streaming-destinations/amazon-s3#connection-settings">Amazon S3 bucket storage settings</Link>.
