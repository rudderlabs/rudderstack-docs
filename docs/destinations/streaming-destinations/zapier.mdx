---
title: "Zapier"
description: Step-by-step guide on sending your event data from RudderStack to Zapier.
...
Also, as mentioned in the <Link to="#connection-settings">Connection settings</Link> above:

- If you set a mapping in the **Mapping Track Events to a particular Zap** dashboard setting, RudderStack sends the `track` events with the specified **Event Name** to the corresponding **Zap URL**.
- If you set a mapping in the **Mapping Page/Screen Events to a particular Zap** dashboard setting, RudderStack sends the `page`/`screen` events with the specified **Event Name** to the corresponding **Zap URL**.

## FAQ

### Where can I find the Zap URL?

To get your Zapier URL, you need to first create a Zap by following these steps:
...
4. In the **Test Trigger** section, you will find your Zap URL or the webhook URL where you can send all the requests. Copy this URL as it is required to set up the Zapier destination in RudderStack.
