...
- Use the <a href="https://docs.developers.optimizely.com/experimentation/v3.1.0-full-stack/reference/activate">`activate`</a> API first and then send your `track` events. If you do not activate the user, RudderStack will send the `track` events to Optimizely and they will not be attributed to the user (visible on the **Results** page).
...
<div class="infoBlock">
All the mappings supported for the <Link to="#supported-mappings">`track`</Link> event are applicable for `page` event as well.
</div>
...
<div class="infoBlock">
All the mappings supported for the <Link to="#supported-mappings">`track`</Link> event are applicable for `screen` event as well.
</div>
