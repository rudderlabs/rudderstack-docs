...
- **Fall back to anonymousId if userId is not present**: If this setting is enabled, RudderStack will use `anonymousId` to identify the user, in case `userId` is not present in the event.

## Identify

You can use the <Link to="/event-spec/standard-events/identify/">`identify`</Link> call to create or update a user in Custify.  RudderStack uses the `userId` or `email` fields in the event's `traits` to do so. Hence, it is highly recommended to include the `userId` or at least their `email` in the `identify` call.
...
RudderStack maps the event properties to a subset of the standard Custify user attributes. It also creates a new custom field for the unmapped attributes. The mapping is shown in the following table:
...
The above `identify` call updates the person as well as removes them from the company having the `id` as `company_123`.
...
- If `userId` and `groupId` are present, RudderStack associates the Custify user with the organization and sends all the user-specific information.
