...
[Google Ads Offline Conversions](https://support.google.com/google-ads/answer/2998031?hl=en&ref_topic=7280668) lets you measure your sale conversions in the offline mode after a user clicks your ad online but does not directly proceed to sale in the online mode. In other words, it helps you track the offline sales which occur when your ads lead to a click or call to your business.
...
| `email` | `hashedEmail` | If **User Identifier Source** is set in the dashboard, either `email` or `phone` must be passed. |
| `phone` | `hashedPhoneNumber` | If **User Identifier Source** is set in the dashboard, either `email` or `phone` must be passed. |
