...
If the **Record Live Chat Events** dashboard setting is enabled during the destination setup, RudderStack records the SnapEngage Live Chat events based on the user's interactions. You can then send them as <Link to="/event-spec/standard-events/track/">track</Link> events to any other tools (connected to the source in RudderStack).
