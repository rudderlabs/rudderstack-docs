...
If you enable the **Record Live Chat Events** dashboard setting, RudderStack records the SnapEngage Live Chat events based on the user's interactions and sends them as <Link to="/event-spec/standard-events/track/">track</Link> events. You can then view and analyze these events in the other tools (connected to the same source in RudderStack).
