...
- **Group Key Settings**: RudderStack sends the `group` calls to Mixpanel only if one or more group keys are specified here. These group keys act as group identifiers in Mixpanel. For more information, refer to the <Link to="#group-key">Group Key</Link> section below.
