...
- **Group Key Settings**: RudderStack sends the `group` calls to Mixpanel only if one or more group keys are specified here. These group keys are used as group identifiers in Mixpanel. For more information, refer to the <Link to="#group-key">Group Key</Link> section below.
...
You can create group keys in [Mixpanel project settings](https://help.mixpanel.com/hc/en-us/articles/360025333632-Group-Analytics#implementation), which act as group identifiers in Mixpanel. To send a `group` call to Mixpanel successfully, you must specify **at least** one group key (must be present in Mixpanel) in the **Group Key** RudderStack dashboard setting, as shown:
