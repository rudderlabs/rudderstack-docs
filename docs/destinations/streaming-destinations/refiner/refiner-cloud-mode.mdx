...
To make an <code class="inline-code">identify</code> call successfully, either of <code class="inline-code">email</code> or <code class="inline-code">userId</code> is required.
</div>
...
| `properties.email` <br/> <span style="color: #4D4DFF;font-size:12px;">Required, userId is absent.</span> | `email` |
| `traits` + `context.traits` | traits |

## Track

The <Link to="/event-spec/standard-events/track">`track`</Link> call lets you capture user events and send them to Refiner as `trackEvent` events. If a `track` event contains a `userId`, then Refiner updates the corresponding user information. Otherwise, it creates a new user.
