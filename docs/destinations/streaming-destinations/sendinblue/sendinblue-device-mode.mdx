...
- The contact attributes present in Sendinblue and sent in the `identify` call's traits are updated each time you make an `identify` call. 
- The contact attributes **not** present in Sendinblue but sent in the `identify` call's traits can be used only in the [Automation app](https://automation.sendinblue.com/) unless you [add them as contact attributes](https://my.sendinblue.com/lists/add-attributes).
...
To create a new contact, you must clear the <code class="inline-code">sib_cuid</code> cookie first.
</div>
