...
<div class="infoBlock">
Note that Google Analytics 4 does not officially support a complete server-to-server integration. RudderStack monitors the GA4 Measurement Protocol API capabilities and accordingly sends a reasonable level of reporting data for the customers to GA4.
</div>
...
| `non_personalized_ads`   |  `context.device.adTrackingEnabled` | Indicates whether the events should be used for personalized ads.|
...
<div class="infoBlock">
The mapping of the <strong>Payment Info Entered</strong> event to the <code class="inline-code">add_payment_info</code> or <code class="inline-code">add_shipping_info</code> event is determined on the basis of parameters passed to it.
</div>
...
While the events typically show up within a few seconds, note that it can take GA4 up to 24 hours to process their data, and not all types of data flow through to its real-time dashboard. It may be best to consult GA4’s FAQs for the most up-to-date information on its latest capabilities and limitations.
