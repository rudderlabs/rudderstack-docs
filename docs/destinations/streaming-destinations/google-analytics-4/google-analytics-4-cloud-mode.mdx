...
RudderStack maps the `client_id` in the following order of precedence:

- If a value is provided in the RudderStack dashboard's **GA4 client_id Field Identifier** setting, it looks up that value in the event payload and maps it to the GA4 `client_id`.
- If no value is provided in the above dashboard setting or a value is provided in the dashboard but not in the payload, RudderStack maps the `client_id` from the following fields in the same priority order as listed below:
  - From `externalID` `ga4ClientId`
  - From `externalID` `anonymousId`
