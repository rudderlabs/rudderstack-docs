...
RudderStack lets you send your event data to Google Analytics 4 destination via device mode using the native web SDK. It supports the `gtag` way of tagging in websites in the device mode. 
...
RudderStack supports ecommerce tracking for Google Analytics 4. Use the <Link to="/event-spec/ecommerce-events-spec/">Ecommerce Events Specification</Link> guide for sending events while instrumenting your site with the RudderStack SDK.
