...
- **Enable debug mode**: Enable this setting to monitor events in the debug view:
  * For device mode, it utilizes the `debug_mode` parameter in the GA4 script. Refer <Link to="https://support.google.com/analytics/answer/7201382?hl=en#zippy=%2Cglobal-site-tag-websites">Google documentation</Link> for more information.
  * For cloud mode, it is supported via the validation server endpoint. Refer <Link to="https://developers.google.com/analytics/devguides/collection/protocol/ga4/validating-events?client_type=gtag">Google documentation</Link> for more information.
