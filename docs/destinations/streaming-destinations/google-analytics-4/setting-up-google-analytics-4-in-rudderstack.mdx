...
- **Add send_to parameter in gtag event call**: Enable this setting to send an event **only** to the specific Google Analytics 4 property whose measurement ID you have mentioned in the connection settings above. When enabled, a sample event to Google Analytics 4 looks like below:

```javascript
gtag('event', 'view_video', { 'send_to': '<property_id>' }); // where <property_id> is the measurement id of the data stream for GA4.
```
