...
<div class="infoBlock">
For more information on setting the trigger type for your cloud function, refer to the <Link to="#faq">FAQ</Link> section below.
</div>
...
<div class="infoBlock">
The API key ID is a string required by the cloud function to identiy a request. RudderStack injects this key in the <code class="inline-code">ApiKey</code> header in the case of both <strong>HTTP</strong> and <strong>HTTPS</strong> trigger types.
</div>

- **Enable Batch Input**: Enable this setting to pass a batch input (array of events) to your cloud function.
- **maxBatchSize**: If **Enable Batch Input** setting is enabled, specify the maximum batch size in this field.
