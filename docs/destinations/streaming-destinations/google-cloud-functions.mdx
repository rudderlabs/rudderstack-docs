...
<div class="infoBlock">
The API key ID is a string required by the cloud function to identiy a request. RudderStack injects this key in the <code class="inline-code">Authorization</code> header in the case of <strong>HTTP</strong> trigger type and the <code class="inline-code">ApiKey</code> header in the case of <strong>HTTPS</strong> trigger type.
</div>
...
4. Enter a name for your function and allocate the memory for the function.
5. In the **Trigger** field, select **HTTP** or **HTTPS**, depending on your use-case. 
6. Save the resulting URL - this is required to set up Google Cloud Functions as a destination in RudderStack.
7. Configure the function's code in the **Source Code** field and the language in the **Runtime** field.
8. Enter the function name as defined in your code in the **Function to execute** field.
9. Click **Create** to save your settings and create the Cloud Function.