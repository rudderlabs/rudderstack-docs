...
Before configuring your source and destination in RudderStack, please check whether the platform you are sending the events from is supported by CleverTap. Please refer the following table to do so:
...
The <Link to="/event-spec/standard-events/alias/">`alias`</Link> call call lets you merge different identities of a known user.
