...
When you track an event with the name `Order Completed` using the using the RudderStack <Link to="/event-spec/ecommerce-events-spec/">E-commerce Events</Link> tracking, RudderStack maps that event to CleverTap’s [Charged](https://developer.clevertap.com/docs/events#recording-customer-purchases) event.
