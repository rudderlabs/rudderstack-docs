...
- **Use device mode to send events**: Enable this setting to send events to Adjust via the <Link to="/destinations/rudderstack-connection-modes/#device-mode">device mode</Link>.
...
<div class="infoBlock">
The partner parameters only accept data of type String.
</div>
...
Adjust's native SDK handles install attribution out of the box with RudderStack.
...
## Adjust environment and logging

RudderStack sends data to the Adjust environment depending on the `RudderLogLevel` set in the SDK.
...
`RudderLogLevel.DEBUG` or higher will set the Adjust environment to **Sandbox** and `logLevel` to `VERBOSE`. For anything below that, RudderStack sets the Adjust environment to **Production** and `logLevel` to `ERROR`.
