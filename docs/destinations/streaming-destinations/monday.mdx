...
The column names provided in the mapping should be existing in your board. Only the column names provided here will be updated.
</div>
...
You can use the <Link to="/event-spec/standard-events/track">`track`</Link> call to create an item in your board. The **Board ID** and **column to property mapping** in the RudderStack dashboard are also required for the same.

To add an item to a specific group, you can provide the group title.
...
| Country | Requires country code and country name both.  |
...
| :-----| :------| :-------| :-------|
| `properties.name` | `item_name` | Required | Attributes passed here are mapped to the first column value by default. |
...
| `properties.emailText` | `text`  | Optional | Can be used to add hyperlink to the email address text on the board.|
| `properties.linkText` | `text`  | Optional | Can be used to add hyperlink to the text on the board.|
...
Follow these steps to find these:
