...
- **Container name**: Enter the name of your Yandex.Metrica ecommerce data container that you set while <a href="https://yandex.ru/support/metrica/general/creating-counter.html">creating a new counter</a>. If not specified, RudderStack sets it to `dataLayer` by default.
...
You can use the <Link to="/event-spec/standard-events/track/">`track`</Link> call to send your <Link to="/event-spec/ecommerce-events-spec/">ecommerce events</Link> to Yandex.Metrica.
