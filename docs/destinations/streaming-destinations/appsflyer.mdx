...
- **Client-side Event Filtering**: This setting is applicable only if you are sending events to AppsFlyer via device mode. 
...
- **Use native SDK to send events**: By default, this setting is enabled. To send events via cloud mode or device mode, you can toggle this setting on or off.
