...
In the above example, the contact with the identifier `1hKOmRA4GRlmr123` is added to the mailing list with the ID `clxf1nxlb000t0ml79ajwcsj0`.
...
Your `track` events will appear on the [Settings > Events](https://app.loops.so/settings?page=events) page in the Loops dashboard.
...
- If you omit both the `email` and `userId` fields from your `track` calls, you must identify the current user first by calling `identify`.
- The `track` event name should match the name of the event in Loops. Also, the event properties must match the properties in Loops. You can set these up by going to [Settings > Events](https://app.loops.so/settings?page=events) in your Loops dashboard.
- You can include [event properties](https://loops.so/docs/events/properties) in the `track` event so you can use them inside your loop emails.
- RudderStack persists any values in `context.traits` from a previous `identify` call in the `track` event properties and updates the contact record in Loops accordingly.
