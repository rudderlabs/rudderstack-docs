...
- **User deletion**: Use this setting to add and authenticate an account which will be used to delete users in Google Analytics. Refer to the <Link to="/destinations/streaming-destinations/google-analytics-ga/#deleting-a-user">Deleting a user</Link> section for more information.
...
2. Select **Sign in with Google** from the modal.
3. Choose the required account and grant RudderStack the required permissions. 

<div class="infoBlock">
If your account authentication fails, you can refer to these steps for <a href="https://developers.google.com/analytics/devguides/config/userdeletion/v3/authorization#troubleshooting">Troubleshooting</a> the root cause.
</div>
...
RudderStack will <strong>throw an error</strong> if you try to delete an account which is being used by any other connection set up for the same destination.
...
<div class="infoBlock">
Refer to the <a href="https://developers.google.com/analytics/devguides/config/userdeletion/v3/limits-quotas#user_deletion_api">Limits and quotas</a> documentation for more information about the specified quotas for user deletion API requests.
</div>
