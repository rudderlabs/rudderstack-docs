...
- **Disable Md5 encryption from Client ID**: This setting determines how RudderStack should map the `client id`. For more information, refer to the <Link to="#how-rudderstack-takes-the-clientid">How RudderStack takes the `clientId`</Link> section.  
...
- **Send user-id to GA**: Enable this setting to set a user ID for identified visitors before sending it to Google Analytics.
...
The `clientId` (cid) is used to keep a track of the unique visitors to your website. The Google Analytics universal cookie looks like the following:
...
In Google Analytics, you can configure the [custom dimensions](https://support.google.com/analytics/answer/10075209?hl=en) in the [Admin](https://support.google.com/analytics/answer/6132368?hl=en) page.
