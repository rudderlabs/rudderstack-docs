...
- **Enable Enhanced Link Attribution**: Enable this setting to retrieve the detailed reports of links clicked on your website. Refer to the [Enhanced Link Attribution](https://support.google.com/analytics/answer/2444872?hl=en#zippy=%2Cin-this-article) guide for more information. 
...
- **Custom Dimensions & Metrics**: Enter the event name, traits or properties you want to map to the corresponding custom dimensions and metrics. For more information on this setting, refer to the <Link to="#custom-dimensions">Custom dimensions</Link> section. 
- **Property & Trait Mapping**: Enter the property you want to map to the Google Analytics content grouping. For more information on this feature, refer to the <Link to="#content-groupings">Content groupings</Link> section. 
- **Enable Server Side Identify**: Enable this setting to trigger the `identify` calls from the server-side.
- **Server Side Identify Event Category**: Specify the event category of the `identify` call triggered from the server-side. Any specified category is sent as `ec` or the default value is set to `All`.
- **Server Side Identify Event Action**: Specify the event action of the `identify` call triggered from the server-side. Any specified action is sent as `ea` or the default value is set to `User Enriched`.
- **Disable Md5 encryption from Client ID**: This setting defines how RudderStack should map the `client id`. For more information on this setting, refer to the <Link to="#how-rudderstack-takes-the-clientid">How RudderStack takes the `clientId`</Link> section.  
...
- **Enable Enhanced Ecommerce**: Enable this setting to derive the detailed insights for e-commerce events. Refer to the <Link to="#enhanced-e-commerce">Enhanced e-commerce</Link> section for more information. 
...
- **Send user-id to GA**: Enable this setting to set a user ID for identified visitors and send it to Google Analytics.
...
- **Set Custom Dimensions & Metrics to the Page**: This setting is enabled by default and passes the custom dimensions and metrics as properties for all the events on a given page. When disabled, RudderStack only passes the custom dimensions and metrics as part of the event payload with which they are explicitly associated. For more information on this feature, refer to the <Link to="#custom-dimensions">Custom dimensions</Link> section. 
- **Use Named Tracker**: Enable this setting to push events to the `rudderGATracker` Google Analytics tracker instead of the default tracker. For more information on this feature, refer to the <Link to="##named-tracker">Named tracker</Link> section. 
...
RudderStack supports basic e-commerce tracking for Google Analytics. The required steps are:
...
Enhanced e-commerce setting must be enabled in RudderStack dashboard, Google Analytics, and should follow RudderStack's <Link to="/event-spec/ecommerce-events-spec/">E-commerce Events Specification</Link>.
...
The `clientId` (cid) is used to keep a track of the unique visitors to your website.

The Google Analytics universal cookie looks like the following:
...
RudderStack takes the `clientId` in the following priority order:
...
<div class="infoBlock">
If the <strong>Disable MD5 encryption from Client ID</strong> setting is disabled in the dashboard and the <code class="inline-code">integrations</code> object, <code class="inline-code">externalId</code>, <code class="inline-code">traits.cid</code>, and <code class="inline-code">anonymousId</code> are absent, RudderStack passes the <code class="inline-code">userId</code> as the <code class="inline-code">clientId</code> in the MD5-hashed format.
...
In Google Analytics, you can configure [custom dimensions](https://support.google.com/analytics/answer/10075209?hl=en) in the [Admin](https://support.google.com/analytics/answer/6132368?hl=en) page.

There are multiple scopes for custom dimensions, such as `hits`, `session`, `user`, and `products` (if **Enhanced e-commerce** setting is enabled). Once these are set up in Google Analytics, the traits and properties can be mapped to custom dimensions in the RudderStack dashboard.

A custom dimension should be set as `dimension{index}` . For example, `dimension1` is mapped accordingly when the user sets the dimension in GA settings as shown below. Similarly, you can set up different metrics.
...
You can set the [content grouping](https://support.google.com/analytics/answer/2853423?hl=en) mappings in the RudderStack dashboard and Google Analytics, as shown in the images below:
