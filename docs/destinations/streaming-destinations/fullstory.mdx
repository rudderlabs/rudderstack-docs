...
RudderStack supports sending event data to FullStory via the following <a href="https://rudderstack.com/docs/rudderstack-cloud/rudderstack-connection-modes/">connection modes</a>:
...
<div class="infoBlock">
In a web device mode integration, that is, using <Link to="/sources/event-streams/sdks/rudderstack-javascript-sdk">JavaScript SDK</Link> as a source, the FullStory native SDK is loaded from <code class="inline-code">https://edge.fullstory.com</code> domain. Based on your website's content security policy, you might need to <Link to="/sources/event-streams/sdks/rudderstack-javascript-sdk/load-js-sdk/#allowlist-destination-domain">allowlist this domain</Link> to load the FullStory SDK successfully.
</div>
...
- Assign a name to your destination and click **Next**. You should be able to see the following screen:
...
* Right-click the FullStory row, and choose <b>Add Row</b>. Set the key name to `OrgId`, and the type to `String`. For the value, paste your assigned organization ID.
...
In web device mode, if the <code class="inline-code">userId</code> is not explicitly provided, then the <code class="inline-code">anonymousId</code> of the user is sent as the <code class="inline-code">uid</code> instead.
</div>
