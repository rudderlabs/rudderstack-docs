...
- **Hub ID**: Enter your HubSpot Hub ID. For more information, refer to the [Hub ID](https://knowledge.hubspot.com/account/manage-multiple-hubspot-accounts#identify-the-current-account-s-hub-id) documentation.

<div class="warningBlock">
If you are using the HubSpot destination with a <Link to="/sources/reverse-etl/">Reverse ETL</Link> source, this field is not required and can be set up as N/A.
</div>
...
-  **API Key**: Enter your HubSpot API key. For more information, refer to the [ API Key](https://knowledge.hubspot.com/integrations/how-do-i-get-my-hubspot-api-key) documentation.
...
Other additional settings for the **new API** configuration are as follows:

<img src="../../../assets/event-stream-destinations/hubspotv2-connection-settings-3.png" alt="HubSpot connection settings" />

- **HubSpot property name to be used as lookup field**: Enter a unique property to be used as a lookup field for <Link to="https://developers.hubspot.com/docs/api/crm/contacts">HubSpot contacts</Link>. The lookup field entered here **must also** be passed in the `identify` call as it's value will be used to look up the contact. Refer to the <Link to="/destinations/streaming-destinations/hubspot/hubspot-cloud-mode/hubspot-new-api-cloud#identify">`identify` call</Link> for more information.
- **Create associations between object records**: Enable this setting to create associations between the object records while using a Reverse ETL source. Refer to <Link to="/destinations/streaming-destinations/hubspot/connect-reverse-etl-source/#creating-association-between-object-records">Creating associations between object records</Link> for more information.
