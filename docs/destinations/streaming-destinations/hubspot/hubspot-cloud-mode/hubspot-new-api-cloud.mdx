...
The lookup field passed in the **HubSpot property name to be used as lookup field** dashboard setting must also be passed in the `identify` call's `traits`. For example, if you have passed `sampleField` in the dashboard, you must pass it and its corresponding value in the `identify` call, as shown:
...
```
Using this lookup field (`sampleField` in above example), you can change the lookup value (`phone` in above example) anytime just by passing it in the `identify` call without editing your dashboard setting.
