...
You can create association between the HubSpot object records while connecting your HubSpot destination to a <Link to="/sources/reverse-etl/">Reverse ETL</Link> source using the <Link to="/sources/reverse-etl/features/visual-data-mapper/">Visual Data Mapper</Link> feature.

### Prerequisites

The following pre-requisites must be met to create association between the HubSpot object records:

- You must create two separate connections in the RudderStack dashboard using a Reverse ETL source and HubSpot destination as shown:
    <img src="../../../assets/event-stream-destinations/hubspot-connections.png" alt="HubSpot connection settings"/>

    - Here, the first connection will push data into the HubSpot object. For example, pushing data for a new company. 
    - The second connection will create association between the object records. You must configure the HubSpot destination in the second connection using the <Link to="/destinations/streaming-destinations/hubspot/setting-up-hubspot-in-rudderstack/#new-api">new API</Link> and enabling the **Create associations between object record** setting in the RudderStack dashboard. The detailed steps to create this connection are explained in the below section.

### Detailed steps 

Follow the below steps to create association between the HubSpot object records:
