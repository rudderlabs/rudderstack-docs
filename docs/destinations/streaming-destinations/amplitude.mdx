...
<div class="infoBlock">
<strong>Applicable for mobile only</strong>: If a trait has a name <code class="inline-code">optOutOfSession</code> with a value of <code class="inline-code">true</code>, then this <code class="inline-code">identify</code> call will be opted out of the current session if it exists or won't start a new session if there isn't any active session.<br/><br/>

<strong>Opting out of sessions</strong> 
...
RudderStack does not support associating a user to more than one group per `group` call sent to Amplitude. To send more than one group per user, you must call the `group` API multiple times with the relevant group information specified in the <Link to="#identify--group-settings">group settings</Link>.
...
RudderStack supports sending `event_id` to Amplitude in device mode. You can include it under the `integrations` object and it is supported for all the above-mentioned API calls, namely, `identify`, `track`, `page`, `screen`, and `group`.
...
You can delete a user in Amplitude using <Link to="/api/data-regulation-api/#adding-a-suppression-with-delete-regulation">Suppression with Delete regulation</Link> in the RudderStack <Link to="/api/data-regulation-api/">Data Regulation API</Link>.
...
RudderStack does not support associating a user to more than one group per `group` API call sent to Amplitude. To send more than one group per user, you need to call the `group` API multiple times with the relevant group information specified in the <Link to="#identify--group-settings">group settings</Link>.
