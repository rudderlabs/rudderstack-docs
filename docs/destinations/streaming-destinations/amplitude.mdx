...
For more information on explicitly setting the <code class="inline-code">anonymousId</code>, refer to <Link to="/sources/sdks/rudderstack-javascript-sdk/supported-api/#overriding-anonymous-id">RudderStack's JavaScript SDK</Link> documentation.
</div>
...
Refer to the <Link to="/sources/sdks/rudderstack-javascript-sdk/supported-api/#alias">JavaScript SDK documentation</Link> for information and examples on how to call the `alias` event.
