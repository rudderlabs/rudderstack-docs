...
- **Mapping to trigger the TikTok Ads standard events for the respective Events**: Enter the event name and pick a corresponding <Link to="https://ads.tiktok.com/help/article?aid=10028">TikTok Ads standard event</Link> from the dropdown to be triggered when that event is called.
