...
For Algolia Insights, RudderStack supports the `products` array for two ecommerce events - <Link to="/event-spec/ecommerce-events-spec/browsing/#product-list-viewed">Product List Viewed</Link> and <Link to="/event-spec/ecommerce-events-spec/ordering/#order-completed">Order Completed</Link>.
