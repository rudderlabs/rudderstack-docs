...
In order to send ecommerce events that have a product array as one of their <code class="inline-code">track</code> properties, RudderStack sends each product property to the <code class="inline-code">/s</code> endpoint from cloud mode, or pushes it as <code class="inline-code">['set', &#123;id:, sku: ...&#125;]</code> from the native SDK.
</div>
