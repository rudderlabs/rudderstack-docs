...
### Should I send the `email` field after hashing it?

You can provide the `email` field in a plain text format, for example, `alex@example.com`. It is SHA1 hashed (with a HEX output type) automatically before being sent to impact.com.
