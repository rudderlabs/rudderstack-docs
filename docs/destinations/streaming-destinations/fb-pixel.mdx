...
Here, the value of [`fbc`](https://developers.facebook.com/docs/marketing-api/conversions-api/parameters/fbp-and-fbc/#fbc) parameter can be sent via `context.fbc` as shown in the above payload. If not sent, Rudderstack auto generates it using the following values:
