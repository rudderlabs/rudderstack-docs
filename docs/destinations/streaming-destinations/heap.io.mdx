---
title: "Heap.io"
description: Step-by-step guide on sending your event data from RudderStack to Heap.io.
---
...
RudderStack supports sending event data to Heap via the following <a href="https://rudderstack.com/docs/rudderstack-cloud/rudderstack-connection-modes/">connection modes</a>:
...
<div class="infoBlock">
In the web device mode integration, that is, using <Link to="/sources/event-streams/sdks/rudderstack-javascript-sdk">JavaScript SDK</Link> as a source, the Heap native SDK is loaded from <code class="inline-code">https://cdn.heapanalytics.com/</code> domain. Based on your website's content security policy, you might need to <Link to="/sources/event-streams/sdks/rudderstack-javascript-sdk/load-js-sdk/#allowlist-destination-domain">allowlist this domain</Link> to load the Heap SDK successfully.
</div>
...
RudderStack's <Link to="/event-spec/standard-events/identify">`identify`</Link> method captures the relevant details about the visiting user.
...
With the <Link to="/event-spec/standard-events/track">`track`</Link> API, RudderStack makes a call to Heap.io to track your user actions as well as their associated properties.
