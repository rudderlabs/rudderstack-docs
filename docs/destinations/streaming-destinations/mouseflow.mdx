...
RudderStack enables you to send custom variables to Mouseflow via the `identify` and `track` calls by passing them via the <Link to="/sources/event-streams/sdks/rudderstack-javascript-sdk/#the-options-parameter">`integrations`</Link> object.
