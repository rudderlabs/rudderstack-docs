...
You can use the <Link to="/event-spec/standard-events/group/">`group`</Link> call to link a contact with a segment, campaign, or a company. 

<div class="infoBlock">
RudderStack uses the <code class="inline-code">type</code> trait present in the <code class="inline-code">group</code> call to assign a contact to a segment, campaign, or a company.
</div>
