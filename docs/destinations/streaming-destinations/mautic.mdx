...
You can use the <Link to="/event-spec/standard-events/group/">`group`</Link> call to link a Mautic contact with a segment, campaign, or a company.
...
RudderStack uses the `type` trait present in the `group` call to link a given user to a segment, campaign, or company. You can add or remove a user to a group using the `operation` trait. If this field is absent, RudderStack sets to `Add` by default.
