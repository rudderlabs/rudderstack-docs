...
| RudderStack property | DCM Floodlight property | Tag | Notes |
| :--- | :--- |:--- | :---- |
| `properties.ord` | `ord` | Counter - standard | - |
| `properties.sessionId` | `ord`  | Counter - per_session <br/><br/> Counter - unique  | RudderStack sends `ord=1` for the Counter - unique tag. |
| `properties.num` | `num` | Counter - unique | - |
| `context.device.adTrackingEnabled` | `dc_lat` | Counter/Sales | - |
| `properties.matchId` | `match_id` |  Counter/Sales | - |
| `properties.orderId` | `ord` |  Sales | - |
...
| `properties.quantity` | `qty` | Sales - items_sold <br/><br/> Sales - transactions | RudderStack sends `qty=1` for the Sales - transactions tag. |
