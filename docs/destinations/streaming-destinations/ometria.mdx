...
The `track` call lets you send custom events to Ometria. Note that `track` also supports the Ecommerce events and sends them using the **Ometria Order Object**.
...
The following table includes all the fields in `track` call for Ecommerce orders, with their relative mapping to the Ometria fields:
