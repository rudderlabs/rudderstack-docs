...
- If **Role-based Authentication** is disabled, you need to enter the **AWS Access Key ID** and **AWS Secret Access Key** to authorize RudderStack to send data to your Lambda function. Refer to the <Link to="#policy-permissions">Policy permissions</Link> section below for more information on specifying the required permissions.
