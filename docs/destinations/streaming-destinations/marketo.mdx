...
<img src="../../assets/event-stream-destinations/marketo-connection-settings-1.png" />
...
<img src="../../assets/event-stream-destinations/marketo-connection-settings-2.png" />
...
The following settings are associated with the <Link to="#track">`track`</Link> events and require you to first <a href="https://experienceleague.adobe.com/docs/marketo/using/product-docs/administration/marketo-custom-activities/create-a-custom-activity.html">create a Custom Activity in Marketo</a> before sending the events. You also need to approve the activity in order to get the <strong>Custom Activity ID</strong>.
...
- **Map event properties to Marketo property name**: Use this setting to map your `track` event properties to the Marketo fields listed in your custom activity. The value passed in this property will be set for the appropriate field present for the custom activity mapped to the event.
- **Map event name to Marketo primary key**: Map the `track` event name to the Marketo primary key associated with the custom activity. Please note that the primary field for a custom activity is denoted by a

<div class="infoBlock">
For more information on these settings, refer to the <Link to="#event-mapping-settings">Event mapping settings</Link> section below.
</div>

### Lead Mapping settings

<img src="../../assets/event-stream-destinations/marketo-connection-settings-3.png" />

- **Map your traits to Marketo custom fields**: This setting lets you map your event traits to custom Marketo fields **apart from** the mappings specified in the <Link to="#supported-mappings">Identify trait mappings</Link> section below. Please note that the traits can only be mapped to this list of standard fields (REST API Name column) as well as other supported field name available on your lead records.
...
<div class="warningBlock">
Make sure you have already <a href="https://experienceleague.adobe.com/docs/marketo/using/product-docs/administration/marketo-custom-activities/create-a-custom-activity.html">created the custom activities in Marketo</a> before sending the events. You also need to approve the activity in order to get the <strong>Custom Activity ID</strong>.
</div>
