...
In the above snippet, set `override: true` if you want RudderStack to identify the users who have already been identified as a ConvertFlow contact. If you set `override: false`, RudderStack identifies the users who have **not** been previously identified as a contact.
...
| `cfClosed`| `CTA Closed` | Fired when the user closes a CTA.
