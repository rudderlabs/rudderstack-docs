...
<div class="infoBlock">
The <Link to="/destinations/streaming-destinations/adobe-analytics/setting-up-adobe-analytics-in-rudderstack/#connection-settings">RudderStack dashboard settings</Link> also has a toggle for SSL which, if enabled, will send the Heartbeat Tracking URL in the HTTPS mode.
</div>
