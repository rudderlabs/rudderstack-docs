...
- **Sales Activities**: Any type of interaction between a sales rep and a prospect is a sales activity. Such an activity might propel the sales process, helping the sales rep get closer to winning a deal.

Through RudderStack we can create the status of the Customer. For creating the status either `salesActivityTypeId` or `salesActivityName` is required. We can create the lifeCycle stage from the Freshmarketer dashboard.


- **Lifecycle Stage**: This event captures the decision journey of a customer. Through RudderStack we can update the status of the Customer. For updating the status either lifecycleStageId or lifecycleStageName is required. We can create the lifeCycle stage from the Freshmarketer dashboard. Here in image lifecycleStageName is Sales Qualified Lead start, final Customer , Lead, Customer . It can be edited and changed through their dashboard.
