...
Yes, an event is required. Here event is basically eventMapping which we can set from RudderStack Dashboard. So, Once the user selects its event from RudderStack Dashboard, we can make the call accordingly.
