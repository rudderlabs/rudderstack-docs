...
The <Link to="/event-spec/standard-events/track/">`track`</Link> call can be used to track the user activities in Freshmarketer.
...
RudderStack lets you add one or more contacts to the specified marketing list. To do so, you need to provide the contact's `email` and `listId`/`listName`. Also, RudderStack creates a new marketing list if the `listName` (case-sensitive) does not exist already.

<div class="infoBlock">
<code class="inline-code">groupType</code> is a required field and its value can be <code class="inline-code">accounts</code> or <code class="inline-code">marketing_lists</code>.
</div>
