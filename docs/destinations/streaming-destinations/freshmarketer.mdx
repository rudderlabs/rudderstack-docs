...
- **Map your events with Freshmarketer Standard Events**: Use this setting to map the standard Freshmarketer events with custom event names.
...
The <Link to="/event-spec/standard-events/track/">`track`</Link> call can be used to track the activities of a user. Rudderstack supports the following events:

- **Sales Activities**: This event captures any type of interaction between a sales representative and the prospect.

RudderStack supports creating the status of a user by taking either the `salesActivityTypeId` or `salesActivityName`.

The following table property mappings:
...
- **Lifecycle Stage**: This event captures the decision journey of a customer. You can create the lifecycle stage in the Freshmarketer dashboard. For example, the below image shows a lifecycle stage where **Sales Qualified Lead start**, **final Customer**, **Lead**, and **Customer** are the lifecycle stage names. You can also change/edit these in the Freshmarketer dashboard.

<img src="../../assets/event-stream-destinations/freshmarketer-connection-settings.png" alt="Freshmarketer connection settings" />

RudderStack lets you update the status of a user by taking either the `lifecycleStageId` or `lifecycleStageName` (case-sensitive). 

<div class="infoBlock">
<code class="inline-code">event</code> is a required field for the <code class="inline-code">track</code> call. 
</div>
...
RudderStack lets you add one or more contacts to the specified marketing list. You must provide the contact's `email` and `listId`/`listName` to add them to the marketing list. Also, RudderStack creates a new marketing list if the `listName` (case-sensitive) is not available.

<div class="infoBlock">
<code class="inline code">groupType</code>  is required and it should be <code class="inline code">accounts</code> or <code class="inline code">marketing_lists</code>.
</div>
