...
RudderStack lets you send your event data to Pinterest Tag via the <Link to="/destinations/rudderstack-connection-modes/#cloud-mode">cloud mode</Link>. It sends the event calls in a <Link to="/docs/api/http-api/#12-batch">batch</Link> where each batch can contain upto 1000 events.
...
The track events are mapped as specified in the **Map Your Events To Pinterest Events** connection setting in the dashboard.
...
- **`view_category`**: RudderStack sends this event if both the `name` and `category` fields are present. The below sample event contains both and is mapped to the Pinterest's `view_category` event:
...
|  Integrations Object `messageId` | `event_id` | Optional | Unique Id (string) that identifies the event and can be used for deduping. |
| `pageUrl` | `event_source_url` | Optional | URL of the web conversion event. |
| `properties.optOut`/`properties.opt_out`/`context.traits.optOut`/`context.device.adTrackingEnabled` | `opt_out` | Optional | When <code class="inline-code">action_source</code> is **web** or **offline**, it defines whether the user has opted out of tracking for web conversion events whereas when <code class="inline-code">action_source</code> is **app_android** or **app_ios**, it defines whether the user has enabled **Limit Ad Tracking** on their iOS device or opted out of **Ads Personalization** on their Android device.|
...
<div class="infoBlock">
For iOS source, if <code class="inline-code">context.device.adTrackingEnabled</code> is <code class="inline-code">true</code>, <code class="inline-code">opt_out</code> should be <code class="inline-code">false</code> and vice-versa.
For other source platforms, RudderStack takes the value directly from <code class="inline-code">properties.opt_out</code>.
</div>
...
| `birthday` | `db` (yyyymmdd format) |  Array of strings with SHA-256 |
|  `lastName` | `ln` | Array of strings with SHA-256 | 
| `firstName` | `fn` | Array of strings with SHA-256 |
...
In order to send events for the <code class="inline-code">track</code> or <code class="inline-code">page</code> call successfully, you need to send <strong>at least one</strong> of the following user properties:
<ul>
