...
- **App Store App ID**: Enter the App Id from your App Store. This field is **required only** when the source is iOS.
...
- **Enable Event Deduplication**: Enable this setting to deduplicate the events. If enabled, you need to enter the following:
  - **Deduplication key**: Enter the key using which Pinterest Tag deduplicates the events.
