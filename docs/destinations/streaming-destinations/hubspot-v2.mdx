...
- **RudderStack Event Name**: Enter the RudderStack event name you want to map to the HubSpot custom behavioral event name.
- **HubSpot Event Name**: Enter the HubSpot event name you want to map to the above-mentioned RudderStack event.
...
<div class="infoBlock">
Refer to the <Link to="https://developers.hubspot.com/docs/api/analytics/events">custom behavioral events</Link> section to know more about them.
</div>
...
RudderStack sends the <Link to="/event-spec/standard-events/identify">`identify`</Link> call to check if a contact exists, if yes, it performs the update through the new update endpoints.
...
<div class="warningBlock">
The custom behavorial events can only be used for the private app authentication type. Also, the private app must have the <code class="inline-code">analytics.behavioral_events.send</code> permission.
</div>
