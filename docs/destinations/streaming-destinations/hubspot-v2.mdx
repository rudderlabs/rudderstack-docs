...
Select authorization type as **Private Apps** which allows you to use HubSpot's APIs to access specific data from your HubSpot account.
...
Select authorization type as **API Key (deprecating soon)** which allows you to create custom applications using HubSpot's APIs.
...
- **HubSpot property name to be used as lookup field**: Enter the unique HubSpot property to be used for <Link to="https://developers.hubspot.com/docs/api/crm/contacts">contact</Link> lookup. The provided lookup field key will be retrieved from traits.
- **RudderStack Event Name**: Enter the custom behavioral RudderStack event name you want to map to the HubSpot event name.
- **HubSpot Event Name**: Enter the custom behavioral HubSpot event name you want to map to the above-mentioned RudderStack event name.
- **Map your Event Property**: Enter the RudderStack and HubSpot event properties you want to map for the above-mentioned custom behavioral events. You can specify multiple properties for a single event.

<div class="infoBlock">
Refer to the <Link to="https://developers.hubspot.com/docs/api/analytics/events">custom behavioral events</Link> section to know more.
</div>
...
RudderStack sends the <Link to="/event-spec/standard-events/identify">`identify`</Link> call to check if a contact exists, if yes, the update is done through the new Update endpoints. Hence, there is an axios call in between.
...
A <Link to="/event-spec/standard-events/track/">`track`</Link> call lets you record various user actions and any properties associated with them. To associate a `track` call with a user, you need to specify the user's `email` under `context.traits`. Additionally, RudderStack associates the `track` events with the same user after an `identify` request is successfully made.
...
The following table lists the (optional) property mappings between RudderStack and HubSpot for the `track` API:
...
- **Properties object**: When you create a custom behavioral event in HubSpot, some default properties are provided with those events.

#### Property mappings

The following table lists the (optional) default property mappings between RudderStack and HubSpot for custom behavioral events:
