...
RudderStack maps the following <Link to="/event-spec/ecommerce-events-spec/">ecommerce events</Link> to the Blueshift events in the `track` call before sending them over Blueshift's HTTP API.
