...
- **Project Name**: Enter your Woopra project name. This is the same as the domain you entered while setting up the project.

<div class="infoBlock">
For more information on obtaining your Woopra project name, refer to the <Link to="#faq">FAQ</Link> section below.
</div>

<div class="infoBlock">
RudderStack uses this name as a fallback value if it is absent in the event's <code class="inline-code">integrations</code> object. For more information, refer to the <Link to="#identify">Identify</Link> section below.
</div>
...
- **Download Tracking**: Enable this setting to track the downloads on the web page.
...
| :-----| :------| :-----|
| `properties.{x}` | `ce_{x}` | - |
| `name` | `event` | String (in lower case) |
...
| `context.app.name` + `context.app.version` | `app` | String | Optional |
| `ua.browser.name` + `ua.browser.version` | `browser` | String | Optional |
| `context.os.name` | `os` | String | Optional |

<div class="warningBlock">
Any user-provided cookie must be mapped to either <code class="inline-code">email</code> or <code class="inline-code">cv_id</code> when creating a new user or using the cookie for the first time. Otherwise, Woopra accepts the data but will reflect it <strong>only when</strong> that cookie is mapped to any other identifier.
</div>

RudderStack uses the below fields to identify a user, in the same precedence:

1. `userId` / `traits.userId` / `traits.id` / `context.traits.userId` / `context.traits.id` (mapped to `cv_id`)
2. `traits.email` / `context.traits.email` / `properties.email` (mapped to `cv_email`)
3. `context.externalId.woodpraId` / `anonymousId` (mapped to `cookie`)
