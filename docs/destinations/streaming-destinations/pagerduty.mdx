...
- **dedup_key Field Identifier**: Enter a `dedup_key` which acts as an identifier for the event. You can provide any value in this field. If not provided, PagerDuty automatically generates and assigns a `dedup_key` to the event. This key is used while updating, resolving, or acknowledging an event.
