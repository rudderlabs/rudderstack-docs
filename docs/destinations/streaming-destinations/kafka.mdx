...
Apache Kafka is a popular distributed streaming platform used for building realtime data pipelines and streaming apps.
...
<div class="infoBlock">
For more information on this setting, refer to the <Link to="#serializing-data-using-avro">Serializing data using Avro</Link> section below.
</div>
...
RudderStack also lets you serialize your data using [Avro](https://www.ibm.com/in-en/topics/avro) and send it to a specific Kafka topic. To do so, it leverages the [goavro](https://pkg.go.dev/github.com/linkedin/goavro/v2) package.
...
1. **Generate Key and Certificates**: Run the following command:
...
2. **Create your own CA**:
   ...
   - Generate a CA that is a public-private key pair and a certificate. **Enter the contents of this certificate in the RudderStack dashboard under the CA certificate setting**.
...
3. **Sign the certificates**:
   ...
   - Export the certificate from the keystore, like so: 
```
keytool -keystore kafka.server.keystore.jks -alias localhost -certreq -file cert-file
```
   - Sign it with the CA, as shown:
```
openssl x509 -req -CA ca-cert -CAkey ca-key -in cert-file -out cert-signed -days {validity} -CAcreateserial -passin pass:{ca-password}
```
   - Import both the certificate of the CA and the signed certificate into the broker keystore, as shown: 
...
By following all the steps described above, the script to create the CA and broker and client truststores and keystores is obtained, as shown:
...
4. Enter the below parameters in your `server.properties`:
...
5. You also need to put RudderStack's CA certificate to your truststore, as shown:
...
The CA certificate that you need to add to your trust store is shown below:
...
You will need to expose one public address to which RudderStack can connect. It is recommended using SSL for that. 
...
Configure your Zookeeper with `SASL_SSL` to resolve this issue.
