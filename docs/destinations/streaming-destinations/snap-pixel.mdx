...
Snap Pixel supports up to 5 custom events, as listed in the table below:
...
For example, if you set **Client Deduplication Field** to `properties.dedup` and the event contains the`properties.deduplication` property instead, then RudderStack sends the event to Snap Pixel **without setting** the client deduplication ID.
