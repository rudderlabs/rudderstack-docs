...
The following scenarios describes the different ways RudderStack sets the custom deduplication key:
...
Suppose you set **Client Deduplication Field** to `properties.dedup` and the event contains `properties.deduplication` instead, then RudderStack sends the event to Snap Pixel **without setting** the client deduplication ID.
