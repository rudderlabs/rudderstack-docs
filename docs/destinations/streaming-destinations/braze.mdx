...
<div class="infoBlock">
You can also send nested objects inside track events to send nested JSON data as properties.
</div>
...
If you are using Braze in device mode, you can save on costs by deduplicating `identify` calls.
Configure this in your dashboard under **SDK settings** for JavaScript, Android, and iOS. If you have enabled *device mode*, a toggle for **Deduplicate Traits** appears underneath. Switch on this toggle to begin sending to Braze only those attribute (trait) fields which have changed.  

We suggest you review Braze's [data points policy](https://www.braze.com/docs/user_guide/onboarding_with_braze/data_points/) first to further understand how this functionality can help you avoid data overages.
## FAQ
