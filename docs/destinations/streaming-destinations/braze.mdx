...
<div class="infoBlock">
For more information on obtaining your Braze app key and the REST API key, refer to the <a href="https://www.braze.com/docs/user_guide/administrative/app_settings/developer_console/api_settings_tab/">Braze documentation</a>. Alternatively, you can refer to this <a href="https://www.braze.co.jp/docs/api/api_key/#where-can-i-find-it">REST API Key Overview</a> guide.
</div>
...
You can use the <Link to="/event-spec/standard-events/identify/">`identify`</Link> call to identify a user in Braze. 

You should call `identify` in any of the following cases:
...
When you call the `track` method for an `Order Completed` event, RudderStack sends the product information present to Braze as **purchases**.
...
Once you send a `group` event, RudderStack sends a custom attribute to Braze with the name as `ab_rudder_group_<groupId>` and the value as `true`. For example, if the `groupId` is `123456` then RudderStack will create a custom attribute to Braze with the name `ab_rudder_group_test_123456` and set the value to `true`.