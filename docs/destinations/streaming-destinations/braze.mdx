...
You can also send nested arrays as the `track` properties and perform create, add, update, and remove operations on them. To do so, you must first enable the **Nested Array Operations** dashboard setting. Then, include the field `nestedOperationType` in the `track` event, as shown:
...
Note that the `nestedOperationType` field is required for this feature to work and must be one of `create`, `update`, `add`, or `remove`.
