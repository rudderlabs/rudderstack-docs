...
Docs notes:
"head's up" »» "heads up" in UI

[Amplitude](https://amplitude.com) is a comprehensive product analytics service for web and mobile platforms. 12,000+ companies use Amplitude to get marketing insights that drive product strategy, conversion, and retention of their customers.

RudderStack provides native integration code for the following languages
...
## Getting started: Creating your Connection

1. If you haven’t already, first create a source in your RudderStack dashboard.  Your source will be where you intend on sending data from.  Learn more about sources here.
2. Connect a source to Amplitude.  You can do this directly from your source by navigating to Overview > Add Destination > Create a New Destination.  Or, you can create the Amplitude destination first by navigating to Add Destination on your dashboard.
3. Verify that your desired  source is supported by Amplitude by referencing the ‘Modes supported’ table above.
4. Set up your connection by inputting some required information:
- Destination name: Choose a name for your destination that will be easily identifiable later.  Often, users like to include suffixes such as ‘-prod’, ‘-dev’, ‘-testing’ to differentiate connection environments.
- API key:  To find your API key, go to your project in Amplitude and look in the general tab.  See below

    [[AMPLITUDE SCREENS]]


<div class="infoBlock">
You should also note down your Secret Key if you intend on deleting users.  You may want to consider this option for GDPR purposes.</div>

- Residency server: Choose whether you want to connect to Amplitude’s Standard Server (US) or EU-based residency server
5. Choose the Connection Mode for each of your sources.  This determines how your data will be routed from your source to your destination.  Learn more about cloud mode vs. device mode here.  We typically recommend using cloud mode as you will have access to Transformations and more reliable performance (compared to device mode).
...
8. Once you are happy with your settings, Enable your connection at the top of your Configurations page to start sending events to Amplitude.
...
### Other settings: Client-side events filtering
...
### Other settings: OneTrust cookie categories
...
You are able to find the OneTrust category names here:

[input screenshot]


Other settings: eCommerce settings
...
| Field                                                  | Description                                                                                                                                                                                                                                                   | Note                                                                 |   |   |
|--------------------------------------------------------|---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|----------------------------------------------------------------------|---|---|
| Replace device ID with anonymous ID                    | Turning this on will allow you to use the anonymous ID generated by RudderStack’s SDK instead of the device ID.  This will automatically be set as anonymousID by RudderStack’s SDK. Or, you can set it yourself using the setAnonymousId() method.           | Only applicable to web device mode.                                  |   |   |
| Set device ID from URL parameter                       | Turning this on will set the device ID by parsing the URL parameter from amp_device_ID.                                                                                                                                                                       | Only applicable to web device mode.                                  |   |   |
| Force HTTPS                                            | Turning this on will upload events to the HTTPS endpoint.  Otherwise, it will use the embedding site’s protocol.                                                                                                                                              | Only applicable to web device mode.                                  |   |   |
| Track GCLID                                            | When on, this captures the GCLID URL parameters along with the user's initial_gclid parameters                                                                                                                                                                | Only applicable to web device mode.                                  |   |   |
| Track referrer information                             | When on, this captures the user's referrer, initial referrer, and referring domain as user properties                                                                                                                                                         | Only applicable to web device mode.                                  |   |   |
| Track UTM properties                                   | When on, this captures the UTM parameters as user properties by either parsing the query string or from the _utmz cookie                                                                                                                                      | Only applicable to web device mode.                                  |   |   |
| Save referrer, UTM params, GCLID only once per session | When on, this tracks referrer, UTM parameters, and GCLID only once per session, ignoring any new values which may enter a user’s session                                                                                                                      | Only applicable to web device mode.                                  |   |   |
| Reset referrer or UTM params for new sessions          | Turning this on will set referrer and utm_parameter properties to null upon instantiating a new session; otherwise, existing referrer and utm_parameter values will be passed to each new session                                                             | Only applicable to web device mode.                                  |   |   |
| Batch events prior to upload                           | Turning this on will batch events to upload only after the number of queued events is greater than or equal to eventUploadThreshold, or after events to upload after eventUploadPeriodMillis milliseconds have passed since the first unsent event was logged | Only applicable to web device mode.                                  |   |   |
...
| Field                          | Description                                                          | Note                                   |   |   |
|--------------------------------|----------------------------------------------------------------------|----------------------------------------|---|---|
| Track session events           | Turning this on will send start and end session events               | Only applicable to mobile device mode. |   |   |
| Use IDFA for device ID         | Turning this on will send the IDFA instead of device ID to Amplitude | Only applicable to mobile device mode. |   |   |
| Batch event upload period (ms) | Set the time limit (in ms) between batch uploads                     | Only applicable to mobile device mode. |   |   |
| Batch event upload threshold   | Set the minimum number of events to be sent in a batch               | Only applicable to mobile device mode. |   |   |
...
| Field                            | Description                                                                                   | Note                                   |   |   |
|----------------------------------|-----------------------------------------------------------------------------------------------|----------------------------------------|---|---|
| Enable location listening        | When on, capture user location information for anyone who has granted app location permission | Only applicable to mobile device mode. |   |   |
| Track session events             | Turning this on will send start and end session events                                        | Only applicable to mobile device mode. |   |   |
| Use advertising ID for device ID | Turning this on will send Advertising ID instead of Device ID to Amplitude                    | Only applicable to mobile device mode. |   |   |
| Batch event upload period (ms)   | Set the time limit (in ms) between batch uploads                                              | Only applicable to mobile device mode. |   |   |
| Batch event upload threshold     | Set the minimum number of events to be sent in a batch                                        | Only applicable to mobile device mode. |   |   |