...
- If both `userId` and `groupId` are present in the `group` event, then RudderStack first finds the user with `email` present in the <Link to="/event-spec/standard-events/common-fields/#contextual-fields">`context.traits`</Link> object.
