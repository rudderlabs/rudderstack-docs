...
* **Create users as verified**: Enable this setting to create verified users in Zendesk. If this setting is enabled, RudderStack sends `verified` as `true` in the event payload and Zendesk skips the email verification for these users.
* **Send Group calls without a User ID**: Enable this setting if you do not want to associate a particular user with a group. If this setting is enabled, the group will be created or updated only if `userId` is **not** present in the event.
...
RudderStack maps the event properties to a subset of the standard Zendesk user attributes. The following table highlights the supported property mappings:
...
Enable the **Create users as verified** dashboard setting in RudderStack to create and register the verified users in Zendesk. Zendesk skips the email verification for these users.
...
The <Link to="/event-spec/standard-events/track/">`track`</Link> call lets you record the user actions along with any properties associated with them and send this information to Zendesk.
...
<div class="infoBlock">
RudderStack uses the <a href="https://developer.zendesk.com/rest_api/docs/sunshine/events_api">Zendesk Sunshine Events API</a> to send the <code class="inline-code">track</code> calls.
</div>
...
You can also use `group` to associate a particular user to an organization. To do so, you need to disable the **Send Group Calls without User ID** dashboard setting in RudderStack and pass the `userId` via the `identify` call before sending any `group` calls to Zendesk.
...
### How RudderStack does the user-group association

- If `groupId` and `userId` both are present in the `group` event, then RudderStack finds the user with `email` present in the `context.traits` object.
- If `email` is absent, RudderStack creates the user. It will then find the organization associated with the `groupId` present in the payload. If `groupId` is absent too, RudderStack creates a new group and then does the user-group association.
- If a user already exists, the `organizationId` will be associated with the user and all the user information will be sent for this call. 
