...
If `groupId` and `userId` both are present in the `group` event, then RudderStack finds the user with `email` present in the `traits`. If `email` is absent, RudderStack creates the user. It will finds the organization associated with the `groupId` present in the payload. If `groupId` is absent too, RudderStack creates a new group and then does the user-group association.
...
* New `groupId` and new `email` (No Zendesk user is associated with the email): RudderStack creates the user and the organization and associates them.
