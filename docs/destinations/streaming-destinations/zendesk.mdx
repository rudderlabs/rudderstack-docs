...
To set a user's email address as the primary email in Zendesk, enable the **Update user's primary email** dashboard setting and pass the `email` trait in the `identify` call, as shown:
...
When you send the above event, Zendesk sets `alex@example.com` as the user's primary email. Any previously set primary email is then set as the secondary email.
