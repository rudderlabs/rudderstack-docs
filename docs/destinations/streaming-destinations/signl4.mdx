...
- **X-S4-Status**: Enter the alert status by choosing the **new**, **acknowledged**, or **resolved** values.
...
- If you provide `alert_status` in the **RudderStack property name** field, then the **X-S4-Status** value is taken from `properties.alert_status`. Suppose the value for `properties.alert_status` is set to `resolved`, the output payload will contain `X-S4-Status: resolved`.

If you provide values in both fields, the final output payload will contain `X-S4-Status: resolved`, as the **RudderStack property name** is given the higher priority.
...
- **Event to title mapping**: Enter the event name and alert title you want to be mapped corresponding to each other. If you do not provide any value, the alert title is set as the event name.

## Signl categories

In SIGNL4, you can use the categories to mark and tag Signls, for example, based on severity, type or relating IT systems, machines, sensors, and services. Refer to the [SIGNL4 documentation](https://account.signl4.com/manage/Category) to know more about different categories.
...
You can pass any key-value pair in the `track` call; it is mapped as it is in the SIGNL4 destination.
