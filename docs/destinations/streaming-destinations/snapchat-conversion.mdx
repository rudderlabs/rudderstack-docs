...
- **Map your events with Snapchat Standard Events**: Enter the event name and select the corresponding <Link to="https://businesshelp.snapchat.com/s/article/chrome-helper?language=en_US">Snapchat standard event</Link> from the dropdown to be triggered when that event is called.
