...
For device mode integrations, the Singular SDK uses RudderStack's <Link to="/event-spec/standard-events/identify">`identify`</Link> method to map the user ID to their custom user ID. RudderStack uses Singular's [`setCustomUserId`](https://support.singular.net/hc/en-us/articles/360037581952-Android-SDK-Basic-Integration#Optional_Setting_the_User_ID) API to forward the identified user ID to Singular.
