...
- **Map Categories to Facebook Content Types**: Enter the category value and the corresponding Facebook [`content_type`](https://developers.facebook.com/docs/meta-pixel/get-started/advantage-catalog-ads#content-type) which should be mapped to each other. The specified `content_type` is sent to Facebook every time the specified category value is sent via Rudderstack. Refer to the <Link to="/destinations/streaming-destinations/fb-offline-conversions/#sending-customized-content_type">Sending customized `content_type`</Link> section for more information on sending the customized `content_type` to Facebook.
...
1. If `content_type` is provided in the `integrations` object, it is given the highest priority.
2. If `content_type` is provided in the RudderStack dashboard, it is given the second highest priority.
