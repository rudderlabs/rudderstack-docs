...
Note the following points:
 - If you are sending `products` array through `properties` then make sure that every product only contains id, quantity, brand, category and price  if you send anything apart from this for product description then facebook-servers will throw an error
