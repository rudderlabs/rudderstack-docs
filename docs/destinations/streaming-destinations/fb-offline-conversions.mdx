...
- **Map Categories to Facebook Content Types**: Enter the category value and the corresponding Facebook [`content_type`](https://developers.facebook.com/docs/meta-pixel/get-started/advantage-catalog-ads#content-type) which should be mapped to each other. This `content_type` is sent to Facebook every time you send the specified category value via Rudderstack. Refer to the <Link to="/destinations/streaming-destinations/fb-offline-conversions/#sending-customized-content_type">Sending customized `content_type`</Link> section for more information on sending the customized `content_type` to Facebook.
