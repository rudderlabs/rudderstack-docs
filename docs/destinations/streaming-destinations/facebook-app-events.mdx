...
This section lists some `track` event and property mappings which are **applicable only** for <Link to="/sources/event-streams/sdks/rudderstack-ios-sdk/ios-v2/">iOS v2</Link> device mode.
...
<div class="infoBlock">
Starting from iOS version 14.5, you need to get the device consent to share data with Facebook or not by setting the  <code class="inline-code">isAdvertiserTrackingEnabled</code> property. Refer to Facebook's <a href="https://developers.facebook.com/docs/app-events/getting-started-app-events-ios#get-device-consent">Get device consent</a> documentation for more information.
</div>
