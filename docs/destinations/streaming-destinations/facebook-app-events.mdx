...
[**Facebook App Events**](https://developers.facebook.com/docs/app-events/) is Facebook's event tracking functionality. It lets you track events via your app or web page, including user activities such as app installation, purchases, etc.
...
To successfully configure App Events as a destination, you will need to configure the following settings:
...
The following options are applicable only if you are sending events to Facebook using the <Link to="/destinations/rudderstack-connection-modes/#device-mode">device mode</Link>:
...
<ol>
<li>Install <code class="inline-code">Rudder-Facebook</code> (available through <a href="https://cocoapods.org/">CocoaPods</a>) by adding the following line to your <code class="inline-code">Podfile</code>:
<span>
...
The <Link to="/event-spec/standard-events/track">`track`</Link> call is logged to Facebook using the `logEvent` method of the `AppEventsLogger` class. RudderStack uses the same `eventName` as you have passed to the `track` method. Along with that, it will pass all the `properties` after converting them into an accepted format.
...
This section lists some `track` event and property mappings which are only applicable for <Link to="/sources/event-streams/sdks/rudderstack-ios-sdk/ios-v2/">iOS v2</Link> device mode.

The following table lists the mapping for `track` event properties between RudderStack and Facebook App Events:
...
The following table lists the mapping for e-commerce events between RudderStack and Facebook App Events :
...
Along with the above, RudderStack supports the following Lifecycle events and maps them as it is before sending them to Facebook App Events:
...
<div class="infoBlock">
Starting from iOS version 14.5, you need to get the device's consent to share data with Facebook or not by setting the  <code class="inline-code">isAdvertiserTrackingEnabled</code> property. Refer to Facebook's <a href="https://developers.facebook.com/docs/app-events/getting-started-app-events-ios#get-device-consent">Get device consent</a> documentation for more information.
</div>
