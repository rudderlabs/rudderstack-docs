...
You can use the <Link to="/event-spec/standard-events/identify">`identify`</Link> call to create a new contact in SendGrid based on the contact's email ID. If the email ID already exists, RudderStack updates the contact details.
...
```javascript
rudderanalytics.identify("user@101", {
  name: "Chris Miles",
  email: "chrismiles@gmail.com"
})
```
