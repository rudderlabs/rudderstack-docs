...
- **SendGrid Contact List Id**: Enter the SendGrid's contact list ID where you want to create a contact. Refer to the [SendGrid documentation](https://docs.sendgrid.com/api-reference/lists/get-all-lists) to use their API endpoint to retrieve all your lists with IDs.
...
Also, RudderStack sends the `identify` events to SendGrid in a batch where the batch limit is 30000.
...
- In `externalId` object as shown in the above code snippet (takes highest precendence)
