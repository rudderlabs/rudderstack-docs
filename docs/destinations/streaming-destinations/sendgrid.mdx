...
- **API Key**: Enter your SendGrid API Key. Refer to the [Sendgrid documentation](https://app.sendgrid.com/settings/api_keys) for more information on obtaining the API Key.
...
<div class="infoBlock">
For more information on how to send these details via the <code class="inline-code">replyTo</code> object, refer to the refer to the <Link to="#replyto">replyTo</Link> section below.
</div>

- **IP Pool Name**: Enter the IP Pool Name. For detailed steps on creating the IP pool name, refer to the <a href="https://docs.sendgrid.com/ui/account-and-settings/ip-pools#create-an-ip-pool">SendGrid documentation</a>.

<div class="infoBlock">
The length of the IP Pool Name must be between 2 and 64 characters. 
</div>

- **SendGrid Contact List Id**: Enter the SendGrid's contact list ID where you want to create the contact. Refer to the [SendGrid documentation](https://docs.sendgrid.com/api-reference/lists/get-all-lists) to use their API endpoint to retrieve all your lists with IDs.
...
- **Event**: Create the list of events for which the `track` calls will be made.
...
Also, RudderStack sends the `identify` events in a batch where the batch limit is 30000.

<div class="infoBlock">
<code class="inline-code">email</code> is a required field to make an <code class="inline-code">identify</code> call successfully.
</div>
...
- In `externalId` object as shown in the above code snippet (takes precendence over the below)
- In **SendGrid Contact List Id** Rudderstack dashboard setting.

<div class="infoBlock">
If there is any custom fields in the event payload, make sure that it is present in SendGrid and mapped in the <strong>Map your traits to SendGrid custom fields</strong> RudderStack dashboard setting.
</div>

<div class="infoBlock">
The maximum number of events that can be batched 
