...
You can use the <Link to="/event-spec/standard-events/identify/">`identify`</Link> call to <Link to="https://developers.canny.io/api-reference#create_or_update_user">create or update a user</Link>.
...
| `originalTimestamp`/`timestamp` | `created` | Optional | Date |
...
| `properties` (after removing `boardID`, `byID`, `categoryID`,`details`, `eta`, `etaPublic`, `title`, `imageURLs`)| `customFields` | Optional | Object | Ensure that you <Link to="#how-to-create-custom-fields-in-canny-dashboard">create the custom fields in Canny dashboard</Link> to send the event seuccessfully.|
| `properties.eta` | `eta` | Optional | String | In MM/YYYY format. Required if `etaPublic` is present. |
| `properties.etaPublic` | `etaPublic` | Optional | Boolean | Required if `eta` is present. |
...
### How to create custom fields in Canny Dashboard?

To create custom fields in Canny Dashboard:

1. Log into your [Canny account](https://canny.io/login).
2. Go to **Settings** > **Boards** under your profile.
3. Select the desired **Board** from the dropdown and select **Create Post Form**.
4. Click on **Create new Post Fields to add them to a board** in the **Fields** section.
5. Click on **Create New Field**, as shown:
