...
To enable the identity verification feature for the web device mode, you can pass `user_hash` within the <Link to="/sources/event-streams/sdks/rudderstack-javascript-sdk/#integrationopts">integrations</Link> object, as shown in the following snippet:
