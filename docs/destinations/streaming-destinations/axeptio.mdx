...
- **Toggle it on to send data through callback**: Enable this setting to allow RudderStack to collect the callback data from the Axeptio SDK as `track` events.
...
By enabling the **Toggle it on to send data through callback** setting in the RudderStack dashboard, you can track and record the interaction events as <Link to="/event-spec/standard-events/track/">`track`</Link> events. 
