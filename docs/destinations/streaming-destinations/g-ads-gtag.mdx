...
- **Track Conversions**: This setting is enabled by default and sends conversion events. However, you can disable it to prevent sending conversion events. When enabled, the following setting is visible:
  - **Enable Conversion Events Filtering**: Enable this setting to enter a list of events you want to send as conversion events in the **Events to Track Conversions** setting. When disabled, RudderStack will send all the events as conversion events.
- **Track Dynamic Remarketing**: Enable this setting to send [dynamic remarketing](https://support.google.com/google-ads/answer/7305793) events. When enabled, the following setting is visible:
  - **Enable Dynamic Remarketing Events Filtering**: Enable this setting to enter a list of events you want to send as dynamic remarketing events in the **Events to Track Dynamic Remarketing** setting. When disabled, RudderStack will send all the events as dynamic remarketing events.
...
As seen in the above snippet, enabling **Track Dynamic Remarketing** option in the RudderStack dashboard allows you to send custom parameters in the `page` call.
