...
| `properties.is_syncing` | `is_syncing` | RudderStack triggers the [targeted automations](https://mailchimp.com/developer/marketing/guides/track-outside-activity-events/#trigger-automations-with-events) by default. However, you can set `is_syncing` to `true` if you don't want to trigger them. |
