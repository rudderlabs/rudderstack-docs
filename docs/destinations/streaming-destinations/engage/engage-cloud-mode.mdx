...
RudderStack uses the <a href="https://engage.so/docs/api/users#create-a-user">Create a User</a> API to add a new user to your Engage account.
...
- If a user is found with an associated `email` or `phone` (in that priority), then RudderStack links the user to the provided Engage List ID. Otherwise, it creates a new user in Engage with the provided details and then links them to the List ID.
- RudderStack can only create a new contact. Updating a user is not supported.
