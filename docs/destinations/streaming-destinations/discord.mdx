...
- **Identify Template**: Enter a template for building a text message while sending an `identify` call to Discord. If not provided, RudderStack uses the default template as: `User {{name}} with {{traits}} is identified`.
- **Event Name**: Enter the event name for which you want the `track` call's event template to be triggered.
- **Event Template**: Enter a template for building a text message while sending a `track` call to Discord. If not provided, RudderStack uses the default template as: `User {{name}} did {{event}} with {{properties}}`.
- **Regex Matching**: Enable this setting if you want to match multiple event names in the `track` call using a regex. It helps in setting the same event template for event names matching the regex.
- **Embed Flag**: Enable this setting to set an embed message for Discord. If enabled, also specify the following:
    - **Title Template**: Enter the template for building the title of the embed message.
    - **Description Template**: Enter the template for building the description of the embed message.
- **Webhook Url**: Enter the webhook URL of a Discord channel where you want to send the data. You can create it by logging in to [Discord](https://discord.com/login) and selecting the channel for which you want to send the data. Further, navigate to the **Channel settings** > **Integrations** > **Create Webhook**. 
...
RudderStack sends the above `identify` call to Discord in the following text message format (using the defaut `identify` template):
- `User Alex Keener with email: alex@example.com, country: USA is identified`
- When **Embed Flag** setting is enabled and the **Title Template** and **Description Template** are provided in the dashboard settings:
...
RudderStack sends the text message after converting it into the `identify` template specified in the dashboard. Also, if the **Embed Flag** setting is enabled, it sends the text message in `content` property and the embed message details in the `embeds` property. Refer to the [Discord documentation](https://discord.com/developers/docs/resources/webhook) for more information.
...
RudderStack sends the text message after converting it into the `track` template specified in the dashboard. Also, if the **Embed Flag** setting is enabled, it sends the text message in `content` property and the embed message details in the `embeds` property. Refer to the [Discord documentation](https://discord.com/developers/docs/resources/webhook) for more information.