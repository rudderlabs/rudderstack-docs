...
- **Embed Flag**: Enable this setting to set an embed message for Discord. If enabled, also enter the following:
...
- **Webhook Url**: Enter the webhook URL of a Discord channel where you want to send the data. You can find it by logging in to [Discord](https://discord.com/login) and selecting the channel for which you want to send the data. Further, navigate to the **Channel settings** > **Integrations** > **Create Webhook**. 
...
For the above `identify` call, RudderStack sends the following text message to Discord (using the defaut `identify` template):
...
For the above `track` call, RudderStack sends the following text message to Discord (using the defaut `track` template):
