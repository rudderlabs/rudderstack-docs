...
- **License Code**: Enter your OneSignal license code.
- **API Key**: Enter your OneSignal API key.
...
RudderStack uses the `identify` call to create or update the customer information in OneSignal. It maps `userId` or `anonymousId` (`userId` is prioritized if both are present ) to OneSignal's `userId` before sending the data via the [`users`](https://docs.OneSignal.com/docs/rest-api-tracking-users#users) API.
...
RudderStack maps the following `identify` traits to the corresponding OneSignal attributes:

| RudderStack trait | OneSignal attribute | Presence | Notes |
| :-------| :--------| :------| :------|
...
RudderStack uses the `track` call to send the custom events to OneSignal via the [`events`](https://docs.OneSignal.com/docs/rest-api-tracking-events#events) API.
...
RudderStack maps the following event properties to the corresponding OneSignal attributes:
...
OneSignal lets you send custom attributes as key-value pairs in the `eventData` parameter as described in the <Link to="#property-mapping">above section</Link>. However, there are a few things you should keep in mind:

- OneSignal ignores any custom attributes starting with `we_` as these are reserved for internal use.
- User attribute names must be less than 50 characters, while the `string` attribute values cannot exceed 1000 characters. OneSignal automatically truncates any additional characters.
...
RudderStack internally transforms the `page` call into a `track` call before sending it as a custom event to OneSignal. It sends the event/category name by transforming it in the `Viewed ${category} ${name} page` format.
...
RudderStack transforms the above event into a `track` call named `Viewed Home Clothes page` and sends it to OneSignal as a custom event.

<div class="infoBlock">
RudderStack sends the <code class="inline-code">page</code> event properties to OneSignal, similar to the <code class="inline-code">track</code> event properties.
</div>
...
RudderStack internally transforms the `screen` call into a `track` call before sending it as a custom event to OneSignal. It sends the event/category name by transforming it in the `Viewed ${category} ${name} screen` format.
...
RudderStack transforms the above event into a `track` call named `Viewed Games home screen` and sends it to OneSignal as a custom event.

<div class="infoBlock">
RudderStack sends the <code class="inline-code">screen</code> event properties to OneSignal, similar to the <code class="inline-code">track</code> event properties.
</div>
