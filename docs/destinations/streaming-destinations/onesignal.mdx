...
- For the browser, RudderStack maps the `device_type` from the browser name and `deviceType` mapping as explained in the below code snippet. It sets the `anonymousId` as an `identifier` depending upon the browser name. 
...
- For iOS and Android, RudderStack takes the `device_type` from the `context.device.type` field and maps it with the `deviceType` value from the above-mentioned mappings. The `context.device.token`/`context.device.id` is set as the `identifier`.
