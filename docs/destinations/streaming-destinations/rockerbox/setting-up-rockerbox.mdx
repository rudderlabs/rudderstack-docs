...
- **Use device-mode to send track events**: Enable this setting to send **only** `track` events from the JavaScript SDK to Rockerbox via the <Link to="/destinations/rudderstack-connection-modes/#device-mode">device mode</Link>.
