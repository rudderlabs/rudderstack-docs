...
If you have self-hosted the control plane using the open-source <Link to="/tmp-get-started/rudderstack-open-source/control-plane-lite/">Control Plane Lite</Link> utility, follow <Link to="//tmp-get-started/rudderstack-open-source/control-plane-lite/#kubernetes">Kubernetes instructions</Link> to set up the data plane.

## Sending test events

Once you have successfully performed the steps above, follow the <Link to="/tmp-get-started/rudderstack-open-source/sending-test-events/">Sending Test Events</Link> guide to verify the installation.
...
- <Link to="/tmp-destinations/streaming-destinations/google-cloud-storage/">Google Cloud Storage</Link> as a destination for dumping jobs, or
- <Link to="/tmp-destinations/warehouse-destinations/bigquery/">Google BigQuery</Link> as a warehouse destination
