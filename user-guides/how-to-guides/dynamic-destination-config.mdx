...
<img src="../../master/assets/Dynamic%20Destination%20Config.png" />
...
As mentioned previously, you will specify the value for the **Tracking ID** field as `{{ message.properties.trackingId || "UA-1659942401" }}` while setting up the Google Analytics destination. 

For the above payload, the `Config.trackingId` will be set to `UA-1234567890`, which is the value assigned to `properties.trackingId`. As a result, `UA-1234567890` will be passed as the value for **Tracking ID**. 

In case `properties.trackingId` does not have any value, then `Config.trackingId` will be set to `UA-1659942401` and passed as the destination configuration value.
