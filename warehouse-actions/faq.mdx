...
The Warehouse Actions feature supports only source-driven configuration of your pipeline. So, you need to configure a new or existing Warehouse Actions source in RudderStack and then connect it to a new or existing destination. Note that this destination **should not be connected** to any other source.
