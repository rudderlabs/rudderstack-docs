---
#slug: "/docs/reverse-etl"
title: "Reverse ETL Sources"
description: >-
  Detailed technical description of RudderStack's Reverse ETL feature,
...
With RudderStack **Reverse ETL**, you can leverage the already processed customer data residing in your data warehouse and route this enriched information to your desired destinations.
...
**You can connect only one destination to a Reverse ETL source**. Also, note that this destination **should not be connected to any other source** \(including [**Event Stream**](https://rudderstack.com/docs/stream-sources/) sources.\)
</div>
...
**You cannot connect a Reverse ETL source to a warehouse destination in RudderStack.**

</div>

Here's a detailed walkthrough of the Reverse ETL feature:
...
## Configuring Reverse ETL on RudderStack

<div class="warningBlock">

**The** **Reverse ETL** **feature supports only source-driven configuration of your data pipeline**. 

So, you need to configure a Reverse ETL source in RudderStack first, and then connect it to a new or existing destination \(the existing destination should not be connected to any other source\), as shown below.
</div>
...
## Reverse ETL constants

Constants give you the ability to add new fields to each event with a pre-defined value. The **key** of a Reverse ETL constant has the same functionality as modifying an existing column, and is in the format \(**test.value, test.value\[0\]**\).

### How to add a Reverse ETL constant

Adding a new constant when configuring a Reverse ETL source is very easy:
...
Reverse ETL supports only source-driven configuration of your pipeline. So you need to configure a Reverse ETL source in RudderStack and then connect it an existing, **free** destination \(this destination should not be connected to any other source\), as shown:
...
We recommend creating a separate Reverse ETL source with the same settings for each destination where you want to send the data.

### What type of events are supported by the RudderStack Reverse ETL?

Currently all events from the RudderStack Reverse ETL are `identify()` events.
...
To know more about RudderStack's Reverse ETL feature, feel free to [**contact us**](mailto:%20docs@rudderstack.com) or start a conversation in our [**Slack**](https://rudderstack.com/join-rudderstack-slack-community) community.
