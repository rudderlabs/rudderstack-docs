...
1. Enable the user `RUDDER` to perform all the operations allowed for the role `RUDDER_ROLE`(via the privileges granted to it), as shown:

```sql
GRANT ROLE RUDDER_ROLE TO USER RUDDER;
```

2. Then, run the following commands to allow the role `RUDDER_ROLE` to look up the objects within your warehouse, database, schema, and the specific table or view:
...
**Replace `<YOUR_WAREHOUSE>`, `<YOUR_DATABASE>`, `<YOUR_SCHEMA>`, `<YOUR_TABLE>`, and `<YOUR_VIEW>` with the exact names of your Snowflake data warehouse, database, schema, table, and view respectively**.

#### Optional commands

<div class="warningBlock">

Run the following commands <strong>only</strong> if you're okay with RudderStack being able to access all the current/future tables/views within your specified schema. 
</div>
...
- To allow the role `<RUDDER_ROLE>` to read data from all the **future tables** in the schema `<YOUR_SCHEMA>`, run the following command.
