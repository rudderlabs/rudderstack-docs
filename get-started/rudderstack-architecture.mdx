...
title: "Architecture"
description: >-
  Detailed technical description of the RudderStack architecture with a special focus on
  the backend (data plane).
---
...
RudderStack's architecture consists of 2 major components: the **Control plane** and **Data plane**, as seen in the following architecture diagram:

<img src="../assets/general/rudderstack-architecture.png" alt="RudderStack Architecture" />
...
The data plane uses PostgreSQL as a streaming database for the event data. Refer to the <a href="#data-plane-architecture">Data Plane Architecture</a> section below for more information.
</div>
...
<img src="../assets/general/rudderstack-backend-architecture.png" alt="RudderStack backend architecture" />
...
The transformation module takes the event data from the processor, converts it into the required destination format, and sends it back to the processor. The processor then forwards this data to the router, which then sends it to the desired destination.
...
RudderStack also supports user-defined transformations where you can use custom JavaScript functions to implement specific use-cases on your event data, like filtering, sampling, removing sensitive information, etc.

Refer to our <a href="https://github.com/rudderlabs/sample-user-transformers">Transformations GitHub Repository</a> for some useful templates that you can use to create your own user transformations.
</div>

## Data plane workflow

The following steps detail the RudderStack backend (data plane) workflow:
...
<img src="../assets/general/rudderstack-backend-workflow.png" alt="RudderStack Backend Workflow" />

## Customizing the RudderStack backend

Although the default configuration works just fine for most use-cases, RudderStack also gives you the flexibility to customize the backend with a variety of configuration options. Some of these include backing up events to S3, rejecting malicious requests by defining the maximum size of the event, and more.
