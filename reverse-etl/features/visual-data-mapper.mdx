---
# slug: "/docs/reverse-etl/features/visual-data-mapper"
...
In the **Choose Identifier** section, choose a warehouse column and destination field to map your records from the source to the destination. The field options in the dopdown are unique identifiers from which you can chose any column which is the unique identifier. An example is shown below:
...
The <strong>identifier</strong> (Map with Visualizer) or <strong>user_id/anonymous_id</strong> (Map with JSON) fields must have unique values in order to successfully sync the data to the destination. For example, if you have chosen <code class="inline-code">name</code> as the <strong>identifier</strong> and it contains more than one values as 'Alex', the data sync will fail for the duplicate value.
</div>
