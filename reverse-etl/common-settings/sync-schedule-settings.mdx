...
- **Frequency** - Sync frequency can be chosen from the following options:
...
- **Sync Starting At** - Time at which the sync should start.
...
The sync frequency needs to be greater than or equal to 15 minutes. Otherwise, you will encounter an error as shown below:
...
For [upsert mode](https://www.rudderstack.com/docs/reverse-etl/common-settings/sync-modes/#upsert), you can trigger any number of syncs by pressing the **Sync Now** button, which can run simultaneously.

RudderStack takes a snapshot of the data and syncs it at a later point once the pending syncs are completed.

### Mirror mode

For [mirror mode](https://www.rudderstack.com/docs/reverse-etl/common-settings/sync-modes/#mirror-mode), only one sync can be run at any given point of time. Once a sync completes, only then a new one can start.
