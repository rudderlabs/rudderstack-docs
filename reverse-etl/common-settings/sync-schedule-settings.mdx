...
- **Frequency** - Data sync frequency can be chosen from the following options:
  - 30 minutes
  - 1 hour
  - 3 hours
  - 6 hours
...
When a sync schedule is set in RudderStack, the behavior of sync modes can be explained as below:
...
However, suppose you have a sync scheduled for every 30 minutes. If there is a sync running for more than 30 minutes, then the next scheduled sync will be skipped.
