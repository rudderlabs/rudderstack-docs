...
RudderStack lets you schedule data imports from your Reverse ETL sources while setting them up in your dashboard. It lets you specify the schedule type that defines how and when the syncs will run.
...
For [upsert mode](https://www.rudderstack.com/docs/reverse-etl/common-settings/sync-modes/#upsert), you can trigger a new sync anytime by pressing the **Sync Now** button. 

RudderStack takes a snapshot of the data and syncs it at a later point once the pending syncs are completed.

### Mirror mode

For [mirror mode](https://www.rudderstack.com/docs/reverse-etl/common-settings/sync-modes/#mirror-mode), you can run only one sync at any given point of time. 
